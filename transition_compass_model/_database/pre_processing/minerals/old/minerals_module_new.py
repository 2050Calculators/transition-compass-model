...
from .....model.common.auxiliary_functions import filter_geoscale, read_level_data
import pickle
import json
import os
import numpy as np
...
def eol_battery(dm_bev_eol, cdm_bev_matdec):
    ...
    cdm_temp.deepen()
    arr_temp = (
        dm_lib_eol.array[..., np.newaxis] * cdm_temp.array[np.newaxis, np.newaxis, ...]
    )
    dm_lib_matrec = DataMatrix.based_on(
        arr_temp,
        dm_lib_eol,
...
def minerals(lever_setting, years_setting, interface=Interface(), calibration=False):
    ...
    minerals_data_file = os.path.join(
        current_file_directory,
        "../_database/data/datamatrix/geoscale/minerals_new.pickle",
    )
    ...
    DM_industry = get_interface(
        current_file_directory, interface, "industry", "minerals", cntr_list
    )

    # batteries eol
    eol_battery(
        DM_industry["veh-to-recycling"].filter(
            {"Categories1": ["LDV"], "Categories2": ["BEV"]}
        ),
        CDM_const["material-decomposition_bat"].filter(
            {"Categories1": ["battery-lion-LDV"], "Categories2": ["BEV"]}
        ),
    )
