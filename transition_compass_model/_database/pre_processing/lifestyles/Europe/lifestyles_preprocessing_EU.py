...
def get_pop_eurostat(code_pop, EU27_cntr_list, dict_iso2, years_ots):
    ...
    ################################
    filter = {"geo\\TIME_PERIOD": list(dict_iso2.keys()), "age": "TOTAL", "sex": "T"}
    mapping_dim = {"Country": "geo\\TIME_PERIOD", "Variables": "age"}
    ...
    #######################################
    filter = {"geo\\TIME_PERIOD": list(dict_iso2.keys()), "sex": ["F", "M"]}
    mapping_dim = {
    ...
    dm_pop_age.array = (
        dm_pop_age.array
        / np.sum(dm_pop_age.array, axis=-1, keepdims=True)
        * dm_pop_tot.array[..., np.newaxis]
    )
...
def get_pop_eurostat_fts(
    ...
    mapping_dim = {"Country": "geo\\TIME_PERIOD", "Variables": "projection"}
    ...
    factors_2024 = (dm_pop_tot_ots[:, 2024, :] - dm_pop_tot[:, 2024, :]) / dm_pop_tot[
        :, 2024, :
    ...
    for y in years_common:
        other_years.remove(y)
    for y in other_years:
        dm_pop_tot[:, y, :] = dm_pop_tot[:, y, :] * (
            1 + factors_2024
        )  # this is applying the 2024 factors to all future years
...
dm_pop_age, dm_pop_tot = get_pop_eurostat(
    "demo_pjan", EU27_cntr_list, dict_iso2, list(range(1990, 2024 + 1))
)

# Get raw fts pop data (fts)
dict_dm_pop_fts, dict_dm_pop_fts_tot = get_pop_eurostat_fts(
    "proj_23np",
    EU27_cntr_list,
    years_fts,
    dict_iso2,
