...
cdm_feed_yield_sugar = (
    DM_agriculture["constant"]["cdm_feed_yield"]
    .filter({"Categories1": ["molasse-to-sugarcrop", "sugar-to-sugarcrop"]})
    .copy()
)
...
# cp = 1 / cp
array_temp = (
    cdm_food_yield_sugar["temp", :] / cdm_food_yield_sugar["cp_ibp_processed", :]
)
cdm_food_yield_sugar.add(
    array_temp, dim="Variables", col_label="cp_ibp_processed_true", unit="t"
)
array_temp = (
    cdm_feed_yield_sugar["temp", :] / cdm_feed_yield_sugar["cp_ibp_processed", :]
)
cdm_feed_yield_sugar.add(
    array_temp, dim="Variables", col_label="cp_ibp_processed_true", unit="t"
)

# Overwrite
DM_agriculture["constant"]["cdm_food_yield"]["cp_ibp_processed", :] = (
    cdm_food_yield_sugar["cp_ibp_processed_true", :]
)
DM_agriculture["constant"]["cdm_feed_yield"][
    "cp_ibp_processed", "molasse-to-sugarcrop"
] = cdm_feed_yield_sugar["cp_ibp_processed_true", "molasse-to-sugarcrop"]
DM_agriculture["constant"]["cdm_feed_yield"][
    "cp_ibp_processed", "sugar-to-sugarcrop"
] = cdm_feed_yield_sugar["cp_ibp_processed_true", "sugar-to-sugarcrop"]
...
dm_demography = DM_lifestyles["ots"]["pop"]["lfs_demography_"].copy()
dm_population = DM_lifestyles["ots"]["pop"]["lfs_population_"].copy()
dm_cal_diet = DM_agriculture["fxa"][
    "cal_agr_diet"
].copy()  # Now it's actually in (kcal/capita/day)
...
DM_agriculture["fxa"]["cal_agr_diet"]["Switzerland", :, "cal_agr_diet", :] = (
    dm_cal_diet["Switzerland", :, "cal_agr_diet_new", :]
)
...
dm_ssr_processing = DM_agriculture["ots"]["climate-smart-crop"][
    "processing-net-import"
].copy()
...
dm_ssr_feed.operation(
    "cal_agr_demand_feed",
    "*",
    "agr_feed-net-import",
    dim="Variables",
    out_col="agr_domestic-production-feed",
    unit="t",
)
...
dm_sugarcrop = dm_cal_diet.groupby(
    {"crop-sugarcrop": ".*-sweet|.*-sugar"},
    dim="Categories1",
    regex=True,
    inplace=False,
)
...
array_temp = (
    dm_sugarcrop[:, :, "cal_agr_diet_new", :]
    * cdm_food_yield[np.newaxis, np.newaxis, "cp_ibp_processed", :]
)
dm_sugarcrop.add(
    array_temp, dim="Variables", col_label="cal_agr_diet_temp", unit="kcal"
)
...
dm_dom_prod.change_unit("agr_food-net-import", 10**3, old_unit="%", new_unit="t")
# Convert from [t] to [kcal]
array_temp = (
    dm_dom_prod[:, :, "agr_food-net-import", :]
    * cdm_kcal[np.newaxis, np.newaxis, "cp_kcal-per-t", :]
)
dm_dom_prod.add(
    array_temp, dim="Variables", col_label="agr_food-net-import_kcal", unit="kcal"
)
dm_dom_prod = dm_dom_prod.filter({"Variables": ["agr_food-net-import_kcal"]})
...
array_temp = (
    dm_dom_prod_crop.array[:, :, idx_dm["cal_agr_domestic-production_food"], :]
    / cdm_kcal_crop.array[idx_cdm["cp_kcal-per-t"], :]
)
dm_dom_prod_crop.add(
    array_temp, dim="Variables", col_label="cal_agr_domestic-production_t", unit="t"
)
...
# Add ssr food = 1
dm_ssr_processing.add(
    1.0, dummy=True, col_label="agr_food-net-import", dim="Variables", unit="%"
)

# Append with relevant categories
dm_cal_diet_processing = dm_cal_diet.filter(
    {"Categories1": ["crop-oilcrop", "crop-sugarcrop"]}
).copy()
dm_ssr_feed_processing = dm_ssr_feed.filter(
    {
        "Categories1": ["crop-oilcrop", "crop-sugarcrop"],
        "Variables": ["agr_domestic-production-feed"],
    }
).copy()
dm_dom_prod_processing = dm_dom_prod_crop.filter(
    {
        "Categories1": ["crop-oilcrop", "crop-sugarcrop"],
        "Variables": ["cal_agr_domestic-production_t"],
    }
...
array_temp = (
    dm_ssr_processing[:, :, "cal_agr_diet_new", :]
    / cdm_kcal_processing[np.newaxis, np.newaxis, "cp_kcal-per-t", :]
)
dm_ssr_processing.add(
    array_temp, dim="Variables", col_label="cal_agr_diet_new_t", unit="t"
)

# Dom prod food = ssr food * agr demand food
dm_ssr_processing.operation(
    "agr_food-net-import",
    "*",
    "cal_agr_diet_new_t",
    dim="Variables",
    out_col="agr_domestic-production-food",
    unit="t",
)

# Append with stock variation
dm_stock.change_unit(
    "fxa_agr_stock-variation", old_unit="kt", new_unit="t", factor=10**3
)
...
dm_ssr_processing.operation(
    "cal_agr_domestic-production_t",
    "-",
    "agr_domestic-production-food",
    dim="Variables",
    out_col="temp_1",
    unit="t",
)
...
dm_ssr_processing.operation(
    ...
    dim="Variables",
    out_col="agr_domestic-production_processed",
    unit="t",
)

# Unit conversion t => kt
dm_ssr_processing.change_unit(
    "agr_domestic-production_processed", old_unit="t", new_unit="kt", factor=10 ** (-3)
)
...
dm_ssr_processing.rename_col(
    "agr_processing-net-import", "processing_demand", "Variables"
)
dm_ssr_processing.operation(
    "agr_domestic-production_processed",
    "/",
    "processing_demand",
    dim="Variables",
    out_col="agr_processing-net-import",
    unit="%",
)

# Overwrite SSR processing
DM_agriculture["ots"]["climate-smart-crop"]["processing-net-import"][
    :, :, "agr_processing-net-import", :
] = dm_ssr_processing[:, :, "agr_processing-net-import", :]
...
dm_ssr_food.drop(dim="Categories1", col_label=["crop-oilcrop", "crop-sugarcrop"])
dm_ssr_food.operation(
    "cal_agr_domestic-production_t",
    "-",
    "agr_domestic-production-feed",
    dim="Variables",
...
cdm_kcal_crop_temp.drop(dim="Categories1", col_label=["crop-oilcrop", "crop-sugarcrop"])
dm_ssr_food.sort(dim="Categories1")
cdm_kcal_crop_temp.sort(dim="Categories1")
array_temp = (
    dm_ssr_food[:, :, "agr_domestic-production-food", :]
...
dm_cal_diet_temp.drop(dim="Categories1", col_label=["crop-oilcrop", "crop-sugarcrop"])
dm_ssr_food.append(dm_cal_diet_temp, dim="Variables")

# SSR Food [%] = Dom prod food [kcal] / cal_agr_diet_new [kcal]
dm_ssr_food.operation(
    "agr_domestic-production-food_kcal",
    "/",
    "cal_agr_diet_new",
    dim="Variables",
    out_col="agr_food-net-import",
    unit="%",
)
dm_ssr_food = dm_ssr_food.filter({"Variables": ["agr_food-net-import"]})

# Compute Food SSR for other categories used only for food
cat_only_food = list(set(cat_agr) - set(list_cat_crop))
cat_only_food = list(
    set(cat_only_food) - set(["pro-crop-processed-cake", "pro-crop-processed-molasse"])
)
...
dm_dom_prod.operation(
    "agr_food-net-import_kcal",
    "/",
    "cal_agr_diet_new",
    dim="Variables",
    out_col="agr_food-net-import",
...
dm_dom_prod = dm_dom_prod.filter({"Variables": ["agr_food-net-import"]})
...
# Overwrite
DM_agriculture["ots"]["food-net-import"]["Switzerland", :, "agr_food-net-import", :] = (
    dm_dom_prod["Switzerland", :, "agr_food-net-import", :]
)
