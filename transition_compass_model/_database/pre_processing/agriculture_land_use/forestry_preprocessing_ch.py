...
def get_wood_energy_by_sector(file_url, local_filename):
    ...
    df.columns = [
        str(int(col)) if isinstance(col, float) else str(col) for col in df.columns
    ]
    ### translate
    new_names = ["Years"]
    for i in range(1):
    ...
    dm_wood_demand_energy.change_unit(
        "fst_wood-energy-demand", factor=0.27778, old_unit="TJ", new_unit="GWh"
    )
...
def get_wood_energy_by_use(file_url, local_filename, clean_local_filename):
    ...
    # Conversion from TJ to GWh:
    dm_wood_energy_use_gwh.change_unit(
        "fst_wood-energy-use", factor=0.27778, old_unit="TJ", new_unit="GWh"
    )
    dm_wood_energy_use_gwh.rename_col(
        "fst_wood-energy-use", "fst_wood-energy-use-gwh", "Variables"
    )
    dm_wood_energy_use_m3.rename_col(
        "fst_wood-energy-use", "fst_wood-energy-use-m3", "Variables"
    )
    # dm_wood_demand_energy.datamatrix_plot()

    dm_wood_energy_use = dm_wood_energy_use_gwh
    dm_wood_energy_use.append(dm_wood_energy_use_m3, dim="Variables")
    dm_wood_energy_use.operation(
        "fst_wood-energy-use-gwh",
        "/",
        "fst_wood-energy-use-m3",
        out_col="fst_energy-density",
        unit="gwh/m3",
    )
...
def get_wood_trade_balance(file):
    try:
        ...
        dm = dm.filter_w_regex(
            {
                "Variables": "industrial roundwood|industrial roundwood, coniferous|industrial roundwood, non-coniferous|\
...
def simulate_industry_input(write_pickle=True):
    if write_pickle is True:
        ...
        # Write Pickle
        f = os.path.join(
            current_file_directory, "../../data/interface/industry_to_forestry.pickle"
        )
...
def simulate_land_input(dm_forest_area, write_pickle=True):
    if write_pickle is True:
        ...
        current_file_directory = os.path.dirname(os.path.abspath(__file__))
        f = os.path.join(
            current_file_directory, "../../data/interface/land_to_forestry.pickle"
        )
        my_pickle_dump(DM_new=dm, local_pickle_file=f)
...
def simulate_industry_other_wood(refresh=True):
    if refresh is True:
        ...
        dm_industry_calibration = DataMatrix.create_from_df(
            df_industry_calibration, num_cat=1
        )
        ...
        ay_fxa_wood_other = dm_industry_calibration[...] - dm_industry_wood_demand[...]
        dm_industry_wood_demand.add(
            ay_fxa_wood_other, col_label="fxa_any-other-wood", dim="Variables", unit="t"
        )
        dm_fxa_wood_demand = dm_industry_wood_demand
        # Sum-up the missing wood
        dm_fxa_wood_demand.filter({"Variables": ["fxa_any-other-wood"]})
        dm_fxa_wood_demand.groupby(
            {"any-other-industrial-demand": ["other-industrial", "pulp", "timber"]},
            dim="Categories1",
            inplace=True,
        )
...
def create_dummy_Vaud_waste(dm_forest_area, dm_wood_wastes, years_ots):
    ...
    dm_tmp.filter(
        {
            "Country": ["Vaud-share"],
            "Variables": ["productive-forest-area"],
            "Years": years_ots,
        },
        inplace=True,
    )
    ...
    dm_wood_wastes_VD.operation(
        "fst_waste-wood",
        "*",
        "productive-forest-area",
        out_col="fst_waste-wood_VD",
        unit="m3",
        dim="Variables",
    )
...
lfs_interface_data_file = os.path.join(
    this_dir, "../../data/interface/energy_to_forestry.pickle"
)
...
dm_harvest_rate_all = dm_harvest_rate.groupby(
    {"total": ["coniferous", "non-coniferous"]}, dim="Categories2", inplace=False
)
...
cdm_wood_density = ConstantDataMatrix(
    col_labels={
        "Variables": ["fst_wood-density"],
        "Categories1": list(forestry_wood_density.keys()),
    },
    units={"fst_wood-density": "m3/t"},
)

cdm_wood_density.array = np.zeros(
    (
        len(cdm_wood_density.col_labels["Variables"]),
...
arr_tonnes = (
    dm[:, :, "fst_production-m3", :, :]
    / cdm_conv[np.newaxis, np.newaxis, "fst_wood-density", :, :]
)
...
dm.rename_col(
    [
        "Total forest area_Type of owners - total",
        "Productive forest area_Type of owners - total",
    ],
...
dm.operation(
    ...
    "-",
    "productive-forest-area",
    out_col="unproductive-forest-area",
    unit="ha",
)
dm.operation(
...
dm_harvest_rate_all.add(
    ay_harvest_rate, col_label="harvest-rate", dim="Variables", unit="m3/ha"
)
...
local_filename = "data/Swiss-wood-energy-statistics.xlsx"
clean_files = {
    "energy-use-m3": "data/wood-energy-m3.csv",
    "energy-use-ghw": "data/wood-energy-ghw.csv",
}
dm_wood_energy_use = get_wood_energy_by_use(
    file_url, local_filename, clean_local_filename=clean_files
)
...
DM_preprocessing_forestry = {
    "calibration": dm_wood_energy_calibration,
...
forestry_energy_density = {
    "average": 2.326,
    "non-coniferous": 2.6749,
    "coniferous": 2.50045,
}
cdm_energy_density = ConstantDataMatrix(
    col_labels={
        "Variables": ["fst_energy-density"],
        "Categories1": ["average", "non-coniferous", "coniferous"],
    },
    units={"fst_energy-density": "MWh/m3"},
)

cdm_energy_density.array = np.zeros(
    (
        len(cdm_energy_density.col_labels["Variables"]),
...
forestry_industry_yields = {
    "wood-fuel-byproducts": 0.5,
    "timber-to-sawlogs:": 1.67,
    "pulp-to-industrial-wood": 2.5,
}

cdm_industry_yields = ConstantDataMatrix(
    col_labels={
        "Variables": ["fst_industry-yields"],
        "Categories1": [
            "wood-fuel-byproducts",
            "timber-to-sawlogs:",
...
cdm_industry_yields.array = np.zeros(
    (
        len(cdm_industry_yields.col_labels["Variables"]),
...
forestry_conversion_industry_to_wood = {
    "other-industrial": 1,
    "timber:": 1.67,
    "pulp": 2.5,
}

cdm_forestry_conversion_industry_to_wood = ConstantDataMatrix(
    col_labels={
        "Variables": ["fst_industry-to-wood-category"],
        "Categories1": list(forestry_conversion_industry_to_wood.keys()),
    },
    units={"fst_industry-to-wood-category": "%"},
)

cdm_forestry_conversion_industry_to_wood.array = np.zeros(
    (
        len(cdm_forestry_conversion_industry_to_wood.col_labels["Variables"]),
...
linear_fitting(
    dm_wood_type,
    dm_wood_type.col_labels["Years"],
    based_on=create_years_list(2004, 2023, 1),
)
...
dm_wood_wastes = dm_wood_wastes.filter(
    {
        "Variables": [
            "fst_wood-energy-use-m3_waste-incineration",
            "fst_wood-energy-use-m3_waste-without-incineration",
        ]
    }
)
dm_wood_wastes.groupby(
    {"fst_waste-wood": ".*"}, regex=True, inplace=True, dim="Variables"
)
...
dm_harvest_coniferous = DataMatrix(
    col_labels={
        "Country": ["Switzerland"],
        "Years": years_all,
        "Variables": ["sustainable-harvest-rate_coniferous"],
    },
    units={"sustainable-harvest-rate_coniferous": "m3/ha"},
)
for yr, val in values_dict.items():
    dm_harvest_coniferous["Switzerland", yr, "sustainable-harvest-rate_coniferous"] = (
        val
    )
...
years_all = create_years_list(1990, 2056, 1)
dm_harvest_non_coniferous = DataMatrix(
    col_labels={
        "Country": ["Switzerland"],
        "Years": years_all,
        "Variables": ["sustainable-harvest-rate_non-coniferous"],
    },
    units={"sustainable-harvest-rate_non-coniferous": "m3/ha"},
)
for yr, val in values_dict.items():
    dm_harvest_non_coniferous[
        "Switzerland", yr, "sustainable-harvest-rate_non-coniferous"
    ] = val
...
# Total
dm_sustainable_harvest_rate.operation(
    "coniferous",
    "+",
    "non-coniferous",
    out_col="total",
...
dm_fts = DataMatrix(
    col_labels={
        "Country": ["Switzerland"],
        "Years": years_all,
        "Variables": ["harvest-rate_coniferous"],
    },
    units={"harvest-rate_coniferous": "m3/ha"},
)
for yr, val in values_dict.items():
...
dm_fts = DataMatrix(
    col_labels={
        "Country": ["Switzerland"],
        "Years": years_all,
        "Variables": ["harvest-rate_non-coniferous"],
    },
    units={"harvest-rate_non-coniferous": "m3/ha"},
)
for yr, val in values_dict.items():
...
for lev in range(4):
    DM_forestry["fts"]["harvest-rate"][lev + 1] = dm_harvest.filter(
        {"Years": years_fts}
    )
