...
# make dm
dm_ots = make_dm(
    df_ots, 2023, 1990, 2022, 1, agg_prod_dict=agg_prod_dict, agg_mat_dict=agg_mat_dict
)
...
dm_fts_level1 = make_dm_fts(
    df_mat, "SSP5-Base", agg_prod_dict=agg_prod_dict, agg_mat_dict=agg_mat_dict
)
dm_fts_level2 = make_dm_fts(
    df_mat, "SSP2-NPi", agg_prod_dict=agg_prod_dict, agg_mat_dict=agg_mat_dict
)
dm_fts_level3 = make_dm_fts(
    df_mat, "SSP2-NDC", agg_prod_dict=agg_prod_dict, agg_mat_dict=agg_mat_dict
)
dm_fts_level4 = make_dm_fts(
    df_mat, "SSP1-PkBudg1150", agg_prod_dict=agg_prod_dict, agg_mat_dict=agg_mat_dict
)
...
def make_dm_dict(df, pattern, deepen_n_cat=1):
    ...
    # fts
    dm_fts_level1 = make_dm_fts(
        df_sub, "SSP5-Base", agg_prod_dict=agg_prod_dict, deepen_n_cat=deepen_n_cat
    )
    dm_fts_level2 = make_dm_fts(
        df_sub, "SSP2-NPi", agg_prod_dict=agg_prod_dict, deepen_n_cat=deepen_n_cat
    )
    dm_fts_level3 = make_dm_fts(
        df_sub, "SSP2-NDC", agg_prod_dict=agg_prod_dict, deepen_n_cat=deepen_n_cat
    )
    dm_fts_level4 = make_dm_fts(
        df_sub,
        "SSP1-PkBudg1150",
...
DM_current = {
    "materials": DM_mat,
    "ene-demand-elec": DM_ene_dem_elec,
    "ene-demand-ff": DM_ene_dem_ff,
    "ecological": DM_eco,
    "global-warming-potential": DM_gwp,
    "water": DM_water,
    "air-pollution": DM_air,
    "heavy-metals-in-soil": DM_heavy_metals,
}

for key in DM_current.keys():
    DM["ots"][key] = DM_current[key]["ots"].copy()
    DM["fts"][key] = {
        1: DM_current[key]["fts1"].copy(),
