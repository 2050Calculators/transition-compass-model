...
def run():
    ...
    dm_energy_agriculture = dm_energy.groupby(
        {"agriculture": [" 0103 Agriculture, sylviculture et pêche"]},
        dim="Categories1",
        inplace=False,
    )
    ...
    dm_employees_agriculture = dm_employees.groupby(
        {
            "agriculture": [
    ...
    arr_energy_canton = (
        dm_energy_agriculture[:, :, "bld_energy-by-sector", "agriculture", :]
        * dm_employees_agriculture[:, :, "ind_employees", "agriculture", np.newaxis]
    )
    dm_energy_agriculture.add(
        arr_energy_canton[:, :, np.newaxis, np.newaxis],
        dim="Country",
        col_label=dm_employees_agriculture.col_labels["Country"],
    )
    ...
    dm_energy_agriculture.rename_col(
        "bld_energy-by-sector", "agr_energy-consumption", dim="Variables"
    )
    ...
    file_agriculture = os.path.join(
        this_dir, "../../../data/interface/agriculture_to_energy.pickle"
    )
