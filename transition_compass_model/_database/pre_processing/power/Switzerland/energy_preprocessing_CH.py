...
def read_table_blocks(filename):
    with open(filename, "r") as file:
        ...
        for line in file:
            ...
            if line == "":
                if block:
                    ...
                    if any(
                        re.search(r":(?!=)", l) and "\t" in l and "param" in l
                        for l in block
                    ):  # Likely a table block
                        yield [
                            l for l in block if not l.startswith("#")
        ...
        if block:
            if any(
                re.search(r":(?!=)", l) and "\t" in l and "param" in l for l in block
            ):  # Likely a table block
                yield [
                    l for l in block if not l.startswith("#")
                ]  # Exclude lines starting with '#'
...
def extract_tables(filename):
    ...
    for block in read_table_blocks(filename):
        ...
        for row in rows:
            ...
            data_dict[index] = {
                col: float(val) for col, val in zip(columns, row_values[1:])
            }

        table_dict[param_name] = {
            "data": data_dict,
            "columns": columns,
            "rows": indexes,
        }
        ...
        if "index" in var:
            ...
            for cat1 in categories1:
                for cat2 in categories2:
                    dm[country, :, var, cat1, cat2] = table_dict[var]["data"][cat1][
                        cat2
                    ]
