...
def extract_energy_data(file_url, local_filename, baseyear, years_ots, outfile_dm):
    ...
    if not os.path.exists(outfile_dm):
        ...
        df[["timescale", "Variable", "Description", "value"]] = df["Jahr"].str.split(
            '",', expand=True
        )
        ...
        dict_ots, dict_fts = database_to_dm(
            df, lever, num_cat=1, baseyear=baseyear, years=years_ots, level="all"
        )
...
if plotting:
    ...
    dm_energy.datamatrix_plot(
        {"Variables": "Energy conversion - Nuclear power plants"},
        title="Energy conversion - Nuclear power plants [TWh]",
    )
    dm_energy.datamatrix_plot(
        {
            "Variables": "Energy conversion - conventional-thermal power, district heating and district heating power plants"
        },
...
dm_capacity_group.groupby(
    {"Gas": "Gas.*", "Hydro": "Dam|RoR"}, regex=True, dim="Categories1", inplace=True
)
...
dm_capacity_waste_ots = adjust_based_on_nexuse(
    dm=dm_capacity_waste_ots, dm_nexuse=dm_capacity
)
...
dm_import = extract_importexport_data(
    file_url, local_filename, sheet_name="T06", var_name="pow_import", mapping=mapping
)
...
dm_export = extract_importexport_data(
    file_url, local_filename, sheet_name="T07", var_name="pow_export", mapping=mapping
)
dm_export.change_unit("pow_export", old_unit="MWh", new_unit="TWh", factor=1e-6)
# Compute Net Import
cat_missing = list(
    set(dm_import.col_labels["Categories1"]) - set(dm_export.col_labels["Categories1"])
)
...
dm_CH.append(
    dm_capacity_PV_wind_ots.filter({"Country": ["Switzerland"], "Years": years_ots}),
    dim="Categories1",
)
...
missing_ots_cat = list(
    set(dm_capacity.col_labels["Categories1"]) - set(dm_CH.col_labels["Categories1"])
)
...
dm_capacity.add(
    np.nan, dim="Variables", col_label="pow_existing-capacity", dummy=True, unit="MW"
)
for yr in years_ots:
...
dm_production.add(
    arr_prod_split, dim="Categories1", col_label=dm_cap_oilgas.col_labels["Categories1"]
)
...
dm_fuels_demand = extract_energy_statistics_data(
    file_url, local_filename, sheet_name="T14", parameters=parameters
)
...
dm_fuels_supply = extract_energy_statistics_data(
    file_url, local_filename, sheet_name="T10", parameters=parameters
)
...
dm_losses = extract_energy_statistics_data(
    file_url, local_filename, sheet_name="T13", parameters=parameters
)
...
dm_oil_split = extract_energy_statistics_data(
    file_url, local_filename, sheet_name="T20", parameters=parameters
)
...
# SECTION - Power capacity levers (nuclear, onshore wind, PV)
...
nuclear_both_raw_mw = reactor_list["Gosgen"]["Pmax"] + reactor_list["Liebstadt"]["Pmax"]
...
for lever_name in ["onshore-wind-capacity", "pv-capacity"]:
    ...
    level1_value = dm_capacity["Switzerland", endyr, "pow_existing-capacity", category]
