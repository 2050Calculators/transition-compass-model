...
saved_files_directory = os.path.join(
    current_file_directory, "../data/eol_intermediate_files"
)
...
def extend_and_sort_years(df, country_col="geoscale", year_col="timescale"):
    ...
    merged_df = merged_df.groupby("Country").apply(
    ...
    merged_df["Country_sort_key"] = merged_df["Country"].apply(
        lambda country: "ZZZ" if country == "EU27" else country
    )
...
df_pc.rename(
    columns={
        col: f"pc_{col}" for col in df_pc.columns if col not in ["Country", "Years"]
    },
    inplace=True,
)
df_elv.rename(
    columns={
        col: f"elv_{col}" for col in df_elv.columns if col not in ["Country", "Years"]
    },
    inplace=True,
)
df_tv.rename(
    columns={
        col: f"tv_{col}" for col in df_tv.columns if col not in ["Country", "Years"]
    },
    inplace=True,
)
df_LA.rename(
    columns={
        col: f"larger-appliances_{col}"
    ...
    for year in future_years:
        for lever in range(1, 5):  # Ensure levels 1 to 4 are included
            ...
            if temp_df.empty:
                ...
                nearest_year = (
                    df_country["Years"].sub(year).abs().idxmin()
                )  # Find the nearest year
                temp_df = df_country[
                    df_country["Years"] == df_country.loc[nearest_year, "Years"]
                ].copy()
                if temp_df.empty:  # Still empty, use zeros
                    temp_df = pd.DataFrame(
                        np.zeros((1, len(df_country.columns))),
                        columns=df_country.columns,
                    )
                temp_df["Years"] = year
                temp_df["Country"] = df_country["Country"].iloc[
                    0
                ]  # Assuming country name is consistent
...
for country in unique_countries:
    ...
    df_country_combined = pd.concat(
        [df_country, expanded_country_df], ignore_index=True, sort=False
    )
...
def apply_thresholds(df, category, thresholds):
    ...
    for level, threshold in enumerate(
        thresholds["recovery"]
    ):  # Loop through each 'lever_eol' level and corresponding threshold
        # print(f"Processing threshold: {threshold} for level {level}")
        mask = df["lever_eol"] == level
        adjust_waste_columns(
            df,
            mask,
            waste_columns[:2],
            [threshold, thresholds["recycling"][level]],
            "max",
        )  # Apply thresholds for recovery and recycling first
        df.loc[mask, waste_columns[3]] = (
            df.loc[mask, waste_columns[0]]
            - df.loc[mask, waste_columns[1]]
            - df.loc[mask, waste_columns[2]]
        )  # Dynamically adjust energy-recovery based on the applied recovery and recycling
        df.loc[mask, waste_columns[3]] = np.maximum(
            df.loc[mask, waste_columns[3]], 0
        )  # Ensure that energy recovery does not go below zero
        adjust_waste_columns(
            df, mask, waste_columns[5:], [thresholds["incineration"][level]], "min"
        )  # Apply minimum adjustment for incineration
        df.loc[mask, waste_columns[4]] = (
            1 - df.loc[mask, waste_columns[0]] - df.loc[mask, waste_columns[5]]
        )  # Dynamically calculate landfill based on the remaining sum
        df.loc[mask, waste_columns[4]] = np.maximum(
            df.loc[mask, waste_columns[4]], 0
        )  # Ensure that landfill does not go below zero
        ...
        mismatch_waste = ~(
            np.isclose(total_waste_management, 1, atol=0.0001)
        )  # Keep mismatch under 0.00001
        mismatch_indices = df.loc[mask].index[
            mismatch_waste
        ]  # Get the indices of mismatches
        df.loc[mismatch_indices, waste_columns[4]] += (
            1 - total_waste_management[mismatch_waste]
        )
    return df
...
def ensure_balances(df, category):
    waste_columns = [
        f"{category}_recovery[%]",
        f"{category}_recycling[%]",
        f"{category}_reuse[%]",
        f"{category}_energy-recovery[%]",
        f"{category}_landfill[%]",
        f"{category}_incineration[%]",
    ]
    collected_waste = (
        df[waste_columns[0]] + df[waste_columns[4]] + df[waste_columns[5]]
    )  # collected waste = recovery + landfill + and incineration
    mismatch_recovery = (
        np.isclose(collected_waste, 1, atol=0.00001) == False
    )  # Identify rows where the collected waste is not ~1 (with a tolerance of 0.00001)
    recovery_difference = (
        1 - collected_waste
    )  # Calculate the difference between 1 and the collected waste for mismatched rows
    df.loc[mismatch_recovery, waste_columns[4]] += recovery_difference[
        mismatch_recovery
    ]  # Adjust the landfill value by adding the recovery difference for mismatched rows
    return df
...
df = df[
    [c for c in df.columns if c != "lever_eol"][: df.columns.get_loc("Years") + 1]
    + ["lever_eol"]
    + [c for c in df.columns if c != "lever_eol"][df.columns.get_loc("Years") + 1 :]
]
...
for column in df.columns:
    if (
        column.startswith("pc_")
        or column.startswith("phone_")
        or column.startswith("tv_")
    ):
        ...
        for term in larger_appliances_terms:
            ...
            new_columns[new_column_name] = df[
                column
            ].copy()  # Copy the original column data
...
save_file_directory = os.path.join(
    current_file_directory, "../data/ind_eol_waste-management.xlsx"
)
...
df = df.sort_values(
    by=["Country", "Years", "lever_eol", "variable", "wst_man_operation"]
)
df["wst_man_operation"].unique()
