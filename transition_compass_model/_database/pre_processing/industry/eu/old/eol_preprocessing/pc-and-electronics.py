...
df_pc["geo\\TIME_PERIOD"] = df_pc["geo\\TIME_PERIOD"].apply(
    lambda x: (
...
excluded_countries = [
    ...
    "Liechtenstein",
    "EU27",
]  # List of non-EU27 countries to be summed up in future steps
included_countries = [
    country for country in countries if country not in excluded_countries
]  # List of countries to include in regression

columns_to_regress = [
    "waste-collected[t]",
    "landfill-mun[t]",
    "incineration-mun[t]",
]  # Define the columns to regress against time

for country in included_countries:
    # print(f"Processing data for country: {country}")
    country_data = merged_df[
        merged_df["geoscale"] == country
    ]  # Create a new df containing only the data for the current country
    if country != "EU27":  # Skip 'EU27' to avoid double-counting
        X_country = (
            country_data["timescale"].astype(float).values.reshape(-1, 1)
        )  # Convert timescale to float and reshape
        for column in columns_to_regress:  # Loop only through columns of interest
            y = (
                country_data[column].astype(float).values
            )  # Convert the specified column to float array
            valid_indices = ~pd.isna(y)  # Keep rows with valid values
            relative_missing_indices = np.where(pd.isna(y))[
                0
            ...
            if relative_missing_indices.size > 0 and valid_indices.sum() > 1:
                ...
                merged_df.loc[country_data.index[relative_missing_indices], column] = (
                    np.maximum(y_pred, 0)
    ...
    for column in columns_to_regress:
        ...
        if column not in calculated_columns:
            ...
            if pd.isna(eu27_data.loc[(eu27_data["timescale"] == year), column]).any():
                ...
                aggregate_value = merged_df[
                    (merged_df["timescale"] == year)
                    & (merged_df["geoscale"].isin(included_countries))
                ][column].sum()
...
exception_countries = [
    ...
    "Finland",
    "Liechtenstein",
]  # These countries do not need to be regressed for 'export[t]'
...
for country in df["geoscale"].unique():
    ...
    if country != "EU27":  # Skip 'EU27' to avoid double-counting
        X_country = (
            country_data["waste-collected[t]"].astype(float).values.reshape(-1, 1)
        )  # Use waste-collected[t] as the independent variable for regression
        for column in columns_to_regress_2:
            if column == "export[t]" and country in exception_countries:
                ...
                df.loc[country_data.index, "export[t]"] = df.loc[
                    country_data.index, "export[t]"
                ].fillna(0)
            ...
            if relative_missing_indices.size > 0 and valid_indices.sum() > 1:
                ...
                df.loc[country_data.index[relative_missing_indices], column] = (
                    np.maximum(y_pred, 0)
                )  # Ensure non-negative values
...
X_predict = (
    df_italy[df_italy["timescale"].isin(prediction_years)]["waste-collected[t]"]
    .astype(float)
    .values.reshape(-1, 1)
)
...
df.loc[
    (df["geoscale"] == "Italy") & (df["timescale"].isin(prediction_years)),
    "recovery[t]",
] = predicted_recovery.astype(int)
...
for year in eu27_data["timescale"].unique():
    for (
        ...
        if column not in calculated_columns:
            ...
            aggregate_value = df[
                (df["timescale"] == year) & (df["geoscale"].isin(eu27_countries))
            ][column].sum()
...
for year in df["timescale"].unique():
    for column in eu27_columns:
        ...
        eu27_value = df.loc[
            (df["geoscale"] == "EU27") & (df["timescale"] == year), column
        ].values[0]
        if eu27_value == 0:
...
df["export[%]"] = (df["export[t]"] + df["illegal-export[t]"]) / df[
    "total-waste[t]"
]  # This is the total export
...
save_file_directory = os.path.join(
    current_file_directory, "../data/eol_intermediate_files/pc_and_electronics.xlsx"
)
