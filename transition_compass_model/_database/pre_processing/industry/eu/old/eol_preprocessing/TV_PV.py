...
def transform_to_long_format(
    ...
    if (
        waste_col_name
    ):  # Include waste_col_name in the pivot index only if it is provided
    ...
    long_df = long_df.sort_values(
        by=["geoscale", timescale_col_name], ascending=[True, True]
    )
    return long_df
...
# Step 2: Filter only the Waste category: Consumer equipment and photovoltaic panels
waste_categories = ["EE_CPV"]  # TV & PV (until 2018)
df_tv = df[df["waste"].isin(waste_categories)]
...
waste_management = [
    ...
    "RCY_PRP_REU",
    "PRP_REU",
]  # Waste collected, Waste treated outside the EU, Recovery, Recycling and preparing for reuse, Preparing for reuse
df_tv = df_tv[df_tv["wst_oper"].isin(waste_management)]
...
df_tv = df_tv.replace(replace_dict)
...
df_tv["geo\\TIME_PERIOD"] = df_tv["geo\\TIME_PERIOD"].apply(
    lambda x: (
        convert_country_code_to_name(x) if isinstance(x, str) and x != "EU27" else x
    )
)

# Step 6: Convert the wide format to long format using the pre-defined function
df_tv = transform_to_long_format(
    df=df_tv,
...
replace_dict = {
    "geo\\TIME_PERIOD": {"EU27_2020": "EU27"},
    "wst_oper": {"DSP_L_OTH": "landfill-mun", "DSP_I": "incineration-mun"},
}
# Use the replace function to rename values
df_mun = df_mun.replace(replace_dict)
# Step 4: Convert Eurostat country codes to English names using pycountry
df_mun["geo\\TIME_PERIOD"] = df_mun["geo\\TIME_PERIOD"].apply(
    lambda x: (
        convert_country_code_to_name(x) if isinstance(x, str) and x != "EU27" else x
    )
)
...
df_mun = transform_to_long_format(
    ...
    id_vars=[
        "geo\\TIME_PERIOD",
        "wst_oper",
    ],  # Columns to be kept as identifiers during the melt operation
    value_vars=[
        str(year) for year in range(2007, 2019)
    ],  # Columns representing the years to unpivot
    value_col_name="value",
    timescale_col_name="timescale",
    geoscale_col_name="geo\\TIME_PERIOD",
    wst_oper_col_name="wst_oper",
...
# Step 1: Merging df_tv and df_mun based on geoscale and timescale
merged_df = pd.merge(df_mun, df_tv, on=["geoscale", "timescale"], how="outer")
...
merged_df = merged_df.sort_values(
    by=["sort_order", "geoscale", "timescale"], ascending=[True, True, True]
)
...
df["reuse[t]"] = df["reuse[t]"].fillna(0)  # Set reuse nan to 0
df.loc[df["geoscale"].isin(geoscale_values), :] = df.loc[
    df["geoscale"].isin(geoscale_values), :
].fillna(0)
# Specific exceptions: export[t] for Croatia, Italy, Poland (except 2018), Portugal
df.loc[(df["geoscale"] == "Italy"), "export[t]"] = df.loc[
    (df["geoscale"] == "Italy"), "export[t]"
].fillna(0)
df.loc[(df["geoscale"] == "Croatia"), "export[t]"] = df.loc[
    (df["geoscale"] == "Croatia"), "export[t]"
].fillna(0)
df.loc[(df["geoscale"] == "Poland") & (df["timescale"] != 2018), :] = df.loc[
    (df["geoscale"] == "Poland") & (df["timescale"] != 2018), :
].fillna(0)

# 3.3 Linear extrapolation for Italy based on "waste-collected[t]"
italy_data = df[df["geoscale"] == "Italy"].copy()
# Ensure that 'waste-collected[t]' has enough data points for regression
if italy_data["waste-collected[t]"].notna().sum() > 1:
    # Iterate through all columns except 'waste-collected_t' and 'timescale'
    for column in [
        col
        for col in italy_data.columns
        if col not in ["waste-collected[t]", "timescale", "geoscale"]
    ]:
        # Check if there are missing values to fill
        if italy_data[column].isna().any():
            # Prepare data for regression: drop rows where either dependent or independent variable is NaN
            clean_data = italy_data.dropna(subset=[column, "waste-collected[t]"])
            # Fit linear model only if there are enough non-NaN data points
            if len(clean_data) > 1:
                x = clean_data["waste-collected[t]"].values.reshape(
                    -1, 1
                )  # Independent variable
                y = clean_data[column].values  # Dependent variable
                # Create and fit the model
                from sklearn.linear_model import LinearRegression

                model = LinearRegression()
                model.fit(x, y)
                # Predict and fill NaNs in the original column
                x_pred = italy_data.loc[
                    italy_data[column].isna(), "waste-collected[t]"
                ].values.reshape(-1, 1)
                italy_data.loc[italy_data[column].isna(), column] = model.predict(
                    x_pred
                )
...
# 4: Linear extrapolation for other geoscales
for geo in countries:
    if geo not in ["Italy", "EU27", "Croatia", "Slovenia"] + geoscale_values:
        geo_data = df[df["geoscale"] == geo].copy()
        for column in geo_data.select_dtypes(include=[np.number]).columns:
            if geo_data[column].notna().sum() > 1:  # Ensure at least two data points
                slope, intercept = np.polyfit(
                    geo_data.loc[geo_data[column].notna(), "timescale"],
                    geo_data.loc[geo_data[column].notna(), column],
                    1,
                )
                geo_data.loc[geo_data[column].isna(), column] = np.maximum(
                    0, geo_data["timescale"] * slope + intercept
                )
        df.update(geo_data)
...
sum_data = df[
    ~df["geoscale"].isin(excluded_geoscales + ["EU27"])
]  # Exclude specific geoscales and drop specified columns before summing
summed_data = (
    sum_data.groupby("timescale").sum(min_count=1).reset_index()
)  # Sum data grouped by 'timescale', with handling for NaN values
sum_dict = summed_data.set_index("timescale").to_dict("index")
eu27_data = df[
    df["geoscale"] == "EU27"
].copy()  # Filling values for EU27 explicitly for all numeric columns
# Ensure that all numeric columns get aggregated (including export[t], reuse[t], etc.)
for column in eu27_data.select_dtypes(include=[np.number]).columns:
    if column not in ["geoscale", "timescale"]:  # Exclude non-data columns
        # Update with the summed value even if column contains values, by using the sum_dict
        eu27_data[column] = eu27_data.apply(
            lambda row: (
                sum_dict.get(row["timescale"], {}).get(column, np.nan)
                if pd.isna(row[column])
                else sum_dict.get(row["timescale"], {}).get(column, row[column])
            ),
            axis=1,
        )
# Update the main dataframe with the updated EU27 data
df.update(eu27_data)
...
df["landfill[t]"] = (
    df["landfill-mun[t]"]
    / (df["landfill-mun[t]"] + df["incineration-mun[t]"])
    * (df["waste-collected[t]"] - df["recovery[t]"])
)
# 8. Incineration calculation
df["incineration[t]"] = (
    df["incineration-mun[t]"]
    / (df["landfill-mun[t]"] + df["incineration-mun[t]"])
    * (df["waste-collected[t]"] - df["recovery[t]"])
)
...
df["total-waste[t]"] = (
    df["waste-collected[t]"]
    + df["waste-uncollected[t]"]
    + df["illegal-export[t]"]
    + df["export[t]"]
    + df["littered[t]"]
)
...
save_file_directory = os.path.join(
    current_file_directory, "../data/eol_intermediate_files/tv.xlsx"
)
