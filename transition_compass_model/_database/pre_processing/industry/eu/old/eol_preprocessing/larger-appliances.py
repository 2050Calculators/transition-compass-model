...
df_LA["geo\\TIME_PERIOD"] = df_LA["geo\\TIME_PERIOD"].apply(
    lambda x: (
...
def fillna_specific_columns(df, conditions):
    ...
    for geoscale, column in conditions.items():
        df.loc[df["geoscale"] == geoscale, column] = df.loc[
            df["geoscale"] == geoscale, column
        ].fillna(0)
    return df
...
if italy_data["waste-collected[t]"].notna().sum() > 1:
    for column in [
        ...
        if col not in ["waste-collected[t]", "timescale", "geoscale", "export[t]"]
    ]:
        if italy_data[column].isna().any():
            ...
            if len(clean_data) > 1:
                ...
                italy_data.loc[italy_data[column].isna(), column] = (
                    x_pred * slope + intercept
                )
...
if italy_data["export[t]"].notna().sum() > 1:
    if italy_data["export[t]"].isna().any():
        ...
        if len(clean_data) > 1:
            ...
            italy_data.loc[italy_data["export[t]"].isna(), "export[t]"] = (
                x_pred * slope + intercept
            )
...
sum_data = df[~df["geoscale"].isin(excluded_countries + ["EU27"])].drop(
    columns=["geoscale"], errors="ignore"
)
    ...
    for column in eu27_data.columns:
        if column not in ["geoscale", "timescale"]:
            ...
            if eu27_value == 0 or pd.isna(eu27_value):
                ...
                eu27_sum = df[
                    (df["timescale"] == year)
                    & (df["geoscale"].isin(df["geoscale"].unique()))
                ][column].sum()
...
df.loc[:, df.select_dtypes(include=[np.number]).columns] = df.select_dtypes(
    include=[np.number]
).clip(lower=0)
...
save_file_directory = os.path.join(
    current_file_directory, "../data/eol_intermediate_files/LA.xlsx"
)
