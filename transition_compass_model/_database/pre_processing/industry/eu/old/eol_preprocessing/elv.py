...
df_elv_1 = df[
    (df["waste"] == "ELV") & (df["wst_oper"].isin(["GEN", "RCY", "RCV", "REU"]))
]
...
df_elv_3 = df[
    (df["waste"].isin(["DMDP", "W1910"]))
    & (df["wst_oper"].isin(["RCV_E", "DSP", "RCV"]))
]
...
combination_replace_dict = {
    ...
    (
        "DMDP",
        "RCV_E",
    ): "R1_DMDP",  # Energy recovery of waste from dismantling and de-pollution
...
df_elv["geoscale"] = df_elv["geoscale"].apply(
    lambda x: (
...
included_years = [
    year for year in df["timescale"].unique() if year not in excluded_years
]  # Include all other years
...
included_countries = [
    country for country in df["geoscale"].unique() if country not in excluded_countries
]
...
for year in eu27_data["timescale"].unique():
    if pd.isna(eu27_data.loc[eu27_data["timescale"] == year, "GEN"]).any():
        aggregate_value = df[
            (df["timescale"] == year) & (df["geoscale"].isin(included_countries))
        ]["GEN"].sum()
        df.loc[(df["geoscale"] == "EU27") & (df["timescale"] == year), "GEN"] = (
            aggregate_value
        )
...
for country in included_countries:
    ...
    if missing_indices.size > 0 and len(y) - len(missing_indices) > 1:
        ...
        slope, intercept = np.polyfit(
            X_country[~pd.isna(y)].flatten(), y[~pd.isna(y)], 1
        )
        y_pred = slope * X_country[missing_indices].flatten() + intercept
        country_data.iloc[missing_indices, country_data.columns.get_loc("GEN")] = (
            np.maximum(0, y_pred)
        )
...
for country in df["geoscale"].unique():
    ...
    if country != "EU27":  # Skip 'EU27' to avoid double-counting
        X_country = country_data["GEN"].values.reshape(
            -1, 1
        )  # Use 'GEN' column as the independent variable for regression
        for column in columns_to_regress:
            ...
            if missing_indices.size > 0 and valid_indices.sum() > 1:
                try:
                    ...
                    slope, intercept = np.polyfit(
                        X_country[valid_indices].flatten(), y[valid_indices], 1
                    )
                    y_pred = slope * X_country[missing_indices].flatten() + intercept
                    # print(f"Predicted values for {country} in {column}: {y_pred}")
                    # Fill the missing values in the original dataframe
                    country_data.loc[country_data.index[missing_indices], column] = (
                        np.maximum(0, y_pred)
                    )
...
for year in eu27_data["timescale"].unique():
    for column in columns_to_regress:
        if pd.isna(eu27_data.loc[eu27_data["timescale"] == year, column]).any():
            ...
            df.loc[(df["geoscale"] == "EU27") & (df["timescale"] == year), column] = (
                aggregate_value
            )
...
df["waste-collected[t]"] = (
    ...
    + df["DSP-DMDP"]
    + df["DSP-W1910"]
)  # sum to 1, where 1 is waste-collected
...
df.update(
    df.drop(columns=exclude_columns).apply(pd.to_numeric, errors="coerce").clip(upper=1)
)
...
save_file_directory = os.path.join(
    current_file_directory, "../data/eol_intermediate_files/elv.xlsx"
)
