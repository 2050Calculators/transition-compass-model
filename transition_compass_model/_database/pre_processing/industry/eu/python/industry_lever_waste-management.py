...
dm_elv = linear_fitting(
    dm_elv.filter(
        {"Variables": ["landfill", "recycling", "energy-recovery", "export", "reuse"]}
    ),
    years_ots=years_missing,
    min_t0=0,
    min_tb=0,
    based_on=[2005],
)
...
dm_elv = make_fts(
    dm_elv, "landfill", 2010, baseyear_end
)  # we keep the decreasing trend since 2010
...
dm_elv.add(
    0, col_label="incineration", dummy=True, dim="Variables", unit="t"
)  # incineration = 0 as it's forbidden in EU to incinerate cars
dm_elv_col = dm_elv.filter(
    {"Variables": ["energy-recovery", "incineration", "landfill", "recycling", "reuse"]}
)
dm_elv_col.sort("Variables")
dm_elv_tot = dm_elv_col.groupby(
    group_cols={
        "collected": [
            "energy-recovery",
            "incineration",
...
for c in dm_elv_col.col_labels["Country"]:
    for y in years_all:
        dm_elv_col.array[idx[c], idx[y], :] = adjust_shares(
            dm_elv_col.array[idx[c], idx[y], :]
        )
...
dm_temp = dm_elv_tot.groupby(
    {"waste-uncollected": ["waste-collected", "export"]}, "Variables", inplace=False
)
...
dm_temp = dm_elv_tot.groupby(
    group_cols={
        "total": ["waste-collected", "export", "waste-uncollected", "littered"]
    },
    dim="Variables",
    aggregation="sum",
    regex=False,
...
for c in dm_elv_tot.col_labels["Country"]:
    for y in years_all:
        dm_elv_tot.array[idx[c], idx[y], :] = adjust_shares(
            dm_elv_tot.array[idx[c], idx[y], :]
        )
...
# store
DM_wst_mgt = {"elv-total": dm_elv_tot, "elv-col": dm_elv_col}
...
dm_rail_tot[:, :, :, "export"] = np.nanmean(
    dm_rail_tot[:, :, :, "export"]
)  # assuming similar export than vehicles
dm_rail_tot[:, :, :, "waste-collected"] = (
    1 - dm_rail_tot[:, :, :, "export"]
)  # assuming all rest is collected
...
# fill nas (including 2023)
years_missing = np.array(years_ots)[
    [y not in dm_bld.col_labels["Years"] for y in years_ots]
].tolist()
...
for c in dm_bld_col.col_labels["Country"]:
    for y in years_all:
        dm_bld_col.array[idx[c], idx[y], :] = adjust_shares(
            dm_bld_col.array[idx[c], idx[y], :]
        )
...
for c in dm_bld_tot.col_labels["Country"]:
    for y in years_all:
        dm_bld_tot.array[idx[c], idx[y], :] = adjust_shares(
            dm_bld_tot.array[idx[c], idx[y], :]
        )
...
dm_temp.rename_col(
    "floor-area-new-residential", "floor-area-new-non-residential", "Variables"
)
...
dm_road_tot = DM_wst_mgt["bld-total"].filter(
    {"Variables": ["floor-area-new-non-residential"]}
)
dm_road_tot.rename_col("floor-area-new-non-residential", "road", "Variables")
DM_wst_mgt["road-total"] = dm_road_tot

dm_road_col = DM_wst_mgt["bld-col"].filter(
    {"Variables": ["floor-area-new-non-residential"]}
)
...
del (
    ...
    years_ots,
    years_fts,
    years_after_regulation,
    years_all,
    years_before_regulation,
    years_setting,
)
...
# clean
del c, country_list, df, df_temp, dict_mapping, drops, indexes, key
...
# fill nas (including 2023)
years_missing = np.array(years_ots)[
    [y not in dm_trc.col_labels["Years"] for y in years_ots]
].tolist()
for y in years_missing:
...
for c in dm_trc_col.col_labels["Country"]:
    for y in years_all:
        dm_trc_col.array[idx[c], idx[y], :] = adjust_shares(
            dm_trc_col.array[idx[c], idx[y], :]
        )
...
for c in dm_trc_tot.col_labels["Country"]:
    for y in years_all:
        dm_trc_tot.array[idx[c], idx[y], :] = adjust_shares(
            dm_trc_tot.array[idx[c], idx[y], :]
        )
...
df = df.pivot(
    index=["geoscale", "waste", "year", "unit"], columns="variable", values="value"
).reset_index()

# fill in nans for Czech Republic, Italy and Lithuania.
df_temp = df.loc[df["geoscale"].isin(["Czech Republic", "Italy", "Lithuania"]), :]
df.loc[df["reuse"].isnull(), "reuse"] = (
    df.loc[df["reuse"].isnull(), "recovery"] - df.loc[df["reuse"].isnull(), "recycling"]
)
...
df.loc[df["recovery"] > df["waste-collected"], "waste-collected"] = df.loc[
    df["recovery"] > df["waste-collected"], "recovery"
]
...
df_mun["unit"] = "T"
df_mun = df_mun.pivot(
    index=["geoscale", "year", "unit"], columns="wst_oper", values="value"
).reset_index()
...
df["incineration"] = np.round(
    (df["waste-collected"] - df["recovery"]) * df["incineration-coeff"], 0
)
df["landfill"] = np.round(
    (df["waste-collected"] - df["recovery"]) * df["landfill-coeff"], 0
)
...
df = df.melt(
    id_vars=["geoscale", "waste", "year", "unit"],
    var_name="variable",
    value_name="value",
)
...
dm_domapp = linear_fitting(
    dm_domapp, years_fitting, min_t0=0, min_tb=0, based_on=[2018]
)
...
dm_temp = linear_fitting(
    dm_temp, years_ots=years_missing, min_t0=0, min_tb=0, based_on=[2007]
)
dm_domapp = linear_fitting(
    dm_domapp.filter(
        {"Categories1": ["energy-recovery", "export", "recycling", "reuse"]}
    ),
    years_ots=years_missing,
    min_t0=0,
    min_tb=0,
...
dm_domapp = make_fts(
    dm_domapp, "domapp_landfill", 2010, baseyear_end
)  # we keep the decreasing trend since 2010
dm_domapp = make_fts(
    dm_domapp, "domapp_incineration", 2010, baseyear_end
)  # we keep the decreasing trend since 2010
...
dm_domapp = make_fts(
    dm_domapp, "electronics_energy-recovery", baseyear_start, baseyear_end
)
dm_domapp = make_fts(dm_domapp, "electronics_export", baseyear_start, baseyear_end)
dm_domapp = make_fts(
    dm_domapp, "electronics_landfill", 2010, baseyear_end
)  # we keep the decreasing trend since 2010
dm_domapp = make_fts(
    dm_domapp, "electronics_incineration", 2010, baseyear_end
)  # we keep the decreasing trend since 2010
...
# make dm for collected waste
dm_domapp_col = dm_domapp.filter(
    {
        "Categories1": [
            "energy-recovery",
            "incineration",
...
dm_domapp_tot = dm_domapp_col.groupby(
    group_cols={
        "collected": [
            "energy-recovery",
            "incineration",
...
for c in dm_domapp_col.col_labels["Country"]:
    for y in years_all:
        for v in dm_domapp_col.col_labels["Variables"]:
            dm_domapp_col.array[idx[c], idx[y], idx[v], :] = adjust_shares(
                dm_domapp_col.array[idx[c], idx[y], idx[v], :]
            )
for v in dm_domapp_col.col_labels["Variables"]:
...
for c in dm_domapp_tot.col_labels["Country"]:
    for y in years_all:
        for v in dm_domapp_tot.col_labels["Variables"]:
            dm_domapp_tot.array[idx[c], idx[y], idx[v], :] = adjust_shares(
                dm_domapp_tot.array[idx[c], idx[y], idx[v], :]
            )
for v in dm_domapp_tot.col_labels["Variables"]:
...
df.loc[df["geoscale"] == "EU27", "export"] = df.loc[
    df["geoscale"] == "EU27", "recycling-neu"
]
...
df_temp["generated"] = (
    df_temp["energy recovery"] + df_temp["recycling"] + df_temp["reuse"]
)
df = pd.concat([df, df_temp])
...
df = pd.merge(df, df_mun, how="left", on=["geoscale", "year", "unit"])
df["incineration"] = np.round(
    (df["generated"] - df["recovery"]) * df["incineration-coeff"], 0
)
...
df_temp["generated"]
(
    df_temp["energy recovery"]
    + df_temp["recycling"]
    + df_temp["incineration"]
    + df_temp["landfill"]
    + df_temp["reuse"]
)
...
# fix
index = (
    (df["geoscale"] == "Finland") & (df["year"] == "2021") & (df["waste"] == "W150102")
)
df.loc[index, "generated"] = (
    df.loc[index, "energy recovery"]
    + df.loc[index, "recycling"]
    + df.loc[index, "incineration"]
    + df.loc[index, "landfill"]
    + df.loc[index, "reuse"]
)
...
dm_pack = linear_fitting(
    dm_pack, years_ots=years_missing, min_t0=0, min_tb=0, based_on=[1997]
)
...
dm_pack = make_fts(
    dm_pack, "aluminium-pack_energy-recovery", baseyear_start, baseyear_end
)
...
dm_pack = make_fts(
    dm_pack, "plastic-pack_energy-recovery", baseyear_start, baseyear_end
)
...
dm_pack_tot = dm_pack_col.groupby(
    group_cols={
        "collected": [
            "energy-recovery",
            "incineration",
...
for c in dm_pack_col.col_labels["Country"]:
    for y in years_all:
        for v in dm_pack_col.col_labels["Variables"]:
            dm_pack_col.array[idx[c], idx[y], idx[v], :] = adjust_shares(
                dm_pack_col.array[idx[c], idx[y], idx[v], :]
            )
for v in dm_pack_col.col_labels["Variables"]:
    dm_pack_col.units[v] = "%"
...
arr_temp = (
    dm_pack_tot[..., "waste-collected"] * perc_uncollected
)  # this should also include littered in the case of packaging
...
for c in dm_pack_tot.col_labels["Country"]:
    for y in years_all:
        for v in dm_pack_tot.col_labels["Variables"]:
            dm_pack_tot.array[idx[c], idx[y], idx[v], :] = adjust_shares(
                dm_pack_tot.array[idx[c], idx[y], idx[v], :]
            )
for v in dm_pack_tot.col_labels["Variables"]:
    dm_pack_tot.units[v] = "%"
...
dm_level4_tot = dm_level4_veh.filter(
    {"Categories1": ["export", "littered", "waste-collected", "waste-uncollected"]}
)
...
dm_level4_tot.array[idx["EU27"], idx[2050], :, idx["export"]] = np.round(
    export / tot, 2
)
dm_level4_tot.array[idx["EU27"], idx[2050], :, idx["waste-collected"]] = np.round(
    collected / tot, 2
)
...
# for layer 2
dm_level4_col = dm_level4_veh.filter(
    {
        "Categories1": [
            "energy-recovery",
            "landfill",
...
for v in variabs:
    dm_level2.array[idx["EU27"], idx[2050], idx["vehicles"], idx[v]] = df_temp.loc[
        df_temp["level"] == 2, v
    ]
...
for v in variabs:
    dm_level3.array[idx["EU27"], idx[2050], idx["vehicles"], idx[v]] = df_temp.loc[
        df_temp["level"] == 3, v
    ]
