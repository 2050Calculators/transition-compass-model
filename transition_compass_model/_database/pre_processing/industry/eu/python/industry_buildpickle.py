...
lever_files = [
    "lever_material-switch.pickle",
    "lever_material-efficiency.pickle",
    "lever_technology-development.pickle",
    "lever_carbon-capture.pickle",
    "lever_technology-share.pickle",
    "lever_material-net-import.pickle",
    "lever_product-net-import.pickle",
    "lever_energy-switch.pickle",
    "lever_waste-management.pickle",
    "lever_material-recovery.pickle",
...
lever_names = ["technology-development", "cc", "energy-carrier-mix"]
for n in lever_names:
...
filepath = os.path.join(
    ...
    "../data/datamatrix/" + "const_material-switch-ratios.pickle",
)
with open(filepath, "rb") as handle:
...
CDM_const["material-decomposition_domapp"] = CDM["bld_domapp"].filter(
    {"Categories1": ["dishwasher", "dryer", "freezer", "fridge", "wmachine"]}
)
CDM_const["material-decomposition_electronics"] = CDM["bld_domapp"].filter(
...
CDM_const["energy_excl-feedstock_eleclight-split"] = CDM[
    "energy-demand-excl-feedstock-eleclight-split"
].copy()
CDM_const["energy_efficiency"] = CDM["energy-efficiency"].copy()
CDM_const_amm["energy_excl-feedstock_eleclight-split"] = CDM[
    "energy-demand-excl-feedstock-eleclight-split"
].copy()
...
CDM_const_amm["material-decomposition_veh"] = CDM_const[
    "material-decomposition_veh"
].filter({"Categories3": ["ammonia"]})
CDM_const["material-decomposition_veh"].drop("Categories3", "ammonia")
CDM_const_amm["material-decomposition_bat"] = CDM_const[
    "material-decomposition_bat"
].filter({"Categories3": ["ammonia"]})
CDM_const["material-decomposition_bat"].drop("Categories3", "ammonia")
lever_names = ["energy_excl-feedstock", "energy_feedstock", "emission-factor-process"]
for n in lever_names:
