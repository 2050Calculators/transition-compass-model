...
def get_data(country):
    ...
    for i in range(0, len(enercarr_jrc)):
        df.loc[df["energy_carrier"] == enercarr_jrc[i], "energy_carrier"] = (
            enercarr_calc_map[i]
        )
    df = df.groupby(["energy_carrier", "year", "unit"], as_index=False)["value"].agg(
        sum
    )
    ...
    df["Country"] = country
    df["variable"] = [
        "energy-demand_" + carrier + "[TWh]" for carrier in df["energy_carrier"]
    ]
...
# save
f = os.path.join(
    current_file_directory, "../data/datamatrix/calibration_energy-demand.pickle"
)
