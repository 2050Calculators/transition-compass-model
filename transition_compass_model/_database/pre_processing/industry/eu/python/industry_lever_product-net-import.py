...
df_sub["selection"] = [
    re.split("product-demand_|product-export_|product-import_", re.split("\\[", i)[0])[
        1
    ]
...
# dm_fert
df_temp = df_sub.loc[
    df_sub["selection"].isin(["fertilizer"]), ["Country", "Years", "variable", "value"]
]
df_temp = df_temp.pivot(
    index=["Country", "Years"], columns="variable", values="value"
).reset_index()
...
dm_trade.array[:, idx[1995], idx["product-import_HDV_PHEV-diesel"]] = dm_trade.array[
    :, idx[2022], idx["product-import_HDV_PHEV-diesel"]
] / (2022 - 1995)
for y in range(1996, 2021 + 1):
    dm_trade.array[:, idx[y], idx["product-import_HDV_PHEV-diesel"]] = dm_trade.array[
        :, idx[1995], idx["product-import_HDV_PHEV-diesel"]
    ]
