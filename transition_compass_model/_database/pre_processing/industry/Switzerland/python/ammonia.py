...
f = os.path.join(
    current_file_directory, "../data/datamatrix/lever_product-net-import_ammonia.pickle"
)
...
f = os.path.join(
    ...
    "../data/datamatrix/lever_material-net-import_ammonia.pickle",
)
with open(f, "wb") as handle:
    pickle.dump(DM, handle, protocol=pickle.HIGHEST_PROTOCOL)
...
missing = np.array(years)[
    [y not in dm_amm_prod_cal.col_labels["Years"] for y in years]
].tolist()
...
filepath = os.path.join(
    current_file_directory, "../data/datamatrix/calibration_emissions.pickle"
)
...
dm_amm.array[...] = np.nan
f = os.path.join(
    current_file_directory, "../data/datamatrix/calibration_emissions_ammonia.pickle"
)
import pickle
