...
def get_import_export_chf(trade_flow, df_map_cpa):
    ...
    filepath = os.path.join(
        current_file_directory, f"../data/import-export/{file}.xlsx"
    )
    ...
    df = df.merge(
        df_map_cpa.loc[:, ["code", "variable", "variable-calc"]],
        "left",
        ["code", "variable"],
    )
    ...
    df = df.groupby(["variable-calc", "year", "trade-flow"], as_index=False)[
        "value"
    ].agg(sum)
...
filepath_map = os.path.join(
    current_file_directory, "../data/products_materials_mapping/CPA_calc_mapping.xlsx"
)
...
def get_price_data(flow):
    ...
    filepath_map = os.path.join(
        ...
        "../data/products_materials_mapping/NST_calc_mapping.xlsx",
    )
    df_map = pd.read_excel(filepath_map)
...
def get_io_data(year):
    ...
    filepath = os.path.join(
        current_file_directory, f"../data/input-output/su-e-04.03-IOT-{year}.xlsx"
    )
    ...
    df.iloc[0, :] = np.array(
        [x.strip() if isinstance(x, str) else x for x in arr], dtype=object
    )
    ...
    df["code"] = np.array(
        [x.rstrip() if isinstance(x, str) else x for x in arr], dtype=object
    )
...
arr_temp = (dm_netimp[:, :, "imports", :] - dm_netimp[:, :, "exports", :]) / dm_netimp[
    :, :, "demand", :
]
dm_netimp.add(arr_temp, "Variables", "net-import", "%")
arr_temp = (
    dm_netimp[:, :, "imports-alt", :] - dm_netimp[:, :, "exports-alt", :]
) / dm_netimp[:, :, "demand-alt", :]
...
dm_wwp_demand = dm_netimp.filter(
    {"Variables": ["demand-alt"], "Categories1": ["wwp"]}
)  # I need this for fxa
...
missing = np.array(years)[
    [y not in dm_out_calib.col_labels["Years"] for y in years]
].tolist()
...
def linear_fitting_per_variab(dm, variable, year_start, year_end, years_fts):
    ...
    dm_temp = linear_fitting(
        dm_temp, years_fts, based_on=list(range(year_start, year_end + 1))
    )
...
del (
    ...
    key,
    my_aggregates,
    plastic_packaging_consumption_2023,
    v,
    y,
    years_fts,
    years_ots,
