...
def extract_public_passenger_pkm(df, years_ots):
    ...
    df_T = pd.melt(
        df_pkm, id_vars=["Variables", "Country"], var_name="Years", value_name="values"
    )
    df_pivot = df_T.pivot_table(
        index=["Country", "Years"],
        columns=["Variables"],
...
def extract_public_passenger_vkm(df, years_ots):
    ...
    df_T = pd.melt(
        df_vkm, id_vars=["Variables", "Country"], var_name="Years", value_name="values"
    )
    df_pivot = df_T.pivot_table(
        index=["Country", "Years"],
        columns=["Variables"],
    ...
    dm.change_unit(
        "tra_passenger_transport-demand-vkm", 1000, old_unit="1000vkm", new_unit="vkm"
    )
