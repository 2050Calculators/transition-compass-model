...
def get_pkm_cap_aviation(file_url, local_filename):
    ...
    unit = "pkm/cap"
    header_row = 2
    dm_pkm = utils.df_fso_excel_to_dm(
        df, header_row, names_map, var_name, unit, num_cat=1
    )
    return dm_pkm
...
def get_world_pop(pop_url, local_filename):
    ...
    unit = "number"
    header_row = 2
    dm_pop = utils.df_fso_excel_to_dm(
        df_pop,
        header_row,
        names_map,
        var_name,
...
def get_aviation_fleet(file_url, local_filename):
    ...
    unit = "number"
    header_row = 2
    dm_fleet_aviation = utils.df_fso_excel_to_dm(
