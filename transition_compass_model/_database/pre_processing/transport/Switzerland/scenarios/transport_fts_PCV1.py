...
def define_variables_for_lever(DM_transport, lever: int):
    dm_modal_share_lever = DM_transport["fts"]["passenger_modal-share"][lever].copy()

    idx = dm_modal_share_lever.idx
    dm_modal_share_lever.array[idx["Vaud"], idx[2025] + 1 :, :, :] = (
        np.nan
    )  # clear level except for 2025
    dm_modal_share_lever_tmp = None

    return dm_modal_share_lever, idx, dm_modal_share_lever_tmp


def update_lever_in_loop(
    key, cat, dm_modal_ots_cat, idx_ots, dict_ratio, array_lever, year=2050
):
    dm_modal_share_lever, idx, dm_modal_share_lever_tmp = array_lever
    ...
    if dm_modal_share_lever_tmp is None:
        dm_modal_share_lever_tmp = dm_modal_fts_cat_lever.copy()
    else:
        dm_modal_share_lever_tmp.append(dm_modal_fts_cat_lever, dim="Categories1")
...
def run(DM_transport, country_list, years_ots, years_fts):
    ...
    # Scénario 2, 3 and 4 respectively: PCV1, PCV2,  DLS
    ...
    param_3 = define_variables_for_lever(DM_transport, 3)
    ...
    dm_modal_share_3, idx_fts, dm_modal_share_3_tmp = param_3

    dm_modal_share_3_tmp.sort("Categories1")
    dm_modal_share_3 = dm_modal_share_3_tmp.copy()

    # Get respective share of public transport and TIM in 2023 to keep the same proportion in 2030 and 2050
    ...
    dm_modal_share_3.normalise_non_fixed_values(["bike"], idx_fts[2035])
    ...
    dm_new_tech_share_2.array[
        idx["Vaud"], :, idx["tra_passenger_technology-share_new"], :
    ] = np.nan
    prop_tech_EV_2023, prop_diesel_ICE_2023, prop_gasoline_ICE_2023 = (
        wkf.compute_prop_tech_EV_2023(dm_new_tech_share_ots)
    )
    # On trouve les proportions de motorisation pour 2035.
    prop_BEV_2035_PCV = prop_EV_PHEV_2035_PCV * prop_tech_EV_2023["BEV"]
    prop_PHEV_diesel_2035_PCV = prop_EV_PHEV_2035_PCV * prop_tech_EV_2023["PHEV-diesel"]
    prop_PHEV_gasoline_2035_PCV = (
        prop_EV_PHEV_2035_PCV * prop_tech_EV_2023["PHEV-gasoline"]
    )
    ...
    values_2035_new_tech_share = {
        "BEV": prop_BEV_2035_PCV,
        "ICE-diesel": prop_diesel_ICE_2035_PCV,
        "ICE-gasoline": prop_gasoline_ICE_2035_PCV,
        "PHEV-diesel": prop_PHEV_diesel_2035_PCV,
        "PHEV-gasoline": prop_PHEV_gasoline_2035_PCV,
    }
    ...
    linear_fitting(
        dm_new_tech_share_trend_PCV, dm_new_tech_share_trend_PCV.col_labels["Years"]
    )
    idx = dm_new_tech_share_trend_PCV.idx
    dm_new_tech_share_trend_PCV.array[
        idx["Vaud"], idx[2035] + 1 :, idx["tra_passenger_technology-share_new"], :
    ] = np.nan
    linear_fitting(
        dm_new_tech_share_trend_PCV,
        dm_new_tech_share_trend_PCV.col_labels["Years"],
        based_on=dm_new_tech_share_2.col_labels["Years"],
        min_tb=0,
    )
    ...
    DM_fts["fts"]["pkm"] = {1: dm_pkm_1, 2: dm_pkm_2, 3: dm_pkm_3, 4: dm_pkm_3}
    ...
    efficiency_2018 = dm_new_eff_ots.array[
        idx["Vaud"], idx[2018], idx["tra_passenger_veh-efficiency_new"], idx["LDV"], :
    ]
    df_mesure_5["efficiency 2018"] = efficiency_2018

    idx = dm_new_tech_share_ots.idx
    market_shares_2018 = dm_new_tech_share_ots.array[
        idx["Vaud"], idx[2018], idx["tra_passenger_technology-share_new"], idx["LDV"], :
    ]
    df_mesure_5["part de marché 2018"] = market_shares_2018

    idx = dm_new_eff_2.idx
    efficiency_2035 = dm_new_eff_2.array[
        idx["Vaud"], idx[2035], idx["tra_passenger_veh-efficiency_new"], idx["LDV"], :
    ]
    df_mesure_5["efficiency 2035"] = efficiency_2035

    idx = dm_new_tech_share_2.idx
    market_shares_2035 = dm_new_tech_share_2.array[
        idx["Vaud"], idx[2035], idx["tra_passenger_technology-share_new"], idx["LDV"], :
    ]
    ...
    efficiency_2018 = (
        df_mesure_5["efficiency 2018"] * df_mesure_5["part de marché 2018"]
    ).sum()
    efficiency_2035 = (
        df_mesure_5["efficiency 2035"] * df_mesure_5["part de marché 2035"]
    ).sum()
    ...
    df_mesure_5.loc["ICE-diesel", "facteurs_CO2_2035"] *= dm_carbon_intensity.array[
        0, idx["CO2"], idx["ICE-diesel"]
    ]
    df_mesure_5.loc["ICE-diesel", "facteurs_CO2_2018"] *= dm_carbon_intensity.array[
        0, idx["CO2"], idx["ICE-diesel"]
    ]
    df_mesure_5.loc["PHEV-diesel", "facteurs_CO2_2035"] *= dm_carbon_intensity.array[
        0, idx["CO2"], idx["PHEV-diesel"]
    ]
    df_mesure_5.loc["PHEV-diesel", "facteurs_CO2_2018"] *= dm_carbon_intensity.array[
        0, idx["CO2"], idx["PHEV-diesel"]
    ]
    df_mesure_5.loc["ICE-gasoline", "facteurs_CO2_2035"] *= dm_carbon_intensity.array[
        0, idx["CO2"], idx["ICE-gasoline"]
    ]
    df_mesure_5.loc["ICE-gasoline", "facteurs_CO2_2018"] *= dm_carbon_intensity.array[
        0, idx["CO2"], idx["ICE-gasoline"]
    ]
    df_mesure_5.loc["PHEV-gasoline", "facteurs_CO2_2035"] *= dm_carbon_intensity.array[
        0, idx["CO2"], idx["PHEV-gasoline"]
    ]
    df_mesure_5.loc["PHEV-gasoline", "facteurs_CO2_2018"] *= dm_carbon_intensity.array[
        0, idx["CO2"], idx["PHEV-gasoline"]
    ]

    dm_elec_intensity = DM_transport["fxa"]["emission-factor-electricity"]
    idx = dm_elec_intensity.idx
    electricite_2035_intensity = dm_elec_intensity.array[
        idx["Vaud"], idx[2035], 0, idx["CO2"], 0
    ]
    df_mesure_5.loc["BEV", "facteurs_CO2_2035"] *= dm_elec_intensity.array[
        idx["Vaud"], idx[2035], 0, idx["CO2"], 0
    ]  # dm.array[0, idx['CO2'], idx['PHEV-gasoline']]
    df_mesure_5.loc["BEV", "facteurs_CO2_2018"] *= dm_elec_intensity.array[
        idx["Vaud"], idx[2035], 0, idx["CO2"], 0
    ]

    pd.set_option("display.max_columns", 10)

    emissions_moyennes_2018 = (
        df_mesure_5["facteurs_CO2_2018"] * df_mesure_5["part de marché 2018"]
    ).sum()
    emissions_moyennes_2035 = (
        df_mesure_5["facteurs_CO2_2035"] * df_mesure_5["part de marché 2035"]
    ).sum()
    ...
    DM_transport["fts"].update(DM_fts["fts"])
