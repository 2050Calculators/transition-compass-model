...
def run(years_ots):
    ...
    dm_energy_private = get_data.extract_EP2050_transport_energy_demand(
    ...
    dm_energy_rail = get_data.extract_EP2050_transport_energy_demand_rail(
        file_url, zip_name, file_pickle
    )
    ...
    dm_passenger_energy_all = dm_energy_private.filter(
        {"Categories1": ["bus", "2W", "LDV"]}
    )

    # Append rail to private transport
    all_cat = list(
        set(dm_passenger_energy_all.col_labels["Categories2"]).union(
            set(dm_passenger_rail.col_labels["Categories2"])
        )
    )
    dm_add_missing_variables(
        dm_passenger_energy_all, {"Categories2": all_cat}, fill_nans=False
    )
    dm_add_missing_variables(
        dm_passenger_rail, {"Categories2": all_cat}, fill_nans=False
    )
