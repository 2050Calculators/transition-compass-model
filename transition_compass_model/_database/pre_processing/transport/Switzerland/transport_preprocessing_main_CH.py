...
mode_cat = list(
    set(dm_private_fleet.col_labels["Categories1"]).union(
        set(dm_public_fleet.col_labels["Categories1"])
    )
)
tech_cat = list(
    set(dm_private_fleet.col_labels["Categories2"]).union(
        set(dm_public_fleet.col_labels["Categories2"])
    )
)
dm_lifetime = passenger_lifetime_run(
    mode_cat, tech_cat, years_ots, years_fts, country_list
)
