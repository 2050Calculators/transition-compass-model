...
def merge_dm(
    dm_A,
    dm_B,
    same_dim=["Country", "Years", "Variables"],
    union_dim=["Categories2"],
    along_dim="Categories1",
):
...
def pkm_monde_fts(value_2024, pkm_rates):
    ...
    for rate in pkm_rates:
        ...
        DM[key] = dm_tmp.filter(
            {"Years": years_fts}
        )  # num cat il lit pkm_aviation tout ce qui est avant _ ca devient variable et tout apres cest la catégorie
    return DM
...
def adj_pkm_monde_ots(dm_pkmsuisse_ots, dm_pkm_ots):
    ...
    dm_pkm_ots.operation(
        "tra_pkm-cap", "/", "tra_pkm-suisse-cap", out_col="ratio", unit="%"
    )
    linear_fitting(dm_pkm_ots, years_ots, based_on=create_years_list(2006, 2017, 1))
    dm_pkm_ots[:, :, "ratio", ...] = np.maximum(1.05, dm_pkm_ots[:, :, "ratio", ...])
...
def pkm_suisse_fts(pkm_rates, dm_pkmsuisse_ots, years_ots, years_fts):
    ...
    for rate in pkm_rates:
        ...
        key = i
        i = i + 1
        dm_pkm_tmp = DataMatrix.create_from_df(df_pkm, num_cat=1).filter(
            {"Years": years_fts}
        )
...
def occupancy_monde_ots(
    dm_pkm_suisse, dm_pkm_tot, dm_occupancy_ots, dm_occupancyworld_ots
):
    # occupancy = (pkm_CH * occupancy_CH + (pkm_tot - pkm_CH) * occupancy_world) / pkm_tot
    array_occupancyponderee = (
        dm_pkm_suisse[...] * dm_occupancy_ots[...]
        + (dm_pkm_tot[...] - dm_pkm_suisse[...]) * dm_occupancyworld_ots[...]
    ) / (dm_pkm_tot[...])
    dm_occupancyworld_ots.add(
        array_occupancyponderee,
        dim="Variables",
        col_label="tra_passenger_occupancy_ponderee",
        unit="%",
    )
    dm_occupancyworld_ots.drop(dim="Variables", col_label=["tra_passenger_occupancy"])
    dm_occupancyworld_ots.rename_col(
    ...
    dm_occupancyworld_ots.change_unit(
...
def compute_seats(dm_pkm, dm_utilirate, dm_occupancy, dm_pop, subfix):
    ...
    dm_occupancy.add(
        arr_seat, dim="Variables", col_label="tra_passenger_seats" + subfix, unit="seat"
    )
    dm_occupancy.add(
        arr_skm,
        dim="Variables",
        col_label="tra_passenger-demand-skm" + subfix,
        unit="skm",
    )
...
def compute_tech_new_from_newfleet(dm_fleet_ofts, dm_max):
    ...
    for var in dm_new_veh_demand.col_labels["Variables"]:
        dm_new_veh_demand.change_unit(var, factor=1, old_unit="number", new_unit="seat")
    ...
    years_fts_all = create_years_list(2025, 2050, 1)
    missing_years = list(set(years_fts_all) - set(years_fts))
    dm_new_veh_demand.add(np.nan, dim="Years", dummy=True, col_label=missing_years)
    ...
    for key in DM_orig.keys():
        if isinstance(DM_orig[key], dict):
            ...
            if "Categories1" in dm_new.dim_labels:
                if (
                    "aviation" in dm_new.col_labels["Categories1"]
                    and "freight" not in str(key)
                    and key != "passenger_aviation-pkm"
                ):
...
dm_pop = DM_lifestyles["ots"]["pop"]["lfs_population_"].filter(
    {"Country": ["Switzerland"]}
)
...
# ICE, BEV, H2
df_passengereff_ots = pd.read_excel(
    "data/aviation_energy_intensity_fleet.xlsx", sheet_name="Feuil1"
)
...
dm_occupancy_monde_ots = occupancy_monde_ots(
    dm_pkm_suisse, dm_pkm_tot, dm_occupancy_ots, dm_occupancyworld_ots
)
...
dm_occupancyworld_ots = DM_transport["ots"]["passenger_occupancy"].filter(
...
df_technoshare_new = pd.read_excel(
    "data/aviation_passenger_technology_share_new_ots.xlsx"
)
...
# !FIXME THIS IS WRONG
linear_fitting(
    dm_utilirate_ots, years_ots=years_ots, based_on=create_years_list(2005, 2019, 1)
)
...
dm_utilirate_ots[0, 2022, "tra_passenger_utilisation-rate", 0] = dm_utilirate_ots[
    0, 2022, "tra_passenger_utilisation-rate_new", 0
]
dm_utilirate_ots[0, 2023, "tra_passenger_utilisation-rate", 0] = dm_utilirate_ots[
...
dm_fleet_eff = DM_transport["ots"]["passenger_veh-efficiency_new"].filter(
    {"Categories1": ["aviation"]}
)
...
dm_fleetshare = DM_transport["ots"]["passenger_technology-share_new"].filter(
    {"Categories1": ["aviation"]}
)
...
df_new_vehicules_max = pd.read_excel(
    "data/aviation_number_seats_max.xlsx", sheet_name="1"
)
dm_new_vehicules_max = DataMatrix.create_from_df(df_new_vehicules_max, num_cat=2)
years_ots = create_years_list(1990, 2023, 1)
dm_new_vehicules_max.rename_col(
    "tra_new-vehicules-max", "tra_vehicles-max", dim="Variables"
)
dm_new_vehicules_max.change_unit(
...
dm_seats_ots, dm_skm_ots = compute_seats(
    dm_pkm_tmp, dm_utilirate, dm_occupancy_ots, dm_pop, subfix=""
)

# SEATS ABROAD - FTS
dm_pkm_tmp = DM_transport["fts"]["passenger_aviation-pkm"][1].copy()
dm_utilirate = DM_transport["fts"]["passenger_utilization-rate"][1].filter(
    {"Categories1": ["aviation"]}
)
dm_occupancy = DM_transport["fts"]["passenger_occupancy"][1].filter(
    {"Categories1": ["aviation"]}
)
dm_seats_fts, dm_skm_fts = compute_seats(
    dm_pkm_tmp, dm_utilirate, dm_occupancy, dm_pop_fts, subfix=""
)
...
dm_utilirate = dm_utilirate_ots
dm_occupancy_ots = DM_ots["ots"]["passenger_occupancy-suisse"].copy()
dm_seats_ots_CH, dm_skm_ots_CH = compute_seats(
    dm_pkm_tmp, dm_utilirate, dm_occupancy_ots, dm_pop, subfix="_CH"
)
...
dm_seats_fts_CH, dm_skm_fts_CH = compute_seats(
    dm_pkm_tmp, dm_utilirate, dm_occupancy, dm_pop_fts, subfix="_CH"
)
del dm_pkm_tmp, dm_utilirate, dm_occupancy_ots
...
dm_new.rename_col(
    "tra_passenger_new-vehicles", "tra_passenger_new-vehicles_kerosene", dim="Variables"
)
...
arr_util_rate = (
    dm_pkm_tmp.array
    * dm_pop.array[..., np.newaxis]
    / (dm_occupancy_ots.array * dm_seats.array)
)
...
dm_pass_tech_aviation.append(
    dm_fleetshare.filter({"Categories2": tech_aviation}), dim="Variables"
)
dm_pass_tech_aviation.append(
    dm_fleet_eff.filter({"Categories2": tech_aviation}), dim="Variables"
)
...
dm_SAF.add(
    np.nan, dim="Variables", col_label="tra_passenger-max-SAF", unit="MJ", dummy=True
)
...
dm_tmp.rename_col(
    "tra_passenger-max-SAF", "tra_passenger_available-fuel-mix_biofuel", dim="Variables"
)
...
dm_new_eff = DM_transport["fts"]["passenger_veh-efficiency_new"][4].filter(
...
dm_max_seat_ICE = dm_max.filter(
    {"Categories2": ["kerosene"], "Variables": ["tra_vehicles-max"], "Years": years_fts}
)
...
for lev in range(4):
    ...
    dm_new = DM[lev + 1]
    dm_fuel_mix_orig["Switzerland", :, :, :, "aviation"] = dm_new[
...
all_countries = set(
    DM_transport_orig["ots"]["passenger_aviation-pkm"].col_labels["Country"]
) - {"Switzerland"}
...
DM_transport_orig["ots"]["passenger_aviation-pkm"]["Switzerland", ...] = DM_transport[
    "ots"
]["passenger_aviation-pkm"]["Switzerland", ...]
DM_transport_orig["ots"]["passenger_aviation-pkm"]["Vaud", ...] = DM_transport["ots"][
...
DM_transport_orig["fxa"]["share-local-emissions"] = DM_transport["fxa"][
    "share-local-emissions"
]
DM_transport_orig["fxa"]["fuel-mix-availability"] = DM_transport["fxa"][
