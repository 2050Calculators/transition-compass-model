...
dm_pkm_pc.rename_col(
    "tra_passenger_modal-share_share", "tra_passenger_modal-share", "Variables"
)
...
dm_pkm_pc_level4.array[idx["EU27"], idx[2050], :, idx["bus"]] = (
    0.08  # i put 8% for buses and 2% for metro tram
)
dm_pkm_pc_level4.array[idx["EU27"], idx[2050], :, idx["metrotram"]] = 0.02
dm_pkm_pc_level4.array[idx["EU27"], idx[2050], :, idx["rail"]] = (
    0.19  # i put 19% so I can put 1% for 2W
)
...
for v in variabs:
    dm_pkm_pc_level2.array[idx["EU27"], idx[2050], :, idx[v]] = df_temp.loc[
        df_temp["level"] == 2, v
    ]
...
for v in variabs:
    dm_pkm_pc_level3.array[idx["EU27"], idx[2050], :, idx[v]] = df_temp.loc[
        df_temp["level"] == 3, v
    ]
...
DM_mod = {
    "ots": {"passenger_modal-share": []},
    "fts": {"passenger_modal-share": dict()},
}
...
# save
f = os.path.join(
    current_file_directory, "../data/datamatrix/lever_passenger_modal-share.pickle"
)
