...
categories2_all = DM_tra["ots"]["freight_technology-share_new"].col_labels[
    "Categories2"
]
...
df_avi = pd.concat(
    [
        get_specific_jrc_data(code, name, 106, 107, "unit")
...
dm_temp = linear_fitting(
    dm_new.filter({"Country": ["EU27"], "Variables": ["HDVH_BEV"]}),
    years_ots + years_fts,
)
idx_temp = dm_temp.idx
dm_new.array[idx["EU27"], :, idx["HDVH_BEV"]] = dm_temp.array[
...
dm_temp = linear_fitting(
    dm_new.filter({"Country": ["EU27"], "Variables": ["HDVL_BEV"]}),
    years_ots + years_fts,
)
idx_temp = dm_temp.idx
dm_new.array[idx["EU27"], :, idx["HDVL_BEV"]] = dm_temp.array[
...
dm_temp = linear_fitting(
    dm_new.filter({"Country": ["EU27"], "Variables": ["HDVM_BEV"]}),
    years_ots + years_fts,
)
idx_temp = dm_temp.idx
dm_new.array[idx["EU27"], :, idx["HDVM_BEV"]] = dm_temp.array[
    idx_temp["EU27"], :, idx_temp["HDVM_BEV"]
]

# rest
dm_new = make_fts(
    dm_new, "HDVH_ICE-diesel", baseyear_start, baseyear_end, dim="Variables"
)
dm_new = make_fts(dm_new, "HDVH_ICE-gas", baseyear_start, baseyear_end, dim="Variables")
dm_new = make_fts(
    dm_new, "HDVH_ICE-gasoline", baseyear_start, baseyear_end, dim="Variables"
)
dm_new = make_fts(
    dm_new, "HDVL_ICE-diesel", baseyear_start, baseyear_end, dim="Variables"
)
dm_new = make_fts(dm_new, "HDVL_ICE-gas", baseyear_start, baseyear_end, dim="Variables")
dm_new = make_fts(
    dm_new, "HDVL_ICE-gasoline", baseyear_start, baseyear_end, dim="Variables"
)
dm_new = make_fts(
    dm_new, "HDVM_ICE-diesel", baseyear_start, baseyear_end, dim="Variables"
)
dm_new = make_fts(dm_new, "HDVM_ICE-gas", baseyear_start, baseyear_end, dim="Variables")
dm_new = make_fts(
    dm_new, "HDVM_ICE-gasoline", baseyear_start, baseyear_end, dim="Variables"
)
dm_new = make_fts(
    dm_new, "aviation_kerosene", baseyear_start, baseyear_end, dim="Variables"
)
...
DM_new = {
    "ots": {"freight_technology-share_new": []},
    "fts": {"freight_technology-share_new": dict()},
}
DM_new["ots"]["freight_technology-share_new"] = dm_new_pc_final.filter(
    {"Years": years_ots}
)
DM_new["ots"]["freight_technology-share_new"].drop("Years", startyear)
for i in range(1, 4 + 1):
    DM_new["fts"]["freight_technology-share_new"][i] = dm_new_pc_final.filter(
        {"Years": years_fts}
    )
...
f = os.path.join(
    current_file_directory,
    "../data/datamatrix/lever_freight_technology-share_new.pickle",
)
