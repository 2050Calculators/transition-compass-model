...
dict_extract = {
    ...
    "calc_names": [
        "seats_domestic",
        "seats_international-int",
        "seats_international-extra",
    ],
}
...
dm_avi.append(
    dm_avi.groupby({"numerator": ["domestic", "international-int"]}, "Categories1"),
    "Categories1",
)
dm_avi.append(
    dm_avi.groupby(
        {"denominator": ["domestic", "international-int", "international-extra"]},
        "Categories1",
    ),
    "Categories1",
)
dm_avi.operation(
    "numerator", "/", "denominator", "Categories1", "tra_share-emissions-local", "%"
)
dm_avi = dm_avi.filter({"Categories1": ["tra_share-emissions-local"]})
dm_avi = dm_avi.flatten()
dm_avi.rename_col(
    "skm_tra_share-emissions-local", "tra_share-emissions-local_aviation", "Variables"
)
dm_avi.deepen()
...
# save
f = os.path.join(
    current_file_directory, "../data/datamatrix/fxa_share-local-emissions.pickle"
)
