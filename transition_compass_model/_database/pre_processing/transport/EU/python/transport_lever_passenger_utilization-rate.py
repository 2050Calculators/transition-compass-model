...
dm_uti_level4.array[idx["EU27"], idx[2050], :, idx["bus"]] = dm_uti_level4.array[
    idx["EU27"], idx[2023], :, idx["bus"]
] * (1 + 0.45)
...
dm_uti_level4.array[idx["EU27"], idx[2050], :, idx["metrotram"]] = dm_uti_level4.array[
...
for v in variabs:
    dm_uti_level2.array[idx["EU27"], idx[2050], :, idx[v]] = df_temp.loc[
        df_temp["level"] == 2, v
    ]
...
for v in variabs:
    dm_uti_level3.array[idx["EU27"], idx[2050], :, idx[v]] = df_temp.loc[
        df_temp["level"] == 3, v
    ]
...
DM_uti = {
    "ots": {"passenger_utilization-rate": []},
    "fts": {"passenger_utilization-rate": dict()},
}
...
# save
f = os.path.join(
    current_file_directory, "../data/datamatrix/lever_passenger_utilization-rate.pickle"
)
