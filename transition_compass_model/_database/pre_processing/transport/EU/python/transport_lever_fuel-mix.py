...
dict_extract = {
    ...
    "sub_variables": ["by fuel (EUROSTAT DATA)", "Renewable energies and wastes"],
...
# make the share
dm_road.operation(
    "biofuel", "/", "total", "Variables", out_col="biofuel-share", unit="%"
)
...
dict_extract = {
    ...
    "sub_variables": ["Inland waterways", "Blended biofuels", "Biogases"],
...
# make the share
dm_iww.groupby(
    {"biofuel": ["biofuel-biogas", "biofuel-blended"]}, "Variables", "sum", inplace=True
)
dm_iww.operation(
    "biofuel", "/", "total", "Variables", out_col="biofuel-share", unit="%"
)
...
dict_extract = {
    ...
    "sheet_last_row": "Biogases",
    "sub_variables": ["Intra-EEA", "Blended biofuels", "Biogases"],
    "calc_names": ["total", "biofuel-blended", "biofuel-biogas"],
}
...
dict_extract = {
    ...
    "sub_variables": ["by fuel", "blended liquid biofuels"],
    "calc_names": ["total", "biofuel"],
}
...
dm_biof.deepen()
dm_biof.add(
    0, col_label="efuel", dummy=True, dim="Variables", unit="%"
)  # assuming efuel at 0
...
dm_temp_avi = dm_biof.filter(
    {"Variables": ["biofuel"], "Categories1": ["aviation"]}
).flatten()
idx = dm_temp_avi.idx
...
dm_mar = linear_fitting(
    dm_mar, list(range(startyear, 1999 + 1)), min_t0=0, min_tb=0, based_on=[2000]
)
dm_mar = linear_fitting(
    dm_mar, list(range(2022, 2023 + 1)), min_t0=0, min_tb=0, based_on=[2020]
)
dm_iww = linear_fitting(
    dm_iww, list(range(startyear, 1999 + 1)), min_t0=0, min_tb=0, based_on=[2000]
)
dm_iww = linear_fitting(
    dm_iww, list(range(2022, 2023 + 1)), min_t0=0, min_tb=0, based_on=[2019]
)
