...
dict_call = {
    "HDVH_BEV": {"n_adj": 2, "year_end_first_adj": 2010, "year_start_second_adj": 2010},
...
for key in dict_call.keys():
    if len(dict_call[key]) > 1:
        ...
        dm_eneff_new = make_fts(
            dm_eneff_new, key, baseyear_start, baseyear_end, dim="Variables"
        )
...
dm_eneff_new.change_unit(
    "tra_freight_vehicle-efficiency_new", 1e-2, "MJ/100 km", "MJ/km"
)
...
DM_ene = {
    "ots": {"freight_vehicle-efficiency_new": []},
    "fts": {"freight_vehicle-efficiency_new": dict()},
}
DM_ene["ots"]["freight_vehicle-efficiency_new"] = dm_eneff_new.filter(
    {"Years": list(range(1990, 2023 + 1))}
)
for i in range(1, 4 + 1):
    DM_ene["fts"]["freight_vehicle-efficiency_new"][i] = dm_eneff_new.filter(
        {"Years": years_fts}
    )
...
f = os.path.join(
    current_file_directory,
    "../data/datamatrix/lever_freight_vehicle-efficiency_new.pickle",
)
