...
def extract_u_value(df_uvalue):
    ...
    df_uvalue.loc[df_uvalue["end_y"] == 1945, "Years"] = (
        1900  # TODO: check this modif with Paola for CH
    )
...
def extract_floor_area_stock():
    ...
    df_area.loc[df_area["end_y"] == 1945, "Years"] = (
        1900  # TODO: check this modif with Paola for CH
    )
...
def estimate_waste_fix_stock(dm_area_stock_tmp, dm_area_new):
    ...
    idx1 = dm_temp.idx
    dm_area_out.array[idx2["Malta"], :, idx2["bld_floor-area_stock"], :] = (
        dm_temp.array[idx1["Malta"], ...]
    )
    ...
    dm_area_out = linear_fitting(
        dm_area_out, years_ots, based_on=list(range(1990, 1995 + 1, 1))
    )
    # dm_area_out.flatten().filter({"Country" : ["EU27"]}).datamatrix_plot()
...
def get_renovation_rate():
    ...
    dm_rr = dm_rr.filter({"Categories1": ["med", "dep"]}).group_all(
        "Categories1", inplace=False
    )
    # dm_rr.flatten().datamatrix_plot()

    # Make EU
    dm_rr_EU27 = dm_rr.groupby(
        {"EU27": EU27_cntr_list}, dim="Country", inplace=False, aggregation="mean"
    )
    dm_rr.append(dm_rr_EU27, "Country")
    dm_rr.sort("Country")
...
def compute_stock_area_by_cat(
    ...
    dm_stock_cat.array[:, idx1[2020], :, :, :] = (
        arr_temp1
        * arr_temp2[..., np.newaxis]
        / np.sum(arr_temp1, axis=3, keepdims=True)
    )
...
def extract_stock_byconstrperiod_2020():
    ...
    for i in range(len(variabs_surface)):
        df_area.rename(
            columns={variabs_surface[i]: variabs_surface_new[i]}, inplace=True
        )
    ...
    df_area = df_area.pivot(
        index=indexes, columns="variable", values="value"
    ).reset_index()
...
def extract_heating_mix():
    ...
    df["variable"] = (
        df["bld-type"] + "_" + df["variable"] + "_" + df["construction-year"]
    )
    ...
    dm.drop(
        "Categories2", ["space-cooling", "no-space-cooling"]
    )  # drop space cooling subs
    ...
    dm_temp = dm_out.filter(
        {"Variables": ["multi-family-households", "single-family-households", "total"]}
    )
    type_missing = ["education", "health", "hotels", "offices", "other", "trade"]
    idx = dm_temp.idx
    for t in type_missing:
        dm_temp.add(
            dm_temp.array[:, :, idx["total"], :, :, :],
            dim="Variables",
            col_label=t,
            unit="%",
        )
    ...
    names_map = {
        "Stock of households": "remove",
        "Space heating": "total",
        "Solids": "coal",
        "Liquified petroleum gas (LPG)": "other",
    ...
    dm_jrc_sh = df_excel_to_dm(
        df_temp,
        names_map,
        var_name="bld_heating-mix",
        unit="number",
        num_cat=1,
        country="EU27",
    )
    ...
    arr_temp = (dm_jrc.array - arr_2017[:, np.newaxis, :, :, :]) / arr_2017[
        :, np.newaxis, :, :, :
    ]
    ...
    linear_fitting(
        dm_out,
        years_missing_e,
        based_on=list(range(2015, dm_out.col_labels["Years"][-1] + 1)),
    )
...
def extract_heating_efficiency(file, sheet_name, years_ots):
    ...
    names_map = {
        ...
        "Space heating": "total",
        "Solids": "coal",
        "Liquified petroleum gas (LPG)": "other-tech",
        "Diesel oil": "heating-oil",
        "Natural gas": "gas",
        "Biomass": "wood",
        "Geothermal": "other-tech",
        "Distributed heat": "district-heating",
        "Advanced electric heating": "heat-pump",
        "Conventional electric heating": "electricity",
    }
    dm_jrc_sh = df_excel_to_dm(
        df_temp,
        names_map,
        var_name="bld_heating-efficiency_sh",
        unit="%",
        num_cat=1,
        country="EU27",
    )
    ...
    dm_jrc_dwh = df_excel_to_dm(
        df_temp,
        names_map,
        var_name="bld_heating-efficiency_dhw",
        unit="%",
        num_cat=1,
        country="EU27",
    )
...
def get_household_size_eustat():
    ...
    dm_hh_size = get_data_api_eurostat(
        "ilc_lvph01", filter, mapping_dim, unit="people", years=years_ots
    )
    ...
    for bld_type in dm_stock_cat.col_labels["Categories1"]:
        for cat, start_yr in first_bld.items():
            ...
            for idx_c in range(nb_cntr):
                dm_age.array[idx_c, :, idx["bld_age"], idx[bld_type], idx[cat]] = (
                    arr_age
                )
    return dm_age
...
def get_energy_demand_useful_dhw():
    ...
    dm_pop = dm_pop.filter(
        {"Country": dm_dhw.col_labels["Country"], "Years": dm_dhw.col_labels["Years"]}
    )
    dm_pop.sort("Years")
    del DM_lfs
...
def get_domapp():
    ...
    def get_domapp_from_jrc(country_code, country_name):
        ...
        dm_out = df_excel_to_dm(
            df_temp,
            names_map,
            var_name="bld_appliances_stock",
            unit="unit/household",
            num_cat=1,
            country=country_name,
        )
        ...
        dm_temp = df_excel_to_dm(
            df_temp,
            names_map,
            var_name="bld_appliances_stock_absolute",
            unit="unit",
            num_cat=1,
            country=country_name,
        )
        ...
        dm_temp = df_excel_to_dm(
            df_temp,
            names_map,
            var_name="bld_appliances_replaced",
            unit="unit",
            num_cat=1,
            country=country_name,
        )
        ...
        dm_temp = df_excel_to_dm(
            df_temp,
            names_map,
            var_name="bld_appliances_electricity-demand_absolute",
            unit="ktoe",
            num_cat=1,
            country=country_name,
        )
    ...
    dm_domapp.operation(
        "bld_appliances_replaced",
        "/",
        "bld_appliances_stock_absolute",
        "Variables",
    ...
    dm_domapp.change_unit(
        "bld_appliances_electricity-demand_absolute", 1.163 * 1e7, "ktoe", "kWh"
    )
    dm_domapp.operation(
        "bld_appliances_electricity-demand_absolute",
        "/",
        "bld_appliances_stock_absolute",
        "Variables",
    ...
    dm_domapp.append(
        dm_temp.filter({"Variables": ["bld_appliances_retirement-rate"]}), "Variables"
    )
    dm_domapp.append(
        dm_temp.filter({"Variables": ["bld_appliances_stock"]}), "Variables"
    )
    ...
    dm_domapp.add(
        np.nan,
        "Categories1",
        ["monitor", "laptop", "set-top-box", "oven-and-stove"],
        dummy=True,
    )
...
def get_services():
    ...
    def get_energy_demand_useful_from_jrc(country_code, country_name):
        ...
        names_map = {
            "Solids": "coal",
            "Liquified petroleum gas (LPG)": "gas",
            "Diesel oil": "heating-oil",
            "Gas heat pumps": "heat-pump",
        ...
        dm_jrc_sh = df_excel_to_dm(
            df_temp,
            names_map,
            var_name="bld_services_tech-mix_space-heating",
            unit="ktoe",
            num_cat=1,
            country=country_name,
        )
        ...
        dm_jrc_wh = df_excel_to_dm(
            df_temp,
            names_map,
            var_name="bld_services_tech-mix_hot-water",
            unit="ktoe",
            num_cat=1,
            country=country_name,
        )
        ...
        df = pd.read_excel(file, sheet_name=sheet)
        df_temp = df[3:5]
        names_map = {"Street lighting": "street", "Building lighting": "building"}
        dm_jrc_light = df_excel_to_dm(
            df_temp,
            names_map,
            var_name="bld_services_tech-mix_lighting_electricity",
            unit="ktoe",
            num_cat=1,
            country=country_name,
        )
        ...
        for cat in categories_missing:
            dm_jrc_light.add(0, "Categories2", cat, dummy=True)
        dm_jrc.append(dm_jrc_light, "Categories1")

        # move electricity to elec
        dm_elec = dm_jrc.filter({"Categories2": ["electricity"]})
        dm_elec.groupby(
            {"elec": ["hot-water", "space-heating"]}, "Categories1", inplace=True
        )
        for cat in categories_missing:
            dm_elec.add(0, "Categories2", cat, dummy=True)
    ...
    def get_efficiency_from_jrc(country_code, country_name):
        ...
        dm_jrc_sh = df_excel_to_dm(
            df_temp,
            names_map,
            var_name="bld_efficiency_space-heating",
            unit="%",
            num_cat=1,
            country=country_name,
        )
        ...
        dm_jrc_wh = df_excel_to_dm(
            df_temp,
            names_map,
            var_name="bld_efficiency_water-heating",
            unit="%",
            num_cat=1,
            country=country_name,
        )
    ...
    dm_demand.rename_col(
        "bld_services_tech-mix", "bld_services_useful-energy", "Variables"
    )
    ...
    # dm_demand.filter({"Country" : ["EU27"]}).flatten().datamatrix_plot()
    for y in [2022, 2023]:
        dm_demand[:, y, :, "lighting"] = dm_demand[:, 2021, :, "lighting"]
    for y in years_fts:
        dm_demand[:, y, :, "lighting"] = dm_demand[:, 2021, :, "lighting"]
...
def get_domapp_other():
    ...
    dm_domapp_lighting.rename_col(
        "bld_lighting", "bld_residential-lighting", "Variables"
    )
    ...
    linear_fitting(
        ...
        based_on=list(range(2000, 2008 + 1)),
    )
    # dm_domapp_lighting.filter({"Country" : ["EU27"]}).datamatrix_plot()
    linear_fitting(
        dm_domapp_lighting,
        list(range(2022, 2023 + 1)),
        based_on=list(range(2020, 2021 + 1)),
    )
    # dm_domapp_lighting.filter({"Country" : ["EU27"]}).datamatrix_plot()
    ...
    dm_domapp_other.rename_col(
        "bld_other", "bld_energy-demand_other_electricity", "Variables"
    )
    dm_domapp_other.change_unit(
        "bld_energy-demand_other_electricity", 11630000, "ktoe", "kWh"
    )
    ...
    dm_pop = dm_pop.filter(
        {
            "Country": dm_domapp_other.col_labels["Country"],
    ...
    dm_domapp_other.rename_col(
        "per_capita", "bld_energy-demand_other_electricity", "Variables"
    )
...
DM_bld["fxa"]["bld_type"].filter({"Country": ["Switzerland"]}).group_all(
    "Categories2", inplace=False
).group_all("Categories1", inplace=False).array
...
dm_area_new = estimate_floor_area(
    dm_new_1, dm_new_2, years_ots
)  # this is over 1990-2023
...
construction_period_envelope_cat = {
    "F": ["0-1945", "1946-1969"],
    "E": ["1970-1979"],
    "D": ["1980-1989"],
    "C": ["1990-1999"],
...
dm_stock_cat_2020 = dm_stock_byconsper_2020.groupby(
    construction_period_envelope_cat, dim="Categories2"
)

# Check
dm_temp = dm_stock_cat_2020.groupby(
    {"B": ["B"], "C": ["C"], "D": ["D"], ">D": ["F", "E"]}, "Categories2", inplace=False
)
dm_temp.filter(
    {
        "Country": ["EU27"],
...
surface_to_floorarea = {
    "single-family-households": np.round(0.8 + 0.18 + 1.17 + 0.72, 1),
...
dm_heating_cat = dm_heating_mix.groupby(
    construction_period_envelope_cat, dim="Categories3", aggregation="mean"
)
...
dm_temp = DM_bld["fxa"]["heating-energy-calibration"].filter(
    {"Country": ["Switzerland"]}
)
...
dm_temp.deepen(based_on="Variables")
dm_temp = dm_temp.filter(
    {"Categories3": ["multi-family-households", "single-family-households"]}
)
...
dm_heating_eff.filter(
    {"Categories1": dm_heating_cat.col_labels["Categories2"]}, inplace=True
)
...
dm_temp.filter(
    {
        "Categories1": DM_bld["fxa"]["hot-water"]["hw-efficiency"].col_labels[
            "Categories1"
        ]
    },
    inplace=True,
