...
bld_type_mapping = {"SFH": "single-family-households", "MFH": "multi-family-households"}
# --------------------------------------------------------------------------------
# UTILITY FUNCTIONS
# --------------------------------------------------------------------------------
...
df_floor_intensity = df_floor_intensity[
    ["Country", "Years", "lfs_floor-intensity_space-cap[m2/cap]"]
]
...
dm_ots = DM_buildings["ots"]["floor-intensity"].filter(
...
dm_ots.append(
    DM_buildings["fts"]["floor-intensity"][3].filter(
...
dm_sre_dls = DM_buildings["fts"]["floor-intensity"][3].filter(
    {"Variables": ["lfs_floor-intensity_space-cap"]}
)
df_sre_dls = dm_sre_dls.write_df()
df_sre_dls["lfs_floor-intensity_space-cap[m2/cap]"] = df_sre_dls.apply(
    lambda row: 35.0 if row["Years"] == 2050 else np.nan, axis=1
)
dm_sre_dls = DataMatrix.create_from_df(df_sre_dls, num_cat=0)

dm_ots_dls = DM_buildings["ots"]["floor-intensity"].filter(
    {"Variables": ["lfs_floor-intensity_space-cap"]}
)
...
dm_bld_building_mix_new_adj = copy.deepcopy(
    DM_buildings["fts"]["building-renovation-rate"]["bld_building-mix"][1]
)
dm_bld_building_mix_new_adj[0, 1:-1, "bld_building-mix_new", :, "B"] = np.nan
dm_bld_building_mix_new_adj[
    0, -1, "bld_building-mix_new", "multi-family-households", "B"
] = (
    dm_bld_building_mix_new_adj[
        0, 0, "bld_building-mix_new", "multi-family-households", "B"
    ]
    + 0.01
)
dm_bld_building_mix_new_adj[
    0, -1, "bld_building-mix_new", "single-family-households", "B"
] = (
    dm_bld_building_mix_new_adj[
        0, 0, "bld_building-mix_new", "single-family-households", "B"
    ]
    - 0.01
)
...
DM_buildings["fts"]["building-renovation-rate"]["bld_renovation-rate"][3] = (
    dm_bld_renovation_rate.filter({"Variables": ["bld_renovation-rate"]})
)
...
dm_renovation = DM_buildings["ots"]["building-renovation-rate"][
    "bld_renovation-rate"
].copy()
...
df_renovation_dls["bld_renovation-rate_single-family-households[%]"] = (
    df_renovation_dls.apply(
        lambda row: (
            0.038
            if row["Years"] == 2030
            else (0.05 if row["Years"] == 2050 else np.nan)
        ),
        axis=1,
    )
)
df_renovation_dls["bld_renovation-rate_multi-family-households[%]"] = (
    df_renovation_dls.apply(
        lambda row: (
            0.038
            if row["Years"] == 2030
...
dm_heatmix_ots = DM_buildings["ots"]["heating-technology-fuel"][
...
dm_heattech_fts.operation(
    "bld_heating-technology_stock",
    "-",
    "bld_heating-technology_stock_tm1",
    out_col="delta_stock",
    unit="%",
)
dm_heattech_fts.operation(
...
for t in years_fts_full:
    for cat in dm_heatmix.col_labels["Categories2"]:
        for fuel in dm_heatmix.col_labels["Categories3"]:
            if not (
                cat == "F" and fuel == "heat-pump"
            ):  # no new hp in F buildings -> more DH
                growthrate_t = dm_heattech_fts[cntr, t, "growth_rate", :, fuel]
                dm_heatmix[cntr, t, "bld_heating-mix", :, cat, fuel] = np.maximum(
                    dm_heatmix[cntr, tm1, "bld_heating-mix", :, cat, fuel]
                    * (1 + growthrate_t),
                    0,
                )
    tm1 = t
...
# DLS scenario
DM_buildings["fts"]["heating-technology-fuel"]["bld_heating-technology"][4] = (
    DM_buildings["fts"]["heating-technology-fuel"]["bld_heating-technology"][3].copy()
)

df_heat_dls = DM_buildings["fts"]["heating-technology-fuel"]["bld_heating-technology"][
    4
].write_df()
...
dm_ots = (
    ...
    .filter({"Variables": ["bld_heating-efficiency"]})
    .copy()
)
...
DM_buildings["fts"]["heating-efficiency"][4] = DM_buildings["fts"][
    "heating-efficiency"
][3]
...
dm_stock_cat.add(
    arr_waste, dim="Variables", col_label="bld_floor-area_waste", unit="Mm2"
)
...
arr_new = (
    dm_stock_all[0, :, "bld_floor_area_stock", :]
    - dm_stock_all[0, :, "bld_floor_area_stock_tm1", :]
...
dm_stock_cat.add(
    0, dim="Variables", col_label="bld_floor-area_new", unit="Mm2", dummy=True
)
dm_stock_cat[0, :, "bld_floor-area_new", :, "B"] = dm_stock_all[
    0, :, "bld_floor-area_new", :
]
arr_ren = (
    dm_stock_cat[0, :, "bld_floor_area_stock", ...]
    - dm_stock_cat[0, :, "bld_floor_area_stock_tm1", ...]
    - dm_stock_cat[0, :, "bld_floor-area_new", ...]
    + dm_stock_cat[0, :, "bld_floor-area_waste", ...]
)
dm_stock_cat.add(
    arr_ren[np.newaxis, ...],
    dim="Variables",
    col_label="bld_floor-area_renovated",
    unit="m2",
)
...
dm_renov_redistr.rename_col(
    "bld_floor-area_renovated", "bld_floor-area_ren-normalised", dim="Variables"
)

dm_renov_redistr.add(
    0,
    col_label="bld_renovation-redistribution-in",
    dim="Variables",
    unit="%",
    dummy=True,
)
dm_renov_redistr.add(
    0,
    col_label="bld_renovation-redistribution-out",
    dim="Variables",
    unit="%",
    dummy=True,
)
...
dm_renov_redistr[0, :, "bld_renovation-redistribution-out", :] = dm_renov_redistr[
    0, :, "bld_floor-area_ren-normalised", :
]
...
DM_buildings["fts"]["building-renovation-rate"]["bld_renovation-redistribution"][3] = (
    copy.deepcopy(
...
DM_buildings["fts"]["building-renovation-rate"]["bld_renovation-redistribution"][4] = (
    DM_buildings["fts"]["building-renovation-rate"]["bld_renovation-redistribution"][3]
)
