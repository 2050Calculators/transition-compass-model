...
def run(country_list, years_ots):
    ...
    dm_lighting_CH_consumption = lt.extract_EP2050_lighting_energy_consumption(
        file_raw=local_filename, file_pickle=file_pickle
    )
    linear_fitting(
        dm_lighting_CH_consumption, years_ots, based_on=list(range(2000, 2004))
    )
    ...
    arr = (
        dm_shares[:, :, :] * dm_lighting_CH_consumption["Switzerland", np.newaxis, :, :]
    )
    dm_lighting_CH_consumption.add(
        arr, dim="Country", col_label=dm_shares.col_labels["Country"]
    )
