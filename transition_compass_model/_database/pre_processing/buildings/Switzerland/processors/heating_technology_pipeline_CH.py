...
def add_dummy_envelope_cat(dm_heating_eff):
    dm_heating_eff.rename_col(
        "bld_heating_efficiency", "bld_heating-efficiency_B", "Variables"
    )
    dm_heating_eff.deepen(based_on="Variables")
    dm_heating_eff.add(
        np.nan, dim="Categories2", col_label=["C", "D", "E", "F"], dummy=True
    )
...
def calibrate_cantons_heating_tech_based_on_EP2050(
    ...
    def calibrate_heating(dm_raw, dm_cal):
        ...
        dm_shares.array = dm_shares.array / np.nansum(
        ...
        dm_raw.add(
            np.nan, dim="Variables", col_label="bld_heating-tech", unit="m2", dummy=True
        )

        idx = dm_raw.idx
        for tech in dm_shares.col_labels["Categories3"]:
            dm_raw[:, idx[2000] :, "bld_heating-tech", :, :, tech] = dm_shares[
                :, idx[2000] :, "bld_heating-tech", :, :, tech
            ]

        dm_raw.operation(
            "bld_heating-tech", "/", "bld_heating-mix", out_col="ratio", unit="%"
        )
        ...
        linear_fitting(
            dm_raw, based_on=list(range(1990, 2000)), years_ots=list(range(2001, 2021))
        )
        ...
        dm_raw.operation(
            "bld_heating-mix", "*", "ratio", out_col="bld_heating-tech", unit="m2"
        )
...
def adjust_tech_mix(dm_tech_mix, dm_apt):
    ...
    arr = dm_apt_loc[:, :, 0, :, np.newaxis, np.newaxis] * dm_tech_mix[:, :, 0, :, :, :]
...
def run(global_var, dm_all, country_list, years_ots):
    ...
    dm_heating_tech_old = ht.compute_heating_mix_F_E_D_categories(
    ...
    dm_heating_EP2050 = ht.extract_heating_technologies_EP2050(
    ...
    # Floor area CH
    dm_floor_area_CH = dm_all.filter(
        {"Variables": ["bld_floor-area_stock"], "Country": ["Switzerland"]}
    ).group_all("Categories2", inplace=False)

    assert (
        dm_floor_area_CH.col_labels["Categories1"]
        == dm_heating_EP2050.col_labels["Categories1"]
    )
    arr = (
        dm_heating_EP2050[:, :, "bld_heating-mix", :, :]
        * dm_floor_area_CH[:, :, "bld_floor-area_stock", :, np.newaxis]
    )
    ...
    dm_heating_tech = calibrate_cantons_heating_tech_based_on_EP2050(
        dm_heating_tech_cantons, dm_heating_EP2050
    )
    ...
    dm_heating_eff = extract_heating_efficiencies_EP2050(
        file_url, zip_name, file_pickle
    )
    dm_heating_eff.add(1, dim="Categories1", col_label=["electricity"], dummy=True)
    dm_heating_eff.add(
        0.6, dim="Categories1", col_label=["solar", "other-tech", "coal"], dummy=True
    )
    dm_heating_eff.sort("Categories1")
...
if __name__ == "__main__":
    ...
    DM_renov = renovation_run(
        dm_stock_tot, dm_stock_cat, dm_new_cat, dm_waste_cat, years_ots
    )
