...
def fill_missing_years_using_FSO_data(dm, dm_raw):
    ...
    dm_raw.rename_col(
        "bld_floor-area_stock", "bld_floor-area_stock_raw", dim="Variables"
    )
    dm.append(dm_raw, dim="Variables")
    dm.operation(
        "bld_floor-area_stock",
        "/",
        "bld_floor-area_stock_raw",
        out_col="cal_factor",
        unit="%",
    )
    idx = dm.idx
    ...
    dm.fill_nans("Years")
    dm[:, :, "bld_floor-area_stock", :] = (
        dm[:, :, "cal_factor", :] * dm[:, :, "bld_floor-area_stock_raw", :]
    )
...
def clean_WP_ERA_file(df, cantons_name_list):
    ...
    df_residential.rename(
        columns={
            "NAME": "Country",
            "EFH": "bld_floor-area_stock_single-family-households[m2]",
            "MFH": "bld_floor-area_stock_multi-family-households[m2]",
            "Sonstige_Wohngeb": "bld_floor-area_stock_other[m2]",
        },
        inplace=True,
    )
    ...
    dm_other_split[:, 2023, :, :] = (
        dm_other_split[:, 2023, :, :] * dm_other[:, 2023, :, :]
    )
    dm[...] = dm_other_split[...] + dm[...]

    dm.groupby(
        {"single-family-households": "single-family.*"},
        regex=True,
        dim="Categories1",
        inplace=True,
    )
...
def run(global_vars, country_list, years_ots):
    ...
    dm_stock_tot, dm_stock_cat, dm_avg_floor_area = fla.compute_floor_area_stock_v2(
        table_id,
        file,
        dm_pop=dm_pop,
    ...
    file = os.path.join(
        this_dir, "../data/bld_new_buildings_1995_2012_all_cantons.pickle"
    )
    dm_bld_new_buildings_2 = fla.extract_bld_new_buildings_2(table_id, file)
    # Floor-area new by sfh, mfh
    dm_new_tot_raw = fla.compute_bld_floor_area_new(
        dm_bld_new_buildings_1,
        dm_bld_new_buildings_2,
        dm_avg_floor_area,
        dm_pop,
        years_ots,
    )
    ...
    dm_stock_tot_CH = fla.extract_EP2050_floor_area_by_type(
    ...
    dm_stock_tot_CH = fill_missing_years_using_FSO_data(
        dm_stock_tot_CH, dm_stock_tot_CH_raw
    )
    ...
    assert (
        dm_stock_tot.col_labels["Categories1"]
        == dm_stock_tot_CH.col_labels["Categories1"]
    )
    dm_stock_tot[...] = dm_stock_tot[...] * dm_stock_tot_CH[...]
    dm_stock_tot.change_unit(
        "bld_floor-area_stock", old_unit="%", new_unit="m2", factor=1
    )
    ...
    assert (
        dm_stock_tot.col_labels["Categories1"] == dm_stock_cat.col_labels["Categories1"]
    )
    assert dm_stock_tot.col_labels["Country"] == dm_stock_cat.col_labels["Country"]
    dm_stock_cat[:, :, :, :, :] = (
        dm_stock_tot[:, :, :, :, np.newaxis] * dm_stock_cat[:, :, :, :, :]
    )
    dm_stock_cat.change_unit(
        "bld_floor-area_stock", old_unit="%", new_unit="m2", factor=1
    )

    # SECTION Floor-area Waste + Recompute New
    dm_waste_tot, dm_new_tot = fla.compute_waste(
        dm_stock_tot, dm_new_tot_raw, years_ots
    )
    ...
    DM = {
        "stock tot": dm_stock_tot.filter({"Country": country_list}),
