...
def map_national_energy_demand_by_sector_to_cantons(
    ...
    dm_employees_mapped.filter(
        {"Categories1": mapping_sectors["services"]}, inplace=True
    )
    ...
    dm_energy_mapped.groupby(
        {"services": mapping_sectors["services"]}, dim="Categories1", inplace=True
    )
    dm_employees_mapped.groupby(
        {"services": mapping_sectors["services"]}, dim="Categories1", inplace=True
    )
...
def split_fuel_demand_by_eud(
    ...
    arr = (
        dm_hw_sp[:, :, var, :, np.newaxis] * dm_fuel_split[:, :, "bld_households", :, :]
    )
...
def adjust_based_on_FSO_energy_consumption(
    dm_fuels_cantons, dm_services_fuels_eud_cantons, years_ots
):
...
def energy_split_from_fuel_to_tech(
    ...
    dm_eff = dm_efficiency.filter(
        {"Years": dm_solar.col_labels["Years"], "Country": ["Switzerland"]}
    )
    ...
    arr_solar = (
        dm_energy[:, :, :, :, "renewables"] * dm_solar[:, :, :, np.newaxis, "solar"]
    )
    dm_energy.add(arr_solar, dim="Categories2", col_label="solar")
    dm_energy.operation(
        "renewables", "-", "solar", out_col="renewable", dim="Categories2"
    )
    dm_energy.drop("Categories2", "renewables")
    dm_eff.filter({"Years": dm_energy.col_labels["Years"]}, inplace=True)
    arr_HP = dm_energy[:, :, :, :, "renewable"] / (
        dm_eff[:, :, :, np.newaxis, "heat-pump"] - 1
    )
    dm_energy.add(arr_HP, dim="Categories2", col_label="heat-pump")
    dm_energy[:, :, :, :, "electricity"] = (
        dm_energy[:, :, :, :, "electricity"] - dm_energy[:, :, :, :, "heat-pump"]
    )
...
def run(country_list, years_ots):
    ...
    dm_water.operation(
        ...
        out_col="bld_households_hot-water",
        unit="%",
    )
    ...
    dm_services_eud = fser.load_services_energy_demand_eud(
        services_agri_split, years_ots
    )
    ...
    dm_energy_services = dm_energy.filter(
        {"Categories1": mapping_sectors["services"]}, inplace=False
    )
    ...
    dm_energy_services = energy_split_from_fuel_to_tech(
        dm_energy_services,
        dm_water_CH,
        dm_space_heat_CH,
        dm_efficiency,
    ...
    dm_fuels_cantons, dm_employees_mapped = (
        map_national_energy_demand_by_sector_to_cantons(
            dm_energy_services, dm_employees, mapping_dict, mapping_sectors, years_ots
        )
    )
    ...
    dm_eff.rename_col(
        "bld_hot-water_efficiency", "bld_services_efficiency", dim="Variables"
    )

    dm_add_missing_variables(
        dm_eff,
        {"Categories1": dm_services_fuels_eud_cantons_FSO.col_labels["Categories2"]},
    )
    ...
    dm_tot_demand.rename_col(
        "enr_services-energy-eud", "bld_services_useful-energy", dim="Variables"
    )
    ...
    dm_tech_mix.rename_col(
        "enr_services-energy-eud_share", "bld_services_tech-mix", dim="Variables"
    )
