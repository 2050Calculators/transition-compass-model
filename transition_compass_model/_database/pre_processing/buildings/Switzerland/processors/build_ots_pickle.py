...
def run(dm_pop, DM_all, years_ots, years_fts):
    ...
    DM_buildings["fxa"]["other-electricity-demand"] = DM_appliances["fxa"][
    ...
    arr = (
        dm_light[:, :, "bld_residential-lighting"]
    ...
    for yr, value in fts_light.items():
        ...
        dm_light[:, yr, "bld_residential-lighting"] = (
            shares * dm_light["Switzerland", np.newaxis, yr, "bld_residential-lighting"]
        )
    ...
    dm_add_missing_variables(
        DM_services["services_tech-mix"], {"Years": years_fts}, fill_nans=False
    )
    dm_add_missing_variables(
    ...
    dm_lfs_household_size = DM_appliances["fxa"]["household"].filter(
    ...
    DM_buildings["ots"]["building-renovation-rate"]["bld_building-mix"] = (
        dm_building_mix.filter({"Variables": ["bld_building-mix_new"]})
    )
    ...
    dm_tot = dm_all.filter(
        {"Variables": ["bld_floor-area_stock", "bld_floor-area_waste"]}
    )
