...
def run(country_list, years_ots):
    ...
    parameters = dict()
    mapping = {
        "heating-oil": ".*Produits pétroliers.*",
        "gas": ".*Gaz.*",
        "coal": ".*Charbon.*",
        "wood": ".*bois.*",
    ...
    dm_residential_energy = get_data.extract_energy_statistics_data(
        file_url,
        local_filename,
        sheet_name="T17a",
    ...
    parameters["unit"] = None
    parameters["cols to drop"] = ".*%.*|.*extra.*"
    dm_services_energy = get_data.extract_energy_statistics_data(
        file_url,
        local_filename,
        sheet_name="T17c",
        parameters=parameters,
