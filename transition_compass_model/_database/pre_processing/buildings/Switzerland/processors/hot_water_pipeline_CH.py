...
def adjust_tech_mix(dm_tech_mix, dm_apt):
    ...
    arr = (
        dm_apt[:, :, "bld_apartments", :, np.newaxis]
        * dm_tech_mix[:, :, "bld_hot-water_tech", :, :]
    )

    dm_tech_mix.add(
        arr, dim="Variables", col_label="bld_hot-water_tech-mix", unit="number"
    )
...
def distribute_hw_energy_consumption_to_cantons(
    ...
    arr = (
        dm_tech[:, :, "bld_hot-water_tech-mix", :]
    ...
    assert (
        dm_water_CH_consumption.col_labels["Categories1"]
        == dm_tech.col_labels["Categories1"]
    )

    arr = (
        dm_tech[:, :, "bld_hw_pop_by_tech", :]
        * dm_water_CH_consumption[
            "Switzerland", np.newaxis, :, "bld_hot-water_energy-consumption", :
        ]
    )

    dm_tech.add(
        arr, col_label="bld_hot-water_energy-consumption", dim="Variables", unit="TWh"
    )
    dm_canton_share = dm_tech.filter({"Variables": ["bld_hw_pop_by_tech"]})
    dm_water_consumption = dm_tech.filter(
        {"Variables": ["bld_hot-water_energy-consumption"]}
    )
...
def extrapolate_missing_years_based_on_tech_mix(
    ...
    dm_cons.operation(
        "bld_hot-water_energy-consumption",
        "/",
        "lfs_population_total",
        out_col="bld_energy-demand-cap",
        unit="TWh/cap",
    )
    ...
    dm_tech.rename_col(
        "bld_hot-water_energy-consumption_share", "bld_hot-water_tech", dim="Variables"
    )

    dm_add_missing_variables(
        dm_water_CH_consumption, {"Years": years_ots}, fill_nans=False
    )
    dm_water_CH_consumption[:, :, "bld_hot-water_energy-consumption", :] = (
        dm_tech[:, :, "bld_hot-water_tech", :]
        * dm_cons[:, :, "bld_hot-water_energy-consumption", np.newaxis]
    )
...
def run(country_list, years_ots):
    ...
    dm_efficiencies = hw.extract_heating_efficiencies_EP2050(
    ...
    file_pickle = os.path.join(
        this_dir, "../data/EP2050_hot_water_households_CH.pickle"
    )
    dm_water_CH_consumption = hw.extract_EP2050_hot_water_energy_consumption(
        file_raw=local_filename, file_pickle=file_pickle
    )
    dm_water_CH_consumption.drop("Categories1", "ambient-heat")
    dm_water_CH_consumption = extrapolate_missing_years_based_on_tech_mix(
        dm_water_CH_consumption, dm_pop, years_ots
    )
    ...
    dm_water_consumption, dm_canton_share = distribute_hw_energy_consumption_to_cantons(
        dm_water_CH_consumption, dm_tech_mix, dm_pop
    )
    ...
    dm_efficiencies.sort("Categories1")
    assert (
        dm_water_consumption.col_labels["Categories1"]
        == dm_efficiencies.col_labels["Categories1"]
    )
    dm_efficiencies.filter(
        {"Years": dm_water_CH_consumption.col_labels["Years"]}, inplace=True
    )

    arr = (
        dm_water_consumption[:, :, "bld_hot-water_energy-consumption", :]
        * dm_efficiencies["Switzerland", np.newaxis, :, "bld_heating_efficiency", :]
    )
    dm_water_consumption.add(
        arr, dim="Variables", col_label="bld_hw_useful-energy", unit="TWh"
    )
    ...
    dm_water_tot.append(
        dm_pop.filter({"Years": dm_water_tot.col_labels["Years"]}), dim="Variables"
    )
    dm_water_tot.operation(
    ...
    dm_water_tot.change_unit(
        "bld_hw_demand", old_unit="TWh/cap", new_unit="MWh/cap", factor=1e6
    )
    ...
    dm_tech_mix_useful = dm_water_consumption.filter(
        {"Variables": ["bld_hw_useful-energy"]}
    )
    dm_tech_mix_useful.normalise("Categories1")
    dm_tech_mix_useful.rename_col(
        "bld_hw_useful-energy", "bld_hw_tech-mix", dim="Variables"
    )
    ...
    DM = {
        ...
        "hw-energy-demand": dm_water_tot.filter(
            {"Variables": ["bld_hw_demand"], "Country": country_list}
        ),
    }
