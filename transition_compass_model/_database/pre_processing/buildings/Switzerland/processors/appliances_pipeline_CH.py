...
def run(dm_pop, country_list, years_ots, years_fts):
    ...
    raw_filename = os.path.join(
        this_dir, "../data/appliances_stock_new_energy_CH_raw.csv"
    )
    save_url_to_file(file_url, raw_filename)
    clean_filename = os.path.join(
        this_dir, "../data/appliances_stock_new_energy_CH.pickle"
    )

    dm_appliances = get_data.get_appliances_stock_new_energy(
        raw_filename, clean_filename
    )
    ...
    missing_years = list(
        set(years_ots + years_fts) - set(dm_appliances.col_labels["Years"])
    )
    ...
    dm_households = get_data.households_fill_missing_years(
        dm_households, dm_pop, years_ots, years_fts
    )
    ...
    dm_cooking = get_data.extract_EP2050_cooking_energy_consumption(
        file_raw=local_filename
    )
    ...
    dm_appliances[
        "Switzerland", :, "bld_appliances_electricity-demand_stock", "oven-and-stove"
    ] = dm_cooking["Switzerland", :, "bld_energy-demand_cooking", "electricity"]

    dm_energy = dm_appliances.filter(
        {"Variables": ["bld_appliances_electricity-demand_stock"]}
    )
    dm_energy.change_unit(
        "bld_appliances_electricity-demand_stock",
        old_unit="kWh",
        new_unit="TWh",
        factor=1e-9,
    )
    ...
    dm_appliances.operation(
        "bld_appliances_electricity-demand_stock",
        "/",
        "bld_appliances_stock",
        out_col="bld_appliances_electricity-demand",
        unit="kWh/unit",
    )

    dm_appliances.filter(
        {
            "Variables": [
                "bld_appliances_stock",
                "bld_appliances_new",
    ...
    dm_appliances.operation(
        ...
        "/",
        "bld_appliances_stock_tm1",
        out_col="bld_appliances_retirement-rate",
        unit="%",
    )
    ...
    dm_appliances.filter(
        {"Variables": ["bld_appliances_stock", "bld_appliances_electricity-demand"]},
        inplace=True,
    )

    # Fill nans for stock and energy demand/unit
    dm_appliances = get_data.appliances_fill_missing_years(
        dm_appliances, dm_households_CH, years_ots, years_fts
    )
    ...
    # n(t) = w(t) - s(t-1) + s(t)
    arr = (
        dm_appliances[:, :, "bld_appliances_waste", :]
    ...
    dm_appliances[:, :, "bld_appliances_new", :] = np.maximum(
        dm_appliances[:, :, "bld_appliances_new", :], 0
    )
    ...
    dm_appliances.change_unit(
        "bld_appliances_stock", factor=1, old_unit="number", new_unit="unit/household"
    )
    ...
    dm_appliances.array[
        :, idx[2023] :, idx["bld_appliances_electricity-demand"], idx["oven-and-stove"]
    ] = np.nan
    ...
    dm_other_elec.append(
        dm_pop.filter(
            {"Country": ["Switzerland"], "Years": years_ots + years_fts_orig}
        ),
        dim="Variables",
    )
    dm_other_elec.operation(
        "bld_energy-demand-total_other_electricity",
        "/",
        "lfs_population_total",
        out_col="bld_energy-demand_other_electricity",
        unit="TWh/cap",
    )
    dm_other_elec.filter(
        {"Variables": ["bld_energy-demand_other_electricity"]}, inplace=True
    )
    ...
    dm_other_elec.change_unit(
        "bld_energy-demand_other", old_unit="TWh/cap", new_unit="kWh/cap", factor=1e9
    )
    ...
    DM = {
        "fxa": {
            ...
            "other-electricity-demand": dm_other_elec.filter(
                {"Years": years_ots + years_fts_orig}
            ),
        }
    }
