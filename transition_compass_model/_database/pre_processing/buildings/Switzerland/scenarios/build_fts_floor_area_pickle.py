...
def run(DM_buildings, years_ots, years_fts):
    ...
    current_floor_area = (
        dm_pop[:, years_ots[-1], var_pop] * dm_floor_ots[:, years_ots[-1], var_floor]
    )
    ...
    for lev in [2, 3, 4]:
        ...
        dm_floor_area_ots[:, years_fts[-1], var_floor] = (
            decrease_lev * dm_floor_ots[:, years_ots[-1], var_floor]
        )
        dm_floor_area_ots.fill_nans("Years")
        DM_buildings["fts"]["floor-intensity"][lev] = dm_floor_area_ots.filter(
            {"Years": years_fts}
        )
