...
def run(DM_buildings, years_ots, years_fts):
    ...
    for lev in [2, 3]:
        ...
        dm_heat_cool_ots[:, 2050, "bld_Tint-heating", :, :] = np.maximum(
            min_T, dm_heat_cool_ots[:, years_ots[-1], "bld_Tint-heating", :, :] + deltaT
        )
        dm_heat_cool_ots[:, 2050, "bld_Tint-heating", :, :] = np.minimum(
            dm_heat_cool_ots[:, years_ots[-1], "bld_Tint-heating", :, :],
            dm_heat_cool_ots[:, 2050, "bld_Tint-heating", :, :],
        )
