...
def compute_new_area_transformed(dm_floor_area, years_ots, cat_map_sfh, cat_map_mfh):
    ...
    dm.operation(
        ...
        out_col="bld_floor-area_transformed",
        unit="m2",
    )
    ...
    dm.array[:, :, idx["bld_floor-area_transformed"], ...] = np.maximum(
        dm.array[:, :, idx["bld_floor-area_transformed"], ...], 0
    )
...
def extrapolate_energy_categories_to_missing_years(
    dm_floor_area, cat_map_sfh, cat_map_mfh
):
    ...
    dm.add(
        arr_floor_cap,
        dim="Variables",
        col_label="bld_floor-area_stock-cap",
        unit="m2/cap",
    )
    linear_fitting(dm, dm.col_labels["Years"])
    idx = dm.idx
    dm.array[:, :, idx["bld_floor-area_stock"], ...] = (
        dm.array[:, :, idx["bld_floor-area_stock-cap"], ...]
        * dm_pop.array[..., np.newaxis]
    )
...
def compute_bld_floor_area_stock_tranformed_avg_new_area(
    table_id, file, years_ots, cat_map_sfh, cat_map_mfh
):
    ...
    dm_area_transformed = compute_new_area_transformed(
        dm_floor_area, years_ots, cat_map_sfh, cat_map_mfh
    )

    dm_floor_area.append(dm_num_bld, dim="Variables")
    dm_floor_area.operation(
        "bld_floor-area_stock",
        "/",
        "bld_stock-number-bld",
        out_col="bld_avg-floor-area-new",
        unit="m2/bld",
    )
    ...
    # Add missing years
    dm_energy_cat = extrapolate_energy_categories_to_missing_years(
        dm_floor_area, cat_map_sfh, cat_map_mfh
    )
    ...
    arr = (
        dm_energy_cat.array[:, :, idx_e["bld_floor-area_stock_share"], :, :]
...
def extract_bld_new_buildings_1(table_id, file):
    try:
        ...
        dm_new_area.rename_col(
            ["Suisse", "- Canton de Vaud"], ["Switzerland", "Vaud"], dim="Country"
        )
        dm_new_area.groupby(
            {
                "bld_new-buildings_single-family-households": ["Maisons individuelles"],
...
def extract_bld_new_buildings_2(table_id, file):
    try:
        ...
        dm_new_area.groupby(
            {
                "bld_new-buildings_single-family-households": [
...
def compute_bld_floor_area_new(
    dm_bld_new_buildings_1, dm_bld_new_buildings_2, dm_bld_area_stock, dm_pop
):
    ...
    dm_bld_area_new.add(
        arr_bld_cap, dim="Variables", col_label="bld_new-buildings-cap", unit="bld/cap"
    )
    linear_fitting(
        dm_bld_area_new, years_ots, based_on=create_years_list(1995, 2002, 1)
    )
    dm_bld_area_new.array[:, -1, ...] = np.nan
    linear_fitting(
        dm_bld_area_new, years_ots, based_on=create_years_list(2015, 2022, 1)
    )
    idx = dm_bld_area_new.idx
    dm_bld_area_new.array[:, :, idx["bld_new-buildings"], :] = (
        dm_bld_area_new.array[:, :, idx["bld_new-buildings-cap"], :]
    ...
    dm_bld_area_new.operation(
        "bld_new-buildings",
        "*",
        "bld_avg-floor-area-new",
        out_col="bld_floor-area_new",
        unit="m2",
    )
...
def clean_floor_area_stock(dm, cat_map):
    ...
    for cat, period in cat_map.items():
        ...
        not_period = list(
            set(range(len(dm.col_labels["Years"]))) - set(idx_period[:-1])
        )
        not_period.sort()
        if not_period is not None:
            ...
            for i in range(2):
                arr = dm.array[
                    :, not_period, idx["bld_floor-area_stock"], :, idx[cat]
                ].reshape((nb_y, nb_c, len_cat1))
                data_smooth = moving_average(arr, window_size, axis=0)
                dm.array[
                    :, not_period[1:-1], idx["bld_floor-area_stock"], :, idx[cat]
                ] = data_smooth
    return
...
def clean_floor_area_new(dm, cat_map):
    ...
    for cat, period in cat_map.items():
        ...
        period_list = list(range(start_yr, end_yr))
        idx_not_period = [
            idx[yr] for yr in dm.col_labels["Years"] if yr not in period_list
        ]
...
def compute_bld_demolition_rate(dm_in, cat_map):
    ...
    # r(t) = R s(t-1)
    dm.append(
        dm_in.filter({"Variables": ["bld_floor-area_transformed"]}, inplace=False),
        dim="Variables",
    )
    ...
    dm.lag_variable("bld_floor-area_stock", shift=1, subfix="_tm1")
    arr_waste = (
        dm.array[:, :, idx["bld_floor-area_stock_tm1"], ...]
    ...
    dm.operation(
        ...
        "/",
        "bld_floor-area_stock_tm1",
        out_col="bld_demolition-rate_tm1",
        unit="%",
    )
    ...
    dm_demolition_rate = dm.filter(
        {"Variables": ["bld_demolition-rate"]}, inplace=False
    )
    ...
    window_size = 3  # Change window size to control the smoothing effect
    data_smooth = moving_average(
        dm_demolition_rate.array,
        window_size,
        axis=dm_demolition_rate.dim_labels.index("Years"),
    )
    ...
    for cat, period in cat_map.items():
        ...
        idx_period = [idx[yr] for yr in period_list]
        dm_demolition_rate.array[
            :, idx_period, idx["bld_demolition-rate"], :, idx[cat]
        ] = 0
    ...
    dm.filter(
        {"Variables": ["bld_floor-area_stock", "bld_floor-area_stock_tm1"]},
        inplace=True,
    )
    ...
    dm_out.rename_col(
        "bld_demolition-rate_tm1_tp1", "bld_demolition-rate", dim="Variables"
    )
...
def extract_number_of_buildings(table_id, file):
    try:
        ...
        dm_nb_bld.groupby(
            {"bld_nb-bld": ".*"}, regex=True, inplace=True, dim="Variables"
        )
        dm_nb_bld.groupby(
            {
                "single-family-households": ["Maisons individuelles"],
...
def compute_renovated_buildings(dm_bld, nb_buildings_renovated, VD_share, share_by_bld):
    dm_bld.add(
        ...
        dim="Variables",
        dummy=True,
        col_label="bld_nb-bld-renovated",
        unit="number",
    )
    idx = dm_bld.idx
...
def extract_renovation_1990_2000(table_id, file, share_thermal):
    try:
        ...
        for e in structure["Epoque de construction"]:
            ...
            unit = ["number"]
            dm_renovation_e = get_data_api_CH(
                table_id,
                mode="extract",
                filter=filter,
                mapping_dims=mapping,
            ...
            if dm_renovation is not None:
                ...
                dm_renovation = dm_renovation_e.groupby(
    ...
    map_to_years = {
        "Rénovés dans les 4 dernières années": [2000, 4],
        "Rénovés dans les 5 à 9 années précédentes": [1990, 5],
    }
    # Add missing years
    dm_tot_bld = dm_renovation.group_all("Categories2", inplace=False)
    dm_tot_bld.add(
        np.nan, dummy=True, dim="Years", col_label=create_years_list(1991, 1999, 1)
    )
    ...
    for cat, values in map_to_years.items():
        ...
        interval = values[1]
        dm_renovation.array[
            :, idx[yr], idx["bld_nb-bld-renovated"], :, idx["total"]
        ] = (
            dm_renovation.array[:, idx[yr], idx["bld_nb-bld-renovated"], :, idx[cat]]
            / interval
        )
...
def extract_bld_floor_area_stock_envelope_cat(
    table_id, file, dm_pop, years_ots, cat_dict
):
...
def compute_new_area_by_energy_cat(dm_bld_area_new, dm_energy_cat, cat_map):
    ...
    dm_energy_cat.add(
        0, dummy=True, dim="Variables", col_label="bld_floor-area_new", unit="m2"
    )
...
def extract_renovation_redistribuition(ren_map_in, ren_map_out, years_ots):
    ...
    dm.array = np.nan * np.ones(
        (
            len(dm.col_labels["Country"]),
    ...
    for year_period, map_values in ren_map_in.items():
        ...
        for key, val in map_values.items():
            dm.array[
                :, idx_year_period, idx["bld_renovation-redistribution-in"], idx[key]
            ] = val
...
def adjust_based_on_renovation(dm_in, dm_rr, dm_renov_distr):
    ...
    for ti in reversed(dm.col_labels["Years"][1:]):
        ...
        stock_tm1 = tmp_t / (
            1
            + dm_rr.array[:, idx_r[ti], idx_r["bld_renovation-rate"], :, np.newaxis]
            * (
                -dm_renov_distr.array[
                    :,
                    idx_d[ti],
                    idx_d["bld_renovation-redistribution-out"],
                    np.newaxis,
                    :,
                ]
                + dm_renov_distr.array[
                    :,
                    idx_d[ti],
                    idx_d["bld_renovation-redistribution-in"],
                    np.newaxis,
                    :,
                ]
            )
...
def compute_building_mix(dm_all):
    ...
    dm_building_mix.rename_col(
        ...
        ["bld_building-mix_stock", "bld_building-mix_new"],
        dim="Variables",
    )
...
def extract_lfs_household_size(years_ots, table_id, file):
    try:
        ...
        arr_avg_size = np.nansum(arr_weighted_size, axis=-1, keepdims=True) / np.nansum(
            dm_household.array, axis=-1, keepdims=True
        )
        # Create new datamatrix
        dm_household_size = DataMatrix.based_on(
            arr_avg_size,
            dm_household,
            change={"Variables": ["lfs_household-size"]},
...
def extract_heating_technologies_old(table_id, file, cat_sfh, cat_mfh):
    ...
    try:
        ...
        for t in structure["Type de chauffage"]:
            for a in structure["Agent énergétique pour l'eau chaude"]:
                ...
                dm_heating_old_t = get_data_api_CH(
                    table_id,
                    mode="extract",
                    filter=filter,
                    mapping_dims=mapping_dim,
    ...
    dm_heating_old.groupby(
        {"1991-2000": ["1991-1995", "1996-2000"]}, dim="Variables", inplace=True
    )

    # Group by construction period
    dm_heating_sfh = dm_heating_old.filter(
        {"Categories1": ["single-family-households"]}, inplace=False
    )
    dm_heating_sfh.groupby(
        {"bld_heating-mix_F": cat_sfh["F"]}, dim="Variables", inplace=True
    )
    dm_heating_sfh.groupby(
        {"bld_heating-mix_E": cat_sfh["E"]}, dim="Variables", inplace=True
    )
    dm_heating_sfh.groupby(
        {"bld_heating-mix_D": cat_sfh["D"]}, dim="Variables", inplace=True
    )

    dm_heating_mfh = dm_heating_old.filter(
        {"Categories1": ["multi-family-households"]}, inplace=False
    )
    dm_heating_mfh.groupby(
        {"bld_heating-mix_F": cat_mfh["F"]}, dim="Variables", inplace=True
    )
    dm_heating_mfh.groupby(
        {"bld_heating-mix_E": cat_mfh["E"]}, dim="Variables", inplace=True
    )
    dm_heating_mfh.groupby(
        {"bld_heating-mix_D": cat_mfh["D"]}, dim="Variables", inplace=True
    )
    ...
    dm_heating_old.groupby(
        {"other-tech": ["Autre agent énergétique (chauf.)", "Sans chauffage"]},
        dim="Categories2",
        inplace=True,
    )
    dm_heating_old.rename_col(
        [
            "Mazout (chauf.)",
            "Bois (chauf.)",
            "Pompe à chaleur (chauf.)",
            "Electricité (chauf.)",
            "Gaz (chauf.)",
            "Chaleur à distance (chauf.)",
            "Charbon (chauf.)",
            "Capteur solaire (chauf.)",
        ],
        [
            "heating-oil",
            "wood",
...
def harmonise_stock_new_renovated_transformed(dm, dm_rr, dm_renov_distr, cat_map):
    ...
    ren_t = np.nansum(
        dm.array[:, :, idx["bld_floor-area_stock_tm1"], :, :]
        * dm_rr.array[:, :, idx_r["bld_renovation-rate"], :, np.newaxis],
        axis=-1,
        keepdims=True,
    ) * (
        -dm_renov_distr.array[
    ...
    waste = (
        ...
        - dm.array[:, :, idx["bld_floor-area_stock"], ...]
        + dm.array[:, :, idx["bld_floor-area_new"], ...]
        + dm.array[:, :, idx["bld_floor-area_renovated"], ...]
        + dm.array[:, :, idx["bld_floor-area_transformed"], ...]
    )
    waste = np.maximum(
        waste, dm.array[:, :, idx["bld_floor-area_stock_tm1"], ...] * 0.001
    )
    ...
    dm.add(
        transf, dim="Variables", unit="m2", col_label="bld_floor-area_transformed_new"
    )

    # dm.operation('bld_floor-area_transformed_new', '+', 'bld_floor-area_new', out_col='bld_floor-area_new2', unit='m2')
    dm.array[:, :, idx["bld_floor-area_new"], ...] = (
        transf + dm.array[:, :, idx["bld_floor-area_new"], ...]
    )
...
def compute_floor_area_stock_v2(table_id, file, dm_pop, cat_map_sfh, cat_map_mfh):
    ...
    idx_p = dm_pop.idx
    arr = (
        dm_stock_tot.array[:, :, idx["bld_floor-area_stock"]]
        / dm_pop.array[:, :, idx_p["lfs_population_total"], np.newaxis]
    )
    dm_stock_tot.add(
        arr, dim="Variables", col_label="bld_floor-area_stock-cap", unit="m2/cap"
    )
    linear_fitting(dm_stock_tot, years_missing)
    dm_stock_tot.array[:, :, idx["bld_floor-area_stock"], :] = (
        dm_stock_tot.array[:, :, idx["bld_floor-area_stock-cap"], :]
        * dm_pop.array[:, :, idx_p["lfs_population_total"], np.newaxis]
    )
...
def compute_floor_area_new_cat(dm_new_tot, cat_map):
    ...
    dm_new_cat = DataMatrix.based_on(
        arr, dm_new_tot, change={"Categories2": env_cat}, units=dm_new_tot.units
    )
...
def compute_waste(dm_stock_tot, dm_new_tot):
    ...
    dm.operation(
        ...
        "-",
        "bld_floor-area_stock",
        out_col="bld_floor-area_delta",
        unit="m2",
    )
    dm.operation(
        "bld_floor-area_delta",
        "+",
        "bld_floor-area_new",
        out_col="bld_floor-area_waste",
        unit="m2",
    ...
    dm.array[:, :, idx["bld_floor-area_waste"], ...] = np.maximum(
        min_waste, dm.array[:, :, idx["bld_floor-area_waste"], ...]
    )
    # Fix 1990 value
...
def compute_floor_area_renovated(dm_stock_tot, dm_renovation, dm_renov_distr):
    ...
    dm.operation(
        "bld_renovation-rate_tm1",
        "*",
        "bld_floor-area_stock_tm1",
        out_col="bld_floor-area_renovated",
        unit="m2",
    )
    ...
    dm_renovated_cat = DataMatrix.based_on(
        arr[:, :, np.newaxis, ...],
        dm,
        change={
            "Variables": ["bld_floor-area_renovated"],
...
def compute_floor_area_waste_cat(dm_waste_tot):
    ...
    dm.rename_col(
        "single-family-households", "single-family-households_F", dim="Categories1"
    )
    dm.rename_col(
        "multi-family-households", "multi-family-households_F", dim="Categories1"
    )
    dm.deepen()
...
def fix_negative_stock(dm_all_cat, dm_stock_tot):
    ...
    dm_tmp.array[:, :, 0, :, :] = (
        dm_tmp.array[:, :, 0, :, :] + shift[:, np.newaxis, :, :]
    )
    dm_stock_wo_F = dm_tmp.group_all(dim="Categories2", inplace=False)
    dm_stock_wo_F.rename_col(
        "bld_floor-area_stock", "bld_floor-area_stock_woF", dim="Variables"
    )
    dm_stock_tot.append(dm_stock_wo_F, dim="Variables")
    dm_stock_tot.operation(
        "bld_floor-area_stock",
        "-",
        "bld_floor-area_stock_woF",
        out_col="bld_floor-area_stock_F",
        unit="m2",
    )
...
def prepare_heating_mix_by_archetype():
    ...
    dict_archetype = {
        "single-family-households": dict_sfh,
        "multi-family-households": dict_mfh,
    }
    ...
    cdm = ConstantDataMatrix(
        col_labels={
            "Variables": ["bld_heating-mix-archetype"],
...
def compute_heating_mix_by_category(dm_heating_tech, cdm_heating_archetypes, dm_all):
    ...
    idx_a = dm_stock.idx
    arr_tmp = (
        cdm_heating_archetypes.array[
            np.newaxis, np.newaxis, idx_h["bld_heating-mix-archetype"], :, :, :
        ]
        * dm_stock.array[:, :, idx_a["bld_floor-area_stock"], :, :, np.newaxis]
    )
    arr = arr_tmp / np.nansum(arr_tmp, axis=-2, keepdims=True)
    dm_heating_mix = DataMatrix.based_on(
        arr[:, :, np.newaxis, ...],
        dm_stock,
        units={"bld_heating-mix-tmp": "%"},
        change={
            "Variables": ["bld_heating-mix-tmp"],
    ...
    fuel_missing = list(
        set(dm_heating_tech.col_labels["Categories2"])
        - set(dm_heating_mix.col_labels["Categories3"])
    )
    ...
    arr = (
        dm_heating_mix.array[:, :, idx["bld_heating-mix-tmp"], :, :, :]
...
def clean_heating_cat(dm_heating_cat, envelope_cat_new):
    ...
    for cat, period in envelope_cat_new.items():
        ...
        dm_heating_cat.array[:, idx_yrs, idx["bld_heating-mix"], :, idx[cat], :] = (
            np.nan
        )
...
def compute_renovation_loi_energie(
    ...
    dm_bld_adj = dm_stock_cat.filter(
        {"Variables": ["bld_floor-area_stock"]}, inplace=False
    )
...
def compute_heating_mix_F_E_D_categories(dm_heating_tech, dm_heating_tech_old):
    ...
    dm_heating_tech_old.array[
        :, idx[1990], :, idx["D"], idx["multi-family-households"], :
    ] = np.nan
...
def compute_heating_mix_C_B_categories(dm_heating_tech, cdm_heating_archetypes):
    ...
    extra_cat = list(
        set(dm_heating_tech_new.col_labels["Categories3"])
        - set(cdm_heating_archetypes.col_labels["Categories3"])
    )
    dm_heating_tech_new.filter(
        {"Categories3": cdm_heating_archetypes.col_labels["Categories3"]}, inplace=True
    )
    ...
    for cat in dm_heating_tech_new.col_labels["Categories1"]:
        ...
        dm_heating_tech_new.array[
            :, idx[start_year], 0, idx[cat], idx["multi-family-households"], :
        ] = cdm_heating_archetypes.array[
            np.newaxis, 0, idx["multi-family-households"], idx_c[cat], :
        ]
        dm_heating_tech_new.array[
            :, idx[start_year], :, idx[cat], idx["single-family-households"], :
        ] = cdm_heating_archetypes.array[
            np.newaxis, 0, idx["single-family-households"], idx_c[cat], :
        ]
...
def compute_heating_efficiency_by_archetype(
    ...
    dm_eff_cat = DataMatrix.based_on(
        arr_w_cat,
        format=dm_heating_eff,
        change={"Categories2": categories},
    ...
    for cat in categories:
        if cat not in envelope_cat_new.keys():
            dm_eff_cat.array[:, :, :, :, idx[cat]] = dm_eff_cat.array[
                :, 0, np.newaxis, :, :, idx[cat]
            ]
        else:
            ...
            for yr in range(start_yr + 1, end_yr + 1):
                ...
                s_tm1 = dm_stock.array[
                    :,
                    idx_s[yr - 1],
                    idx_s["bld_floor-area_stock"],
                    np.newaxis,
                    idx_s[cat],
                ]
                new_t = dm_stock.array[
                    :, idx_s[yr], idx_s["bld_floor-area_new"], np.newaxis, idx_s[cat]
                ]
            ...
            if (end_yr + 1) in dm_eff_cat.col_labels["Years"]:
                dm_eff_cat.array[:, idx[end_yr] :, 0, :, idx[cat]] = dm_eff_cat.array[
                    :, idx[end_yr], np.newaxis, 0, :, idx[cat]
                ]
    ...
    dm_eff_cat.array[:, :, :, idx["heating-oil"], :] = dm_eff_cat_raw.array[
        :, :, :, idx["heating-oil"], :
    ]

    dm_eff_cat_raw.rename_col(
        "bld_heating-efficiency", "bld_heating-efficiency-JRC", dim="Variables"
    )
...
def extract_heating_efficiency(file, sheet_name, years_ots):
    ...
    df = df[0:13].copy()
    names_map = {
        "Ratio of energy service to energy consumption": "remove",
        "Space heating": "other-tech",
        "Solids": "coal",
        ...
        "Advanced electric heating": "heat-pump",
        "Conventional electric heating": "electricity",
        "Electricity in circulation": "remove",
    }
    dm_heating_eff = df_excel_to_dm(
        df, names_map, var_name="bld_heating-efficiency", unit="%", num_cat=1
    )
    ...
    years_missing_e = [
        yr for yr in years_missing if yr > dm_heating_eff.col_labels["Years"][0]
    ]
    years_missing_i = [
        yr for yr in years_missing if yr < dm_heating_eff.col_labels["Years"][0]
    ]
    linear_fitting(
        dm_heating_eff,
        years_missing_e,
        based_on=list(range(2015, dm_heating_eff.col_labels["Years"][-1] + 1)),
    )
...
dm_lfs_household_size = extract_lfs_household_size(
    years_ots, table_id="px-x-0102020000_402", file=filename
)
...
#                                                                construction_period_envelope_cat_mfh)
dm_stock_tot, dm_stock_cat, dm_avg_floor_area = compute_floor_area_stock_v2(
    table_id,
    file,
    dm_pop=dm_pop,
...
dm_new_tot_raw = compute_bld_floor_area_new(
    dm_bld_new_buildings_1, dm_bld_new_buildings_2, dm_avg_floor_area, dm_pop
)
...
print(
    "Maybe you should considered the buildings undergoing systemic renovation as well"
)
...
for yr in nb_buildings_isolated.keys():
    nb_buildings_renovated[yr] = (
        nb_buildings_isolated[yr] + nb_buildings_systemic_renovation[yr]
    )
...
share_by_bld = {
    ...
    "multi-family-households": 0.35,
    "other": 0.1,
}
dm_renovation = compute_renovated_buildings(
    dm_bld, nb_buildings_renovated, VD_share, share_by_bld
)
...
# s_{c,t}(t-1) &= s_{c,t}(t) - n_{c,t}(t) - r_{c,t}(t) + w_{c,t}(t)
dm_all = compute_stock_area_by_cat(
    dm_stock_cat, dm_new_cat, dm_renov_cat, dm_waste_cat, dm_stock_tot
)
...
envelope_cat_u_value = {
    "single-family-households": {"F": 0.82, "E": 0.69, "D": 0.53, "C": 0.41, "B": 0.25},
    "multi-family-households": {"F": 0.93, "E": 0.70, "D": 0.63, "C": 0.48, "B": 0.29},
}
...
dm_heating_tech = extract_heating_technologies(
    table_id,
    file,
    construction_period_envelope_cat_sfh,
    construction_period_envelope_cat_mfh,
...
dm_heating_tech_old = extract_heating_technologies_old(
    table_id,
    file,
    construction_period_envelope_cat_sfh,
    construction_period_envelope_cat_mfh,
...
dm_heating_tech_old = compute_heating_mix_F_E_D_categories(
    dm_heating_tech, dm_heating_tech_old
)
# Reconstruct heating-mix for new categories using archetypes for B and C
# !FIXME Vaud behave differently than Switzerland
dm_heating_tech_new = compute_heating_mix_C_B_categories(
    dm_heating_tech, cdm_heating_archetypes
)
...
if alternative_computation:
    ...
    dm_heating_cat = compute_heating_mix_by_category(
        dm_heating_tech, cdm_heating_archetypes, dm_all
    )
    ...
    for cat in dm_heating_cat.col_labels["Categories2"]:
        dm_heating_cat.filter(
            {"Categories1": ["multi-family-households"], "Categories2": [cat]}
        ).flatten().flatten().datamatrix_plot(
...
if check:
    arr_tmp = (
        dm_heating_cat.array[:, :, 0, ...]
        * dm_stock_cat.array[:, :, -1, :, :, np.newaxis]
    )
    dm_heating_cat.add(
        arr_tmp, dim="Variables", unit="m2", col_label="bld_heating-mix-area"
    )
...
dm_heating_eff_cat = compute_heating_efficiency_by_archetype(
    dm_heating_eff,
    dm_stock_cat,
    envelope_cat_new,
...
dm_pop_ots = DM_lifestyles["ots"]["pop"]["lfs_population_"].filter(
    {"Country": ["Switzerland", "Vaud"]}
)
...
dm_clm_ots = DM_clm["ots"]["temp"]["bld_climate-impact-space"].filter(
    {"Country": ["Switzerland", "Vaud"]}
)
dm_clm_fts = DM_clm["fts"]["temp"]["bld_climate-impact-space"][1].filter(
    {"Country": ["Switzerland", "Vaud"]}
)
...
arr_shape = (
    len(dm_Tint_heat.col_labels["Country"]),
    len(years_ots + years_fts),
    len(dm_Tint_heat.col_labels["Variables"]),
...
DM_buildings["ots"]["building-renovation-rate"]["bld_building-mix"] = dm_tmp.filter(
    {"Years": years_ots}
)
# FTS
...
dm_heating_cat_fts_2.array[
    idx["Vaud"], :, idx["bld_heating-mix"], :, idx["B"], idx_fossil
] = 0
dm_heating_cat_fts_2.array[
    idx["Vaud"], 1 : idx[2045], idx["bld_heating-mix"], :, :, idx_fossil
] = np.nan
dm_heating_cat_fts_2.array[
