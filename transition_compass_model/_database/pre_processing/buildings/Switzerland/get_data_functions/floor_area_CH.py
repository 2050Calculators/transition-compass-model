...
def extract_stock_floor_area(table_id, file):
    ...
    try:
        ...
        cantons_list = [
            var
            for var in structure["Canton (-) / District (>>) / Commune (......)"]
            if "- " in var
        ]
        ...
        for cntr in cantons_list:
            ...
            # Get api data
            dm_floor_area_cntr = get_data_api_CH(
                table_id,
                mode="extract",
                filter=filter,
...
def compute_floor_area_stock_v2(
    table_id, file, dm_pop, cat_map_sfh, cat_map_mfh, years_ots
):
    # Computes:
    ...
    dm_stock_tot = extrapolate_missing_years_based_on_per_capita(
        dm_stock_tot, dm_pop, years_ots, var_name="bld_floor-area_stock"
    )
...
def extract_bld_new_buildings_1(table_id, file):
    try:
        ...
        cantons_list = [
            var
            for var in structure["Grande région (<<) / Canton (-) / Commune (......)"]
            if "Canton " in var
        ]
        cantons_list.append("Suisse")
        # Extract buildings floor area
...
def extract_bld_new_buildings_2(table_id, file):
    try:
        ...
        cantons_list = [
            var
            for var in structure["Canton (-) / Commune (......)"]
            if "Canton " in var
        ]
        cantons_list.append("Suisse")
        # Extract buildings floor area
...
def extract_nb_of_apartments_per_building_type(table_id, file, cantons_fr, cantons_en):
    try:
        ...
        cantons_list = [
            c
            for c in structure["Canton (-) / District (>>) / Commune (......)"]
            if "- " in c
        ]
        mapping_dim = {
            "Country": "Canton (-) / District (>>) / Commune (......)",
...
def extract_EP2050_floor_area_by_type(file_url, zip_name, file_pickle):
    ...
    try:
        ...
        full_name = ["bld_floor-area_stock_" + var + "[Mm2]" for var in df["Type"]]
        df["Type"] = full_name
        ...
        dm.groupby(
            {
                "single-family-households": [
                    "EZFH",
                    "EZFH - Zweit- und Ferienwohnungen",
                ],
                "multi-family-households": [
                    "MFH",
                    "MFH - Zweit- und Ferienwohnungen",
                    "SoGWo",
                ],
            },
            dim="Categories1",
