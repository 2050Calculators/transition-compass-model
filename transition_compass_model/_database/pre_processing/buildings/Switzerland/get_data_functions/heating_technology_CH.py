            ...
            for t in structure["Type de chauffage"]:
                for a in structure["Agent énergétique pour l'eau chaude"]:
                    ...
                    if dm_heating_old_cntr is None:
                        ...
                        dm_heating_old_cntr.array = (
                            dm_heating_old_t.array + dm_heating_old_cntr.array
                        )
...
def extract_heating_technologies(table_id, file, cat_sfh, cat_mfh):
    ...
    def extract_cntr_heating(cntr_list):
        ...
        # Get api data
        dm_heating_cntr = get_data_api_CH(
            table_id,
            mode="extract",
            filter=filter,
            mapping_dims=mapping_dim,
    ...
    try:
        ...
        cantons_list = list(
            set(structure["Canton"]) - {"Suisse", "Neuchâtel", "St. Gallen"}
        )
...
def compute_heating_mix_F_E_D_categories(
...
def extract_heating_technologies_EP2050(file_url, zip_name, file_pickle):
    ...
    try:
        ...
        def format_df_tech_EP250(df, start_row, var_name, unit):
            ...
            full_name = [
                var_name + "_" + cat + "[" + unit + "]" for cat in df["Energy_source"]
            ]
        ...
        file_industry = (
            extract_dir
            + "/EP2050+_Szenarienergebnisse_Details_Nachfragesektoren/EP2050+_Detailergebnisse 2020-2060_Private Haushalte_alle Szenarien_2022-05-17.xlsx"
        )
        ...
        dm_single = format_df_tech_EP250(
            df_single,
            col_row,
            var_name="bld_heating-mix_single-family-households",
            unit="%",
        )
        dm_single.deepen(based_on="Variables")
        dm_single.switch_categories_order()
        dm_single.rename_col(
            [
                "Fernwärme/Nahwärme",
                "Gas",
                "Heizöl",
        ...
        dm_multi = format_df_tech_EP250(
            df_multi,
            col_row,
            var_name="bld_heating-mix_multi-family-households",
            unit="%",
        )
