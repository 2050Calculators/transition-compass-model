...
def extract_hotwater_technologies(table_id, file):
    ...
    try:
        ...
        dm_hw.groupby(
            {
                "bld_hot-water_tech_single-family-house": ["Maisons individuelles"],
...
def extract_hotwater_technologies_old(table_id, file):
    ...
    try:
        ...
        for t in structure["Type de chauffage"]:
            for a in structure["Agent énergétique pour le chauffage"]:
                ...
                if dm_heating_old is None:
                    ...
                    assert (
                        dm_heating_old.col_labels["Country"]
                        == dm_heating_old_t.col_labels["Country"]
                    )
                    assert (
                        dm_heating_old.col_labels["Categories1"]
                        == dm_heating_old_t.col_labels["Categories1"]
                    )
        ...
        dm_heating_old.groupby(
            {
                "bld_hot-water_tech_single-family-households": [
                    "Maisons individuelles"
                ],
                "bld_hot-water_tech_multi-family-households": [
                    "Maisons à plusieurs logements",
                    "Bâtiments d'habitation avec usage annexe",
                    "Bâtiments partiellement à usage d'habitation",
                ],
    ...
    dm_heating_old.groupby(
        {
            "other": [
                "Autre agent énergétique (eau ch.)",
    ...
    dm_heating_old.rename_col(
        [
            "Mazout (eau ch.)",
            "Bois (eau ch.)",
            "Pompe à chaleur (eau ch.)",
            "Electricité (eau ch.)",
            "Gaz (eau ch.)",
            "Chaleur à distance (eau ch.)",
            "Charbon (eau ch.)",
            "Capteur solaire (eau ch.)",
        ],
        [
            "heating-oil",
            "wood",
    ...
    dm_heating_old.rename_col(
        "single-family-households", "single-family-house", dim="Categories1"
    )
    dm_heating_old.rename_col(
        "multi-family-households", "multi-family-house", dim="Categories1"
    )
...
def extract_EP2050_hot_water_energy_consumption(file_raw, file_pickle):
    ...
    try:
        ...
        full_name = [
            "bld_hot-water_energy-consumption_" + var + "[PJ]"
            for var in df["Energy_source"]
        ]
...
def extract_heating_efficiencies_EP2050(file_url, zip_name, file_pickle):
    ...
    try:
        ...
        full_name = [
            "bld_heating_efficiency_" + var + "[%]" for var in df["Energy_source"]
        ]
        df["Energy_source"] = full_name
...
def extract_hot_water_efficiency_JRC(file, sheet_name, years_ots):
    ...
    dm_heating_eff = df_excel_to_dm(
        df, names_map, var_name="bld_hot-water-efficiency", unit="%", num_cat=1
    )
