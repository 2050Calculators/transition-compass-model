from .energy.energyscopepyomo.ses_pyomo import (
    load_data,
    make_highs,
    attach,
    solve,
...
def capture_model_state(model, filename):
    ...
    for con in model.component_objects(pyo.Constraint):
        con_data = {
            ...
            "active_count": (
                sum(1 for idx in con if con[idx].active)
                if con.is_indexed()
                else (1 if con.active else 0)
            ),
        }
    ...
    for var in model.component_objects(pyo.Var):
        var_data = {"count": len(var) if var.is_indexed() else 1}
