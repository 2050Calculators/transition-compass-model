...
def database_from_csv_to_datamatrix():
    ...
    df = read_database_fxa(
        file, filter_dict={"eucalc-name": "agr_climate-smart-crop_ef_agroforestry"}
    )
    ...
    df = read_database_fxa(
        file, filter_dict={"eucalc-name": "agr_climate-smart-livestock_ef_agroforestry"}
    )
    dm_livestock_ef_agroforestry = DataMatrix.create_from_df(df, num_cat=1)
    dict_fxa["agr_climate-smart-livestock_ef_agroforestry"] = (
        dm_livestock_ef_agroforestry
    )
    # AGROFORESTRY Forestry - natural losses & others
    df = read_database_fxa(
        file, filter_dict={"eucalc-name": "agr_climate-smart-forestry_def"}
    )
    ...
    cdm_burnt = cdm_const.filter(
        {
            "Variables": [
                "cp_fst_ef_emissions-N2O_burnt",
                "cp_fst_ef_emissions-CH4_burnt",
                "cp_fst_ef_emissions-CO2_burnt",
            ],
            "units": ["t/m3"],
        }
    )
    ...
    # Land matrix OTS
    DM_landuse["ots"]["land-man"]["agr_land-man_matrix"].rename_col_regex(
        str1="agr_land-man_matrix", str2="agr_matrix", dim="Variables"
    )
    ...
    for i in DM_landuse["fts"]["land-man"]["agr_land-man_matrix"].keys():
        # Renaming
        DM_landuse["fts"]["land-man"]["agr_land-man_matrix"][i].rename_col_regex(
            str1="agr_land-man_matrix", str2="agr_matrix", dim="Variables"
        )
        # Deepening
        DM_landuse["fts"]["land-man"]["agr_land-man_matrix"][i].deepen(
            based_on="Variables"
        )
    ...
    # Land c-stock
    DM_landuse["fxa"]["land-man_ef"].rename_col_regex(
        str1="c-stock_", str2="", dim="Variables"
    )
    DM_landuse["fxa"]["land-man_ef"].rename_col_regex(
        str1="ef_", str2="ef_c-stock_", dim="Variables"
    )
    DM_landuse["fxa"]["land-man_ef"].rename_col_regex(
        str1="soil_", str2="soil-", dim="Variables"
    )
    DM_landuse["fxa"]["land-man_ef"].rename_col_regex(
        str1="biomass_", str2="biomass-", dim="Variables"
    )
    ...
    f = os.path.join(
        current_file_directory, "../_database/data/datamatrix/landuse.pickle"
    )
...
def read_data(data_file, lever_setting):
    ...
    dm_agroforestry_liv = DM_landuse["fxa"][
        "agr_climate-smart-livestock_ef_agroforestry"
    ]
...
def wood_workflow(dm_wood, dm_lgn, DM_ind, dm_cal_wood, years_setting):
    ...
    dm_lgn.operation(
        ...
        "*",
        "kcal_to_TWh",
        out_col="agr_bioenergy_biomass-demand_liquid_lgn_TWh",
        unit="TWh",
    )
    ...
    dm_wood_liquid = dm_lgn.filter(
        {
            "Variables": ["agr_bioenergy_biomass-demand_liquid_lgn_TWh"],
            "Categories1": ["lgn-btl-fuelwood-and-res"],
        }
    )
    dm_wood_liquid.rename_col(
        "lgn-btl-fuelwood-and-res", "fuelwood-and-res", dim="Categories1"
    )
    ...
    dm_wood.operation(
        "agr_bioenergy_biomass-demand_solid",
        "+",
        "agr_bioenergy_biomass-demand_liquid_lgn_TWh",
        out_col="lus_fst_demand_rwe_wood-fuel",
        unit="TWh",
    )
    ...
    dm_wood.rename_col(
        "lus_fst_demand_rwe_wood-fuel_fuelwood-and-res",
        "lus_fst_demand_rwe_wood-fuel_temp",
        dim="Variables",
    )

    # Unit conversion : Wood-fuel demand [TWh] => [m3]
    dm_wood.add(
        174420.0, dummy=True, col_label="TWh_to_cubic_m", dim="Variables", unit="m3"
    )
    dm_wood.operation(
        "lus_fst_demand_rwe_wood-fuel_temp",
        "*",
        "TWh_to_cubic_m",
        out_col="lus_fst_demand_rwe_raw_wood-fuel",
        unit="m3",
    )
    ...
    dm_timber.rename_col(
        "ind_material-production_timber",
        "lus_fst_demand_rwe_ind-sawlog_temp",
        dim="Variables",
    )
    dm_timber.add(
        1500.0, dummy=True, col_label="kt_to_cubic_m", dim="Variables", unit="m3"
    )
    dm_timber.operation(
        "lus_fst_demand_rwe_ind-sawlog_temp",
        "*",
        "kt_to_cubic_m",
        out_col="lus_fst_demand_rwe_raw_ind-sawlog",
        unit="m3",
    )
    ...
    dm_woodpulp.rename_col(
        ...
        "lus_fst_demand_rwe_pulp_temp",
        dim="Variables",
    )
    dm_woodpulp.add(
        4500000.0, dummy=True, col_label="Mt_to_cubic_m", dim="Variables", unit="m3"
    )
    dm_woodpulp.operation(
        "lus_fst_demand_rwe_pulp_temp",
        "*",
        "Mt_to_cubic_m",
        out_col="lus_fst_demand_rwe_raw_pulp",
        unit="m3",
    )
    ...
    dm_biomaterial.rename_col(
        "ind_biomaterial_solid-bio",
        "lus_fst_demand_rwe_oth-ind-wood_temp",
        dim="Variables",
    )
    dm_biomaterial.add(
        174420.0, dummy=True, col_label="TWh_to_cubic_m", dim="Variables", unit="m3"
    )
    dm_biomaterial.operation(
        "lus_fst_demand_rwe_oth-ind-wood_temp",
        "*",
        "TWh_to_cubic_m",
        out_col="lus_fst_demand_rwe_raw_oth-ind-wood",
        unit="m3",
    )
    dm_biomaterial = dm_biomaterial.filter(
        {"Variables": ["lus_fst_demand_rwe_raw_oth-ind-wood"]}
    )
    ...
    dm_cal_rates_wood = calibration_rates(
        dm_wood,
        dm_cal_wood,
    ...
    dm_wood.operation(
        "lus_fst_demand_rwe_raw",
        "*",
        "cal_rate",
        dim="Variables",
        out_col="lus_fst_demand_rwe",
        unit="m3",
    )
    df_cal_rates_wood = dm_to_database(
        dm_cal_rates_wood, "none", "agriculture", level=0
    )
...
def land_allocation_workflow(DM_land_use, dm_land_use):
    ...
    dm_land_demand.groupby(
        {"total-land": ".*"}, dim="Categories1", regex=True, inplace=True
    )
    ...
    DM_land_use["land_total"].operation(
        "lus_land_total-area",
        "-",
        "lus_demand_total-land",
        out_col="lus_land_dynamics",
        unit="ha",
    )
    ...
    DM_land_use["land_total"].add(
        array_available_land,
        dim="Variables",
        col_label="lus_dyn_available-land",
        unit="ha",
    )
    ...
    DM_land_use["land_total"].add(
        array_deforestation,
        dim="Variables",
        col_label="lus_dyn_deforestation",
        unit="ha",
    )
    ...
    array_temp = (
        DM_land_use["land_man_dyn"].array[:, :, idx_dyn["agr_land-man_dyn"], :]
        * DM_land_use["land_total"].array[
            :, :, idx_land["lus_dyn_available-land"], np.newaxis
        ]
    )
    DM_land_use["land_man_dyn"].add(
        array_temp, dim="Variables", col_label="lus_dyn", unit="ha"
    )

    # LAND DYNAMICS ACCOUNTING FOR ALLOCATED AVAILABLE LAND ------------------------------------------------------------

    # Pre processing land use dynamics
    DM_land_use["land_man_dyn"].rename_col(
        "unmanaged", "cropland", dim="Categories1"
    )  # (approximation because unmanaged land has negative impacts)
    DM_land_use["land_man_dyn"].add(
        0.0, dummy=True, col_label="settlement", dim="Categories1", unit="ha"
    )
    DM_land_use["land_man_dyn"].add(
        0.0, dummy=True, col_label="wetland", dim="Categories1", unit="ha"
    )
    DM_land_use["land_man_dyn"].add(
        0.0, dummy=True, col_label="other", dim="Categories1", unit="ha"
    )
    DM_land_use["land_man_dyn"].sort(dim="Categories1")

    # Pre processing deforestation
    dm_deforestation_temp = DM_land_use["land_total"].filter(
        {"Variables": ["lus_dyn_deforestation"]}
    )
    dm_deforestation_temp.rename_col(
        "lus_dyn_deforestation", "lus_dyn_deforestation_forest", dim="Variables"
    )
    dm_deforestation_temp.deepen()
    dm_deforestation_temp.add(
        0.0, dummy=True, col_label="cropland", dim="Categories1", unit="ha"
    )
    dm_deforestation_temp.add(
        0.0, dummy=True, col_label="grassland", dim="Categories1", unit="ha"
    )
    dm_deforestation_temp.add(
        0.0, dummy=True, col_label="settlement", dim="Categories1", unit="ha"
    )
    ...
    DM_land_use["land_man_use"].append(
        DM_land_use["land_man_dyn"].filter({"Variables": ["lus_dyn"]}), dim="Variables"
    )
    ...
    DM_land_use["land_man_use"].operation(
        "agr_land-man_use",
        "+",
        "lus_dyn_deforestation",
        out_col="lus_land_temp",
        unit="ha",
    )
    DM_land_use["land_man_use"].operation(
        "lus_land_temp", "+", "lus_dyn", out_col="lus_land", unit="ha"
    )
...
def land_use_change(dm_need, dm_excess):
    ...
    dm_land_use_change = DataMatrix.based_on(
        C,
        dm_excess,
        change={
            "Variables": ["land_use_change_without_rem"],
...
def land_use_change_old(dm_need, dm_excess, dm_matrix):
    ...
    dm_land_use_change = DataMatrix.based_on(
        arr_to_solve,
        dm_matrix,
        change={"Variables": ["land_use_change_without_rem"]},
...
def land_matrix_workflow(DM_land_use, years_setting):
    ...
    DM_land_use["land_man_gap"].add(
        array_temp, dim="Variables", col_label="lus_land_initial-area", unit="ha"
    )
    ...
    DM_land_use["land_man_gap"].operation(
        "lus_land_initial-area",
        "+",
        "agr_land-man_gap",
        out_col="lus_land_initial-area_unfccc",
        unit="ha",
    )
    ...
    DM_land_use["land_man_gap"].operation(
        "lus_land",
        "-",
        "lus_land_initial-area_unfccc",
        out_col="lus_land_diff",
        unit="ha",
    )

    # Sum (Land difference [ha])
    dm_land_total = DM_land_use["land_man_gap"].groupby(
    ...
    dm_temp.operation(
        "forest", "-", "total", dim="Categories1", out_col="forest_new", unit="ha"
    )
    ...
    dm_temp = DM_land_use["land_man_gap"].filter(
        {"Variables": ["lus_land_diff_adjusted"]}
    )
    array_temp = dm_temp.array[:, :, :, :]
    array_land_to_crop = np.maximum(array_temp, 0)
    DM_land_use["land_man_gap"].add(
        array_land_to_crop, dim="Variables", col_label="lus_land_to-crop", unit="ha"
    )
    ...
    DM_land_use["land_man_gap"].add(
        array_land_excess, dim="Variables", col_label="lus_land_excess", unit="ha"
    )
    ...
    DM_land_use["land_matrix"].add(
        np.nan, dim="Variables", dummy=True, col_label="matrix", unit="ha"
    )
    ...
    dm_land_to_crop = DM_land_use["land_man_gap"].filter(
        {"Variables": ["lus_land_to-crop"]}
    )

    # Add a new cat2 = 'total_excess' with the land excess (land type to "change")
    dm_land_excess = DM_land_use["land_man_gap"].filter(
        {"Variables": ["lus_land_excess"]}
    )
    ...
    arr_remaining = np.minimum(
        dm_temp.array[:, :, idx["lus_land"], :],
        dm_temp.array[:, :, idx["lus_land_initial-area_unfccc"], :],
    )
    DM_land_use["land_man_gap"].add(
        arr_remaining, col_label="land_remaining_land", dim="Variables", unit="ha"
    )
    ...
    DM_land_use["land_matrix"].add(
        arr_temp, col_label="land_remaining_land_matrix", dim="Variables", unit="ha"
    )

    # Land use change matrix [ha] = Land remaining land matrix [ha] + land use change matrix without remaining [ha]
    DM_land_use["land_matrix"].operation(
        "land_use_change_without_rem",
        "+",
        "land_remaining_land_matrix",
        dim="Variables",
        out_col="lus_land_matrix",
        unit="ha",
    )
...
def land_carbon_dynamics_workflow(DM_land_use):
    ...
    dm_land_matrix = DM_land_use["land_matrix"].filter(
        {"Variables": ["lus_land_matrix"]}
    )
    DM_land_use["land_soil-type"].append(dm_land_matrix, dim="Variables")
    DM_land_use["land_soil-type"].operation(
        "lus_land_matrix",
        "*",
        "fxa_land-man_soil-type_mineral",
        out_col="lus_land_matrix_mineral",
        unit="ha",
    )
    ...
    DM_land_use["land_soil-type"].operation(
        "lus_land_matrix",
        "*",
        "fxa_land-man_soil-type_organic",
        out_col="lus_land_matrix_organic",
        unit="ha",
    )

    # C stock soil pre processing
    dm_soil_c_stock = DM_land_use["land_c-stock"].filter(
        {"Categories1": ["soil-mineral", "soil-organic"]}
    )
    ...
    DM_land_use["land_soil-type"].operation(
        "lus_land_matrix_organic",
        "*",
        "fxa_land-man_ef_c-stock_soil-mineral",
        out_col="lus_land_c-stock_mineral-soil",
        unit="tC",
    )
    ...
    DM_land_use["land_soil-type"].operation(
        "lus_land_matrix_organic",
        "*",
        "fxa_land-man_ef_c-stock_soil-organic",
        out_col="lus_land_c-stock_organic-soil",
        unit="tC",
    )

    # Total soil carbon [tC] = sum soil carbon (mineral, organic)
    DM_land_use["land_soil-type"].operation(
        "lus_land_c-stock_organic-soil",
        "+",
        "lus_land_c-stock_mineral-soil",
        out_col="lus_land_c-stock_total-soil",
        unit="tC",
    )
    ...
    DM_land_use["land_matrix"].operation(
        "lus_land_matrix",
        "*",
        "fxa_land-man_ef_c-stock_biomass-dead-wood",
        out_col="lus_land_c-stock_biomass-dead-wood",
        unit="tC",
    )
    DM_land_use["land_matrix"].operation(
        "lus_land_matrix",
        "*",
        "fxa_land-man_ef_c-stock_biomass-gain",
        out_col="lus_land_c-stock_biomass-gain",
        unit="tC",
    )
    DM_land_use["land_matrix"].operation(
        "lus_land_matrix",
        "*",
        "fxa_land-man_ef_c-stock_biomass-loss",
        out_col="lus_land_c-stock_biomass-loss",
        unit="tC",
    )

    # Total biomass carbon stock [tC] = sum biomass carbon stock (gain, loss, deadwood)
    DM_land_use["land_matrix"].operation(
        "lus_land_c-stock_biomass-dead-wood",
        "+",
        "lus_land_c-stock_biomass-gain",
        out_col="lus_land_c-stock_biomass-dead-wood-and-gain",
        unit="tC",
    )
    DM_land_use["land_matrix"].operation(
        "lus_land_c-stock_biomass-dead-wood-and-gain",
        "+",
        "lus_land_c-stock_biomass-loss",
        out_col="lus_land_c-stock_biomass-total",
        unit="tC",
    )
    ...
    DM_land_use["land_matrix"].append(
        DM_land_use["land_soil-type"].filter(
            {"Variables": ["lus_land_c-stock_total-soil"]}
        ),
        dim="Variables",
    )
    DM_land_use["land_matrix"].operation(
        "lus_land_c-stock_total-soil",
        "+",
        "lus_land_c-stock_biomass-total",
        out_col="lus_land_lulucf",
        unit="tC",
    )
    ...
    dm_total_c_stock = DM_land_use["land_matrix"].filter(
        {"Variables": ["lus_land_lulucf"]}
    )
    ...
    # Add dummy column
    DM_land_use["land_man_gap"].add(
        -3.667, dummy=True, col_label="tC_to_tCO2", dim="Variables", unit="t"
    )

    # Unit conversion : tC stocked/emitted => tCO2 emitted/stocked
    DM_land_use["land_man_gap"].operation(
        "lus_land_lulucf_to_tC",
        "*",
        "tC_to_tCO2",
        out_col="lus_land_lulucf_to",
        unit="t",
    )

    # Unit conversion : tCO2 emitted/stocked => MtCO2 emitted/stocked
    DM_land_use["land_man_gap"].add(
        0.000001, dummy=True, col_label="t_to_Mt", dim="Variables", unit="t"
    )
    DM_land_use["land_man_gap"].operation(
        "lus_land_lulucf_to",
        "*",
        "t_to_Mt",
        out_col="lus_emissions-CO2_land-to",
        unit="Mt",
    )
...
def forestry_workflow(DM_land_use, dm_wood, dm_land_use):
    ...
    dm_crop_land_agr = dm_land_use.filter(
        {"Variables": ["agr_lus_land"], "Categories1": ["cropland"]}
    )
    dm_grass_land_agr = dm_land_use.filter(
        {"Variables": ["agr_lus_land"], "Categories1": ["grassland"]}
    )
    ...
    array_temp = (
        dm_crop_land_agr.array[:, :, :, np.newaxis]
        * DM_land_use["crop_ef_agroforestry"].array[:, :, :, :]
    )
    DM_land_use["crop_ef_agroforestry"].add(
        array_temp,
        dim="Variables",
        col_label="lus_land_lulucf_agroforestry_cropland",
        unit="tC",
    )
    ...
    DM_land_use["liv_ef_agroforestry"].add(
        array_temp,
        dim="Variables",
        col_label="lus_land_lulucf_agroforestry_grassland",
        unit="tC",
    )
    ...
    # Pre processing
    dm_forest = DM_land_use["land_man_gap"].filter(
        {"Variables": ["lus_land"], "Categories1": ["forest"]}
    )
    ...
    DM_land_use["forestry"].operation(
        "lus_land_forest",
        "*",
        "agr_climate-smart-forestry_csf-man",
        out_col="lus_climate-smart-forestry_biomass_csf-inc",
        unit="m3",
    )

    # Incremental CO2 capture from forestry [Mt] = Incremental biomass gain from forestry [m3] * CO2 capture factor [Mt/m3]
    DM_land_use["forestry"].add(
        -0.0000009,
        dummy=True,
        col_label="CO2_capture_factor",
        dim="Variables",
        unit="Mt/m3",
    )
    DM_land_use["forestry"].operation(
        "CO2_capture_factor",
        "*",
        "lus_climate-smart-forestry_biomass_csf-inc",
        out_col="lus_climate-smart-forestry_biomass_csf-inc_CO2-capture",
        unit="Mt",
    )
    ...
    DM_land_use["forestry"].operation(
        "lus_land_forest",
        "*",
        "agr_climate-smart-forestry_g-inc",
        out_col="lus_forestry_biomass_gross-increment",
        unit="m3",
    )
    ...
    DM_land_use["forestry"].operation(
        "lus_forestry_biomass_gross-increment",
        "*",
        "agr_climate-smart-forestry_faws-share",
        out_col="lus_forestry_biomass_faws_gross-increment",
        unit="m3",
    )
    ...
    #                                                           * harvesting rate [%]
    DM_land_use["forestry"].operation(
        "lus_forestry_biomass_faws_gross-increment",
        "*",
        "agr_climate-smart-forestry_h-rate",
        out_col="lus_forestry_biomass_faws_harvested",
        unit="m3",
    )
    ...
    DM_land_use["forestry"].operation(
        "lus_forestry_biomass_faws_harvested",
        "-",
        "lus_fst_demand_rwe_total",
        out_col="",
        unit="m3",
    )
...
def forestry_biomass_emissions_workflow(DM_land_use, CDM_const):
    ...
    DM_land_use["forestry"].groupby(
        {"yield_nat-losses_total": "agr_climate-smart-forestry_nat-losses.*"},
        dim="Variables",
        regex=True,
        inplace=True,
    )
    ...
    DM_land_use["forestry"].operation(
        "lus_land_forest",
        "*",
        "yield_nat-losses_total",
        out_col="lus_forestry_biomass_loss",
        unit="m3",
    )
    ...
    DM_land_use["forestry"].operation(
        "lus_forestry_biomass_loss",
        "*",
        "fxa_agr_climate-smart-forestry_def-wood-fuel",
        out_col="lus_forestry_biomass_loss_def-wood-fuel",
        unit="m3",
    )
    DM_land_use["forestry"].operation(
        "lus_forestry_biomass_loss",
        "*",
        "fxa_agr_climate-smart-forestry_def-burnt",
        out_col="lus_forestry_biomass_loss_def-burnt",
        unit="m3",
    )
    ...
    array_temp = (
        DM_land_use["forestry"].array[
            :, :, idx_forestry["lus_forestry_biomass_loss_def-burnt"]
        ]
    ...
    DM_land_use["forestry"].add(
        array_temp,
        dim="Variables",
        col_label="lus_emissions_emissions-CO2_forest_to_land_biomass_emissions-CH4",
        unit="t",
    )
    # N2O
    idx_cdm = cdm_burnt.idx
    ...
    DM_land_use["forestry"].add(
        array_temp,
        dim="Variables",
        col_label="lus_emissions_emissions-CO2_forest_to_land_biomass_emissions-N2O",
        unit="t",
    )
    ...
    DM_land_use["forestry"].add(
        array_temp,
        dim="Variables",
        col_label="lus_emissions_emissions-CO2_forest_to_land_biomass_emissions-CO2",
        unit="t",
    )
    return DM_land_use
...
def simulate_industry_to_landuse_input():
    ...
    DM_ind["timber"].rename_col(
        "ind_timber", "ind_material-production_timber", "Variables"
    )

    # woodpulp
    DM_ind["woodpulp"] = dm_ind.filter(
        {"Variables": ["ind_material-production_paper_woodpulp"]}
    )
...
def landuse_emissions_interface(DM_land_use, write_xls=False):
    ...
    dm_ems = DM_land_use["land_man_gap"].filter(
        {"Variables": ["lus_emissions-CO2_land-to"]}
    )
    ...
    if write_xls is True:
        ...
        dm_ems.to_excel(
            ...
            + "/../_database/data/xls/"
            + "All-Countries_interface_from-landuse-to-climate.xlsx",
            index=False,
        )
...
def landuse_TPE(DM_land_use, dm_wood_TPE):
    ...
    dm_land_emissions = DM_land_use["land_man_gap"].filter(
        {"Variables": ["lus_emissions-CO2_land-to"]}
    )
...
def land_use(lever_setting, years_setting, interface=Interface(), calibration=False):
    ...
    dm_wood, dm_wood_TPE, df_cal_rates_wood = wood_workflow(
        DM_agr["wood"], DM_agr["lgn"], DM_ind, DM_land_use["cal_wood"], years_setting
    )
