...
def bld_emissions_interface(dm_appliances, DM_energy):
    dm_emissions_fuel = DM_energy["heat-emissions-by-fuel"].filter(
        {
            "Categories1": [
                "gas-ff-natural",
                "heat-ambient",
    ...
    dm_emissions_fuel.rename_col(
        "bld_CO2-emissions", "bld_emissions-CO2", dim="Variables"
    )

    dm_appliances = dm_appliances.filter({"Categories1": ["non-residential"]})
    dm_appliances.rename_col(
        "bld_CO2-emissions_appliances", "bld_emissions-CO2_appliances", dim="Variables"
    )
...
def bld_industry_interface(DM_floor, dm_appliances, dm_pipes):
    ...
    dm_reno.rename_col(
        "bld_renovated-surface-area", "bld_product-demand", dim="Variables"
    )
    ...
    dm_constructed.rename_col(
        "bld_floor-area-constructed", "bld_product-demand", dim="Variables"
    )
    ...
    dm_pipes.rename_col(
        ...
        "bld_product-demand_new-dhg-pipe",
        dim="Variables",
    )
    dm_pipes.deepen()
