...
def define_sets(ampl):
    ...
    ampl.getSet("SECTORS").setValues(
        ["HOUSEHOLDS", "SERVICES", "INDUSTRY", "TRANSPORTATION"]
    )
    ampl.getSet("END_USES_INPUT").setValues(
        [
            "ELECTRICITY",
            "LIGHTING",
            "HEAT_HIGH_T",
    ...
    ampl.getSet("END_USES_CATEGORIES").setValues(
        [
            "ELECTRICITY",
            "HEAT_HIGH_T",
            "HEAT_LOW_T",
    ...
    ampl.getSet("RESOURCES").setValues(
        [
            "ELECTRICITY",
            "GASOLINE",
            "DIESEL",
    ...
    ampl.getSet("INFRASTRUCTURE").setValues(
        [
            "EFFICIENCY",
            "DHN",
            "GRID",
...
def extract_sankey_energy_flow(DM):
    ...
    if "NG_CCS-NG_CCSv2" in dm_energy_prod.col_labels["Categories1"]:
        dm_natural_gas_elec = dm_energy_prod.filter(
            {"Categories1": ["NG_CCS-NG_CCSv2"]}
        )
...
def extract_2050_output(ampl, country_prod, endyr, years_fts, DM_energy):
    ...
    DM["efficiency"] = utils.ampl_param_to_dm(
        ampl,
        ampl_var_name="layers_in_out",
    ...
    power_categories = list(
        ampl.get_set("TECHNOLOGIES_OF_END_USES_TYPE").get("ELECTRICITY")
    )
...
def create_future_country_trend(DM_2050, DM_input, years_ots, years_fts):
    ...
    dm_cap.array[0, idx_fts, idx["pow_capacity"], -1] = np.minimum(
        dm_cap.array[0, idx_fts, idx["pow_capacity"], -1],
    ...
    dm_cap_tmp.add(
        fake_net_import_cap, dummy=True, dim="Categories1", col_label="Net-import"
    )
    dm_prod_hist.append(
        dm_cap_tmp.filter({"Categories1": dm_prod_hist.col_labels["Categories1"]}),
        dim="Variables",
    )
    dm_prod_hist.operation(
        "pow_production",
        "/",
        "pow_capacity",
    ...
    idx_fts = [idx[yr] for yr in years_fts]
    dm_prod_hist.array[0, idx_fts, idx["pow_production"], :] = (
        dm_prod_hist.array[0, idx_fts, idx["pow_capacity"], :]
        * dm_prod_hist.array[0, idx_fts, idx["pow_cap-fact"], :]
    )

    dm_prod_hist.change_unit(
        "pow_cap-fact",
        old_unit="TWh/MW",
        new_unit="%",
        factor=8.760 * 1e-3,
...
def energyscope(data_path, DM_tra, DM_bld, DM_ind, years_ots, years_fts, country_list):
    ...
    if ["EU27"] == country_list:  # If you are running for EU27
        ...
        inter.impose_capacity_constraints(
            ampl, endyr, dm_capacity, country=country_prod
        )
        share_of_pop = 1
    ...
    dm_tra_demand_trend = inter.impose_transport_demand(
        ampl, endyr, share_of_pop, DM_tra, country_dem
    )
    dm_bld_demand_trend = inter.impose_buildings_demand(
        ampl, endyr, share_of_pop, DM_bld, DM_ind, country_dem
    )
    dm_ind_demand_trend = inter.impose_industry_demand(
        ampl, endyr, share_of_pop, DM_ind, country_dem
    )
    ...
    dm_prod_cap_cntr = create_future_country_trend(
