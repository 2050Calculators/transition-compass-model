...
def ampl_var_to_dm(
    ...
    i = 1
    for cat_group in indexes:
        col_labels["Categories" + str(i)] = [
            row[0] for row in ampl.get_set(cat_group).get_values()
        ]
        i = i + 1
...
def get_pyomo_output(m, country, endyr):
    dm_f_mult = pyomo_var_to_dm(
        m,
        pyomo_var_name="F_Mult",
        cntr_name=country,
        end_yr=endyr,
    ...
    dm_storage_in = pyomo_var_to_dm(
        m,
        pyomo_var_name="Storage_In",
        cntr_name=country,
        end_yr=endyr,
    ...
    dm_storage_out = pyomo_var_to_dm(
        m,
        pyomo_var_name="Storage_Out",
        cntr_name=country,
        end_yr=endyr,
        indexes=["STORAGE_TECH", "LAYERS", "PERIODS"],
    ...
    dm_installed_n = pyomo_var_to_dm(
        m,
        pyomo_var_name="Number_Of_Units",
        cntr_name=country,
        end_yr=endyr,
        indexes=["TECHNOLOGIES"],
    ...
    dm_GWP = pyomo_var_to_dm(
        m,
        pyomo_var_name="GWP_op",
        cntr_name=country,
        end_yr=endyr,
        indexes=["RESOURCES"],
    ...
    dm_Losses = pyomo_var_to_dm(
        m,
        pyomo_var_name="Losses",
        cntr_name=country,
        end_yr=endyr,
    ...
    DM = {
        "installed_GW": dm_f_mult,
        "installed_N": dm_installed_n,
        "emissions": dm_GWP,
        "storage_in": dm_storage_in,
        "storage_out": dm_storage_out,
        "monthly_operation_GW": dm_f_mult_t,
        "Losses": dm_Losses,
    }
...
def get_ampl_output(ampl, country, endyr):
    ...
    # Sto_in: Power [GW] input to the storage in a certain period
    dm_storage_in = ampl_var_to_dm(
        ampl,
        ampl_var_name="Storage_In",
        cntr_name=country,
        end_yr=endyr,
        indexes=["STORAGE_TECH", "LAYERS", "PERIODS"],
        unit_dict={"Storage_In": "GW"},
    ...
    # Sto_out: Power [GW] output from the storage in a certain period
    dm_storage_out = ampl_var_to_dm(
        ampl,
        ampl_var_name="Storage_Out",
        cntr_name=country,
        end_yr=endyr,
        indexes=["STORAGE_TECH", "LAYERS", "PERIODS"],
        unit_dict={"Storage_Out": "GW"},
    ...
    # N: number of units of size ref_size which are installed
    dm_installed_n = ampl_var_to_dm(
        ampl,
        ampl_var_name="Number_Of_Units",
        cntr_name=country,
        end_yr=endyr,
        indexes=["TECHNOLOGIES"],
        unit_dict={"Number_Of_Units": "-"},
    ...
    # Total yearly emissions of the resources [ktCO2-eq./y]
    dm_GWP = ampl_var_to_dm(
        ampl,
        ampl_var_name="GWP_op",
        cntr_name=country,
        end_yr=endyr,
        indexes=["RESOURCES"],
        unit_dict={"GWP_op": "ktCO2-eq./y"},
...
def pyomo_param_to_dm(
    ...
    rename_dict = {}
    if len(indexes) == 2:
        common_cat = set(col_labels["Categories1"]).intersection(
            set(col_labels["Categories2"])
        )
        rename_dict = {k: k + "v2" for k in common_cat}
        new_cat_2 = list(
            (set(col_labels["Categories2"]) - common_cat).union(
                set(rename_dict.values())
            )
        )
