...
def extract_transport_demand(DM_tra):
    ...
    dm_demand_trend.operation(
        ...
        out_col="freight_energy-consumption",
        unit="kWh",
    )
    dm_demand_trend.group_all("Categories1")
    dm_demand_trend.filter({"Variables": ["freight_energy-consumption"]}, inplace=True)
    dm_demand_trend.change_unit(
        "freight_energy-consumption", old_unit="kWh", new_unit="TWh", factor=1e-9
    )
    dm_demand_trend.groupby(
        {"electricity": ".*"}, regex=True, inplace=True, dim="Categories1"
    )
    ...
    dm_tmp.operation(
        ...
        out_col="pass_energy-consumption",
        unit="kWh",
    )
    dm_tmp.change_unit(
        "pass_energy-consumption", old_unit="kWh", new_unit="TWh", factor=1e-9
    )
    ...
    dm_demand_trend.groupby(
        {"tra_energy-consumption": ".*"}, regex=True, inplace=True, dim="Variables"
    )
...
def impose_transport_demand_pyomo(m, endyr, share_pop, DM_tra, cntr):
    ...
    DM_tra["passenger"].change_unit(
        "tra_passenger_transport-demand", old_unit="pkm", new_unit="Mpkm", factor=1e-6
    )
    DM_tra["passenger"].filter(
        {"Categories2": ["BEV", "CEV", "mt"]}, inplace=True
    )  # 'PHEV-diesel', 'PHEV-gasoline',
    ...
    DM_tra["freight"].filter(
        {"Categories2": ["BEV", "CEV"]}, inplace=True
    )  # 'PHEV-diesel', 'PHEV-gasoline'
    ...
    DM_tra["freight"].change_unit(
        "tra_freight_transport-demand-tkm", old_unit="tkm", new_unit="Mtkm", factor=1e-6
    )
    tot_Mtkm = (
        np.nansum(
    ...
    dm_private.groupby({"CAR_BEV": ["BEV"]}, dim="Categories2", inplace=True)
    ...
    dm_public.groupby(
        {"TRAIN_PUB": ["rail_CEV"], "TRAMWAY_TROLLEY": ["metrotram_mt", "bus_CEV"]},
        dim="Categories1",
        inplace=True,
    )
    ...
    dm_eff_freight.groupby({"TRAIN_FREIGHT": ["rail"]}, dim="Categories1", inplace=True)
    dm_eff_freight.group_all("Categories2")
    dm_eff_freight.operation(
...
def impose_space_heating_pyomo(m, endyr, share_of_pop, DM_bld, cntr, eps):
    ...
    dm_heating = DM_bld.filter(
        {"Variables": ["bld_heating_useful-energy", "bld_heating_energy-consumption"]}
    )
...
def reorganise_space_heat_hot_water(DM_bld, DM_ind):
    ...
    dm_house_hotwater = DM_bld["households_hot-water"].filter(
        {
            "Variables": ["bld_hw_useful-energy", "bld_hot-water_energy-demand"],
            "Categories1": ["district-heating", "electricity", "heat-pump"],
        }
    )
    dm_house_hotwater.rename_col(
        ["bld_hw_useful-energy", "bld_hot-water_energy-demand"],
        [
            "bld_useful-energy_hot-water_households",
            "bld_energy-consumption_hot-water_households",
        ],
        "Variables",
    )
    ...
    dm_house_heat.rename_col(
        "bld_heating", "bld_useful-energy_space-heating_households", "Variables"
    )
    dm_house_heat.rename_col(
        "bld_energy-demand_heating",
        "bld_energy-consumption_space-heating_households",
        "Variables",
    )
    ...
    dm_service_heat.rename_col(
        "bld_services_useful-energy", "bld_useful-energy_services", "Variables"
    )
    dm_service_heat.rename_col(
        "bld_services_energy-consumption",
        "bld_energy-consumption_services",
        "Variables",
    )
    ...
    dm_ind_heat = DM_ind["ind-energy-demand"].filter(
        {
    ...
    dm_ind_heat.rename_col(
        "ind_energy-end-use", "ind_heat_energy-consumption", "Variables"
    )
...
def impose_buildings_demand_pyomo(m, endyr, share_of_pop, DM_bld, DM_ind, cntr):
    ...
    if validation:
        ...
        DM_ind["ind-energy-demand"].filter(
            {"Categories2": ["electricity", "heat-pump"]}, inplace=True
        )
        DM_ind["ind-energy-demand"].group_all("Categories2")
        DM_ind["ind-energy-demand"].group_all("Categories1")

        DM_bld["services_all"].filter(
            {
    ...
    dm_heat.change_unit(
        "bld_useful-energy", old_unit="TWh", new_unit="GWh", factor=1000
    )
    dm_heat.change_unit(
        "bld_energy-consumption", old_unit="TWh", new_unit="GWh", factor=1000
    )
    dm_ind_heat.change_unit(
        "ind_heat_energy-consumption", old_unit="TWh", new_unit="GWh", factor=1000
    )
    # Section: LOW T HEAT
    ...
    dm_heat.rename_col(
        ["bld_energy-consumption", "bld_useful-energy"],
    ...
    dm_elec_light.groupby(
        {"bld_energy-consumption-elec_electricity": ".*"},
        regex=True,
        inplace=True,
        dim="Variables",
    )
    ...
    dm_add_missing_variables(
        dm_elec_light, {"Categories1": dm_demand_trend.col_labels["Categories1"]}
    )
    dm_demand_trend.filter({"Variables": ["bld_energy-consumption"]}, inplace=True)
    dm_demand_trend.append(dm_elec_light, dim="Variables")
    dm_demand_trend.groupby(
        {"bld_energy-consumption": ".*"}, regex=True, inplace=True, dim="Variables"
    )
...
def impose_capacity_constraints_pyomo(m, endyr, dm_capacity, country):
    ...
    dm_CH.change_unit("pow_capacity-Pmax", old_unit="MW", new_unit="GW", factor=1e-3)
...
def extract_industry_demand(DM_ind):
    dm_demand_trend = DM_ind["ind-energy-demand"].filter(
        {
            "Categories1": ["elec", "lighting", "process-heat"],
            "Categories2": ["electricity"],
        }
    )
    dm_demand_trend.group_all("Categories1", inplace=True)
    dm_demand_trend.add(
        0, dim="Categories1", col_label=["heat-pump", "district-heating"], dummy=True
    )
    return dm_demand_trend
...
def extract_agriculture_demand(DM_agr):
    ...
    dm_agr_demand_trend = DM_agr["power"].filter(
        {"Categories1": ["electricity", "district-heating"]}
    )
...
def impose_industry_demand_pyomo(m, endyr, share_of_pop, DM_ind, DM_agr, cntr):
    ...
    dm_ind_elec_tot = DM_ind["ind-energy-demand"].filter(
    ...
    # LIGHTING
    dm_ind_light_tot = DM_ind["ind-energy-demand"].filter(
