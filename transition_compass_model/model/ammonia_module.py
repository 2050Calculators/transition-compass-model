...
from .common.auxiliary_functions import energy_switch
import pickle
import json
import os
import numpy as np
...
def variables_for_tpe(
    ...
    dm_ind_material_production,
    dm_energy_demand_bymat,
    dm_ind_energy_demand,
    dm_energy_demand_bymatcarr,
):
    ...
    dm_tpe.rename_col(
        "material-production", "ind_material-production_chemicals", "Variables"
    )

    # energy demand chemicals
    dm_temp = dm_energy_demand_bymat.copy()
    dm_temp.append(
        dm_ind_energy_demand.group_all("Categories2", inplace=False), "Categories1"
    )
...
def ammonia(
    ...
    dm_agriculture = get_interface(
        current_file_directory, interface, "agriculture", "ammonia", cntr_list
    )
    DM_industry = get_interface(
        current_file_directory, interface, "industry", "ammonia", cntr_list
    )
    ...
    dm_material_demand = apply_material_decomposition(
        dm_production_fert, CDM_const["material-decomposition_fertilizer"]
    )
    ...
    results_run = variables_for_tpe(
        DM_material_production["bymat"],
        DM_industry["material-production"],
        DM_energy_demand["bymat"],
        DM_industry["energy-demand"],
        DM_energy_demand["bymatcarr"],
    )

    # interface energy
    DM_ene = ammonia_energy_interface(
        DM_energy_demand["bycarr"],
