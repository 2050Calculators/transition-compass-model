...
def read_data(DM_buildings, lever_setting):
    ...
    DM_appliances = {
        "demand": DM_buildings["fxa"]["appliances"],
        "household-size": DM_ots_fts["floor-intensity"].filter(
    ...
    DM_energy = {
        "heating-efficiency": DM_ots_fts["heating-efficiency"],
        "heating-technology": DM_ots_fts["heating-technology-fuel"][
...
def buildings(lever_setting, years_setting, DM_input, interface=Interface()):
    ...
    DM_floor_out = wkf.bld_floor_area_workflow(
    ...
    DM_energy_out = wkf.bld_energy_workflow(
        DM_energy, dm_clm, DM_floor_out["wf-energy"], cdm_const
    )
    ...
    if check_residential_energy_demand:
        ...
        dm_hot_water.rename_col(
            "bld_hot-water_energy-demand", "bld_energy-demand_hot-water", "Variables"
        )
        dm_add_missing_variables(
            dm_hot_water, {"Categories1": dm_heat_cool.col_labels["Categories1"]}
        )
        dm_appliances = DM_appliances_out["power"].copy()
        dm_appliances.rename_col(
            "bld_appliances_tot-elec-demand",
            "bld_energy-demand_appliances_electricity",
            "Variables",
        )
        dm_appliances.deepen()
        dm_add_missing_variables(
            dm_appliances, {"Categories1": dm_heat_cool.col_labels["Categories1"]}
        )
        dm_light = DM_light_out["TPE"].copy()
        dm_light.rename_col(
            "bld_residential-lighting",
            "bld_energy-demand_light_electricity",
            "Variables",
        )
        dm_light.deepen()
        dm_add_missing_variables(
            dm_light, {"Categories1": dm_heat_cool.col_labels["Categories1"]}
        )
        ...
        dm_residential_energy.change_unit(
            "bld_energy-demand", old_unit="TWh", new_unit="TJ", factor=3600
        )
