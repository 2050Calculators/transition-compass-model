...
def read_data(DM_forestry, lever_setting):
    ...
    DM_wood_conversion = {
        "coniferous-share": dm_wood_type_split_per_use,
    ...
    DM_forestry = {"fxa": dict_fxa, "constant": dict_const}
...
def wood_demand_m3(dm_wood_demand, DM_wood_conversion):
    ...
    ay_wood_conversion = (
        dm_wood_demand_industry_m3[:, :, :, :]
        * dm_wood_yields[np.newaxis, np.newaxis, :, :]
    )
    dm_wood_demand_industry_m3.add(
        ay_wood_conversion, col_label="wood-category-eq", dim="Variables", unit="t"
    )
    ...
    dm_wood_demand_industry_m3.groupby(
        {"any-other-wood": ["any-other-wood", "any-other-industrial-demand"]},
        dim="Categories1",
        inplace=True,
    )

    # Split between coniferous and non-coniferous wood
    dm_wood_split = DM_wood_conversion["coniferous-share"].filter_w_regex(
        {"Variables": "fst_production-t_share"}
    )
    dm_wood_split.drop(dim="Categories1", col_label="woodfuel")
    dm_wood_demand_t = dm_wood_split
    ay_wood_split = (
        dm_wood_demand_industry_m3[:, :, "wood-category-eq", :, np.newaxis]
        * dm_wood_split[:, :, "fst_production-t_share", :, :]
    )
    dm_wood_demand_t.add(
        ay_wood_split,
        col_label="wood-demand-per-type",
        dim="Variables",
        unit="t",
        dummy=True,
    )
    ...
    dm_wood_density = DM_wood_conversion["wood-density"].filter(
        {"Categories1": ["any-other-wood", "industrial-wood", "sawlogs", "woodfuel"]}
    )
    ...
    ay_wood_density = (
        dm_wood_use_m3[:, :, :, :, :] * dm_wood_density[np.newaxis, np.newaxis, :, :, :]
    )
    dm_wood_use_m3.add(
        ay_wood_density, col_label="wood-use", dim="Variables", unit="m3", dummy=True
    )
    dm_wood_total = dm_wood_use_m3.flatten()
    dm_wood_total = dm_wood_total.filter({"Variables": ["wood-use"]})
    dm_wood_total.groupby(
        {"total-industry": ".*"}, regex=True, inplace=True, dim="Categories1"
    )

    # Industrial by-production for fuelwood
    cdm_byproduct = DM_wood_conversion[
        "industry-yields"
    ]  # .filter({'Categories1': ['wood-fuel-byproducts']})
    cdm_byproduct = cdm_byproduct.filter({"Categories1": ["wood-fuel-byproducts"]})
    ay_byproduct = (
        dm_wood_use_m3[:, :, "wood-use", :, :]
        * cdm_byproduct[np.newaxis, np.newaxis, np.newaxis :, :]
    )
    dm_wood_use_m3.add(
        ay_byproduct,
        col_label="woodfuel-byproducts",
        dim="Variables",
        unit="m3",
        dummy=True,
    )
    dm_supply_byproduct = dm_wood_use_m3.filter(
        {"Variables": ["woodfuel-byproducts"]}
    ).flatten()
    dm_supply_byproduct.groupby(
        {"total-industry": ".*"}, regex=True, inplace=True, dim="Categories1"
    )
...
def fuelwood_demand_m3(dm_fuelwood_demand, DM_wood_conversion):
    ...
    # Split between coniferous and non-coniferous wood
    ay_wood_split = (
        dm_fuelwood_demand[:, :, "pow_energy-supply", :, np.newaxis]
        * dm_wood_split[:, :, "fst_production-t_share", :, :]
    )
    dm_wood_split.add(
        ay_wood_split,
    ...
    dm_wood_yields = DM_wood_conversion["energy-density"].filter(
        ({"Categories1": ["coniferous", "non-coniferous"]})
    )
    dm_fuelwood_t = dm_wood_split.filter(({"Variables": ["wood-demand-per-type"]}))
    ay_fuelwood_t = (
        dm_fuelwood_t[:, :, "wood-demand-per-type", :, :]
        * 1000000
        / dm_wood_yields[np.newaxis, np.newaxis, :, :]
    )
    dm_fuelwood_t.add(
        ay_fuelwood_t,
        col_label="wood-demand-per-species",
        dim="Variables",
        unit="m3",
        dummy=True,
    )
    ...
    dm_woodfuel_demand.groupby(
        {"total-demand": ".*"}, regex=True, inplace=True, dim="Categories1"
    )
    dm_woodfuel_demand.rename_col(
        ["wood-demand-per-species"], ["fuelwood"], dim="Variables"
    )

    DM_power = {"total-demand": dm_woodfuel_demand, "tpe": dm_fuelwood_m3}
...
def wood_supply(dm_forest_area, DM_forestry, DM_ots_fts):
    ...
    dm_exploited.operation(
        "productive-share",
        "*",
        "total-forest-area",
        out_col="exploited-forest",
        unit="ha",
    )
    dm_exploited.operation(
        "unproductive-share",
        "*",
        "total-forest-area",
        out_col="unexploited-forest",
        unit="ha",
    )

    dm_forest_land = dm_exploited.filter(
        ({"Variables": ["exploited-forest", "unexploited-forest"]})
    )
    ...
    ay_harvest = (
        dm_productive_area[:, :, "exploited-forest", np.newaxis]
        * dm_harvested_wood[:, :, "harvest-rate", :]
    )
    dm_harvested_wood.add(
        ay_harvest, col_label="wood-harvested", dim="Variables", unit="m3", dummy=True
    )
...
def forestry_to_tpe(DM_supply, DM_power, DM_industry, DM_ots_fts):
    ...
    # Wood demand per use
    tpe_wood_demand = (
        DM_industry["wood-demand"]
        .filter(({"Variables": ["wood-use"]}))
        .flatten()
        .flatten()
    )
    tpe_supply_species = tpe_wood_demand.copy()
    tpe_supply_species.groupby(
        {
            "wood-supply_coniferous": ".*_coniferous",
            " wood-supply_non-coniferous": ".*_non-coniferous",
        },
        regex=True,
        inplace=True,
        dim="Variables",
    )
    tpe_wood_demand.groupby(
        {
            "any-other-wood": ".*any-other-wood.*",
    ...
    tpe_energy_wood_demand = (
        DM_power["total-demand"].filter(({"Variables": ["fuelwood"]})).flatten()
    )
    tpe_wood_demand.append(tpe_energy_wood_demand, dim="Variables")
    tpe_wood_demand.rename_col(["fuelwood_total-demand"], ["woodfuel"], dim="Variables")

    # tpe_wood_demand.datamatrix_plot({'Country': ['Switzerland']}, stacked=True)

    # Wood demand per use per species
    tpe_industry_wood_species = (
        DM_industry["wood-demand"].filter(({"Variables": ["wood-use"]})).flatten()
    )
    ...
    # Harvested wood
    dm_harvested_wood = DM_supply["harvested"].filter(
        ({"Categories1": ["coniferous", "non-coniferous"]})
    )
    dm_harvested_wood.rename_col(["wood-supply"], ["harvested-wood"], dim="Variables")
    ...
    dm_harvest_supply.groupby(
        {"harvested-wood": ".*"}, regex=True, inplace=True, dim="Categories1"
    )
    ...
    # Harvest rate
    tpe_harvest_rate = DM_ots_fts["harvest-rate"].filter(
        ({"Variables": ["harvest-rate"]})
    )
...
def forestry(lever_setting, years_setting, DM_input, interface=Interface()):
    ...
    if interface.has_link(from_sector="energy", to_sector="forestry"):
        dm_fuelwood_demand = interface.get_link(
            from_sector="energy", to_sector="forestry"
        )
    else:
    ...
    if interface.has_link(from_sector="industry", to_sector="forestry"):
        dm_wood_demand = interface.get_link(
            from_sector="industry", to_sector="forestry"
        )
    else:
        if len(interface.list_link()) != 0:
    ...
    dm_rwe_demand.add(
        ay_net_balance, col_label="net-balance", dim="Variables", unit="m3", dummy=True
    )
...
def local_forestry_run():
    ...
    DM_input = filter_country_and_load_data_from_pickles(
        country_list=country_list, modules_list="forestry"
    )
