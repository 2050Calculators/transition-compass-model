...
# ImportFunctions
from .common.auxiliary_functions import read_level_data, my_pickle_dump
...
def lifestyles(
    ...
    interface.add_link(
        from_sector="lifestyles", to_sector="agriculture", dm=DM_pop["pop"]
    )
    ...
    interface.add_link(
        from_sector="lifestyles", to_sector="transport", dm={"pop": dm_pop}
    )
    ...
    if write_pickle is True:
        ...
        my_pickle_dump(DM_new={"pop": dm_pop}, local_pickle_file=f)
    interface.add_link(
        from_sector="lifestyles", to_sector="buildings", dm={"pop": dm_pop}
    )
    ...
    if write_pickle is True:
        f = os.path.join(
            current_file_directory,
            "../_database/data/interface/lifestyles_to_industry.pickle",
        )
        my_pickle_dump(DM_new={"pop": dm_pop}, local_pickle_file=f)
    interface.add_link(
        from_sector="lifestyles", to_sector="industry", dm={"pop": dm_pop}
    )
...
def local_lifestyles_run(write_pickle=False):
    ...
    DM_input = filter_country_and_load_data_from_pickles(
        country_list=country_list, modules_list="lifestyles"
    )

    lifestyles(
        lever_setting, years_setting, DM_input["lifestyles"], write_pickle=write_pickle
    )
    return
