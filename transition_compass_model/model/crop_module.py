...
def read_data(DM_crop_pickle, lever_setting):
    ...
    # For levers : ssr-bev-.*
    # list of lever names
    levers = ["ssr-bev-beer", "ssr-bev-wine", "ssr-bev-bev-alc", "ssr-bev-bev-fer"]
    ...
    # For levers : ssr-pro-.*
    # list of lever names
    levers = ["ssr-pro-sugar", "ssr-pro-voil", "ssr-pro-sweet"]
    ...
    # list of lever names
    levers = ["crop-share-organic", "crop-share-extensive", "crop-share-intensive"]
    ...
    dm_fxa_cal_crop_imports_countries = DM_crop_pickle["fxa"][
        "cal_agr_imports-crop_countries"
    ]
...
def simulate_livestock_to_crop_input():
    current_file_directory = os.path.dirname(os.path.abspath(__file__))
    f = os.path.join(
        current_file_directory, "../_database/data/interface/livestock_to_crop.pickle"
    )
    with open(f, "rb") as handle:
...
def crop_workflow(
    ...
    dm_pro_yield = DM_crop_prod["processing-yields"].filter(
        {"Categories1": ["cake-to-oilcrop", "molasse-to-sugarcrop"]}
    )
    dm_pro_yield.rename_col("cake-to-oilcrop", "crop-processed-cake", dim="Categories1")
    dm_pro_yield.rename_col(
        "molasse-to-sugarcrop", "crop-processed-molasse", dim="Categories1"
    )
    ...
    dm_feed_processed.groupby(
        {"crop-oilcrop": ".*cake", "crop-sugarcrop": ".*molasse"},
        dim="Categories1",
        regex=True,
        inplace=True,
    )
    ...
    dm_ssr_food_pro = (
        DM_crop_prod["ssr-pro"]
        .filter(
            {
                "Variables": ["agr_ssr"],
                "Categories1": dm_food_processed.col_labels["Categories1"],
            }
        )
        .copy()
    )
    ...
    dm_food_processed.groupby(
        {"pro-crop-processed-sugar": ".*sugar|.*sweet"},
        dim="Categories1",
        regex=True,
        inplace=True,
    )
    dm_food_processed.rename_col(
        "pro-crop-processed-sugar", "crop-sugarcrop", dim="Categories1"
    )
    dm_food_processed.rename_col(
        "pro-crop-processed-voil", "crop-oilcrop", dim="Categories1"
    )
    dm_pro_yield = DM_crop_prod["processing-yields"].filter(
        {"Categories1": ["sugar-to-sugarcrop", "voil-to-oilcrop"]}
    )
    dm_pro_yield.rename_col("sugar-to-sugarcrop", "crop-sugarcrop", dim="Categories1")
    dm_pro_yield.rename_col("voil-to-oilcrop", "crop-oilcrop", dim="Categories1")
    ...
    dm_food_processed_imports.operation(
        "agr_demand",
        "-",
        "agr_domestic-production_food_pro",
    ...
    dm_cal_rates_food_pro = calibration_rates(
        dm_food_processed_imports.filter(
            {"Variables": ["agr_imported-production_food_pro_raw"]}
        ),
        DM_crop_prod["cal_imports-crop-pro_tot"],
    ...
    dm_food_processed_imports.operation(
        "agr_imported-production_food_pro_raw",
        "*",
        "cal_rate",
        dim="Variables",
        out_col="agr_imported-production_food_pro",
        unit="kcal",
    )
    dm_food_processed_imports.filter(
        {
            "Variables": [
                "agr_demand",
                "agr_domestic-production_food_pro",
                "agr_imported-production_food_pro",
            ]
        },
        inplace=True,
    )
    ...
    array_temp = (
        dm_food_processed_imports[:, :, "agr_imported-production_food_pro", :]
        * dm_trade[:, :, "agr_split-import", :]
    )
    DM_crop_prod["split-import-crop-pro"].add(
    ...
    DM_crop_prod["split-import-crop-pro"].add(
        0.0,
        dummy=True,
        col_label="agr_domestic_production_food-pro_raw",
        dim="Variables",
        unit="kcal",
    )
    ...
    # Sugar to sugarcrop
    array_temp = (
        DM_crop_prod["split-import-crop-pro"][
            :, :, "agr_domestic_production", "pro-crop-processed-sugar"
        ]
        * cdm_food["cp_ibp_processed", "crop-processed-sugar"]
    )
    DM_crop_prod["split-import-crop-pro"][
        :, :, "agr_domestic_production_food-pro_raw", "pro-crop-processed-sugar"
    ] = array_temp

    # Sweet to sugarcrop
    array_temp = (
        DM_crop_prod["split-import-crop-pro"][
            :, :, "agr_domestic_production", "pro-crop-processed-sweet"
        ]
        * cdm_food["cp_ibp_processed", "crop-processed-sweet"]
    )
    DM_crop_prod["split-import-crop-pro"][
        :, :, "agr_domestic_production_food-pro_raw", "pro-crop-processed-sweet"
    ] = array_temp

    # Voil to oilcrop
    array_temp = (
        DM_crop_prod["split-import-crop-pro"][
            :, :, "agr_domestic_production", "pro-crop-processed-voil"
        ]
        * cdm_food["cp_ibp_processed", "crop-processed-voil"]
    )
    DM_crop_prod["split-import-crop-pro"][
        :, :, "agr_domestic_production_food-pro_raw", "pro-crop-processed-voil"
    ] = array_temp
    ...
    dm_crop_demand = dm_demand.filter_w_regex(
    ...
    dm_sugarcrop = dm_food_processed.filter(
        {"Variables": ["agr_demand_food"], "Categories1": ["crop-sugarcrop"]}
    )
    ...
    dm_crop_demand[:, :, "agr_demand_food", "oilcrop"] = (
        dm_crop_demand[:, :, "agr_demand_food", "oilcrop"]
    ...
    dm_crop_demand.operation(
        ...
        out_col="agr_domestic_production_raw",
        unit="kcal",
    )
    ...
    dm_cal_rates_crop.add(
        1.0, dummy=True, col_label="rice", dim="Categories1", unit="%"
    )
    dm_crop_demand.append(dm_cal_rates_crop, dim="Variables")
    dm_crop_demand.operation(
        "agr_domestic_production_raw",
        "*",
        "cal_rate",
        dim="Variables",
        out_col="agr_domestic_production",
        unit="kcal",
    )
    dm_crop_demand.filter(
        {
            "Variables": [
    ...
    dm_cal_imports_tot = DM_crop_prod["cal_imports-crop_tot"].filter_w_regex(
        {"Categories1": "crop-"}
    )
    dm_cal_imports_tot.rename_col_regex("crop-", "", dim="Categories1")
    dm_imports_tot = dm_crop_demand.filter(
        {"Variables": ["agr_imported_production_total_raw"]}
    )
    dm_cal_rates_imports = calibration_rates(
        dm_imports_tot,
        dm_cal_imports_tot,
        calibration_start_year=1990,
        calibration_end_year=2023,
    ...
    # Imported production per region [kcal] = Imported production total [kcal] * split per region [-]
    DM_crop_prod["split-import-crop"].filter_w_regex(
        {"Categories1": "crop-"}, inplace=True
    )
    dm_trade = DM_crop_prod["split-import-crop"].copy()
    array_temp = (
        dm_crop_demand[:, :, "agr_imported_production_total", :]
        * dm_trade[:, :, "agr_split-import", :]
    )
    DM_crop_prod["split-import-crop"].add(
        array_temp,
        dim="Variables",
        col_label="agr_domestic_production_raw",
        unit="kcal",
    )

    # Filter to only have imported production per countries
    dm_production = DM_crop_prod["split-import-crop"].filter_w_regex(
        {"Variables": "agr_domestic_production_raw"}
    )

    # Calibration - Imports per countries
    dm_cal_imports_countries = DM_crop_prod[
        "cal_imports-crop_countries"
    ].filter_w_regex({"Categories1": "crop-"})
    dm_cal_rates_imports_countries = calibration_rates(
        dm_production,
        dm_cal_imports_countries,
        calibration_start_year=1990,
        calibration_end_year=2023,
        years_setting=years_setting,
    )
    ...
    array_temp = dm_crop_demand["Switzerland", :, "agr_domestic_production", :]
...
def crop(lever_setting, years_setting, DM_input, write_pickle, interface=Interface()):
    ...
    current_file_directory = os.path.dirname(os.path.abspath(__file__))
    DM_ots_fts, DM_crop_prod, DM_cropland, CDM_const = read_data(
    ...
    if interface.has_link(from_sector="dietary-habits", to_sector="crop"):
        DM_diet_crop = interface.get_link(
            from_sector="dietary-habits", to_sector="crop"
        )
    ...
    if interface.has_link(from_sector="livestock", to_sector="crop"):
        DM_livestock_to_crop = interface.get_link(
            from_sector="livestock", to_sector="crop"
        )
    ...
    # alcoholic-beverages
    if interface.has_link(from_sector="livestock", to_sector="crop"):
        DM_alc_to_crop = interface.get_link(
            from_sector="alcoholic-beverages", to_sector="crop"
        )
        dm_bev_dom_prod = DM_alc_to_crop["crop_bev"]
        dm_bev_raw_imports = DM_alc_to_crop["imports_bev_raw"]
        dm_match_countries(
            dm_bev_raw_imports, DM_crop_prod["split-import-crop"], "perfect match"
        )
    else:
        if len(interface.list_link()) != 0:
            print("You are missing livestock to crop interface")
    ...
    DM_crop_prod, dm_crop_ch, dm_feed_processed, dm_food_processed = crop_workflow(
        DM_crop_prod,
        CDM_const,
        dm_feed_processed,
        dm_feed_unprocessed,
    ...
    # crop to land-use
    DM_crop_landuse = DM_crop_prod["crop"].filter(
    ...
    interface.add_link(from_sector="crop", to_sector="land-use", dm=DM_crop_landuse)
    ...
    # crop to TCAF
    DM_crop_to_TCAF = DM_crop_prod["crop"].filter(
        {"Variables": ["agr_domestic-production_afw"]}
    )
    if write_pickle is True:
        current_file_directory = os.path.dirname(os.path.abspath(__file__))
...
def crop_local_run():
    ...
    DM_input = filter_country_and_load_data_from_pickles(
        country_list=country_list, modules_list="crop", filter_country=False
    )
