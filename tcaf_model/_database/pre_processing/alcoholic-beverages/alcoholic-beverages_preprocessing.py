...
def bev_calibration(list_countries_calc, dm_fxa_pro_yield, cdm_bev):
    ...
    df_dict_calibration = pd.read_excel(
        "dictionaries/dictionnary_alcoholic-beverages.xlsx", sheet_name="calibration"
    )
    ...
    array_temp = (
        ...
        * dm_fxa_pro_yield_temp[
            "Switzerland", :, "fxa_agr_processing-yield", "wine-to-fruit"
        ]
    )
    # Overwrite
...
def trade_origin_processing(years_ots, list_countries_calc, file_dict):
  ...
  df_trade_agg.rename(columns={'Partner Country Group': 'geoscale',
    ...
    cdm_kcal_temp = cdm_kcal_temp.filter(
        {"Categories1": dm_bev_trade_origin.col_labels["Categories1"]}
    )
    dm_bev_trade_origin.sort("Categories1")
    cdm_kcal_temp.sort("Categories1")
    array_temp = (
        dm_bev_trade_origin[:, :, "agr_split-import", :]
...
def fxa_processing_yield(cdm_kcal):
    ...
    # Read excel file
    df_dict = pd.read_excel(
        "dictionaries/dictionnary_alcoholic-beverages.xlsx", sheet_name="fxa"
    )
...
def constant():
    ...
    df_kcal_t = pd.read_excel(
        "data/alcoholic-beverages_constants.xlsx", sheet_name="cp_kcal_t"
    )
    ...
    df_cp_bev = pd.read_excel(
        "data/alcoholic-beverages_constants.xlsx", sheet_name="cp_ibp_bev"
    )
...
def fts_processing(list_countries_calc, years_ots, years_fts, cdm_kcal):
    ...
    # Read Excel
    df_fts_data = pd.read_excel("data/alcoholic-beverages_fts.xlsx", sheet_name="fts")
...
dm_bev_trade_origin, dm_cal_imports_countries, dm_cal_imports_tot = (
...
dm_match_countries(
    dm_cal_imports_countries, dm_fxa_pro_yield, parameter="perfect match"
)
