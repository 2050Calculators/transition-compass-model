...
def self_sufficiency_processing(years_ots, list_countries_calc, file_dict):
    ...
    try:
        ...
        df_feed_voil_2010_2022.loc[
            df_feed_voil_2010_2022["Element"].str.contains(
                "Production Quantity", case=False, na=False
            ),
            "Element",
        ] = "Production"
    ...
    pivot_df_feed_pro = df_ssr_feed_pro.pivot_table(
        index=["Area", "Year", "Item"], columns="Element", values="Value"
    ).reset_index()
    ...
    pivot_df_feed_pro["SSR[%]"] = pivot_df_feed_pro["Production"] / (
        pivot_df_feed_pro["Food"]
        + pivot_df_feed_pro["Feed"]
        + pivot_df_feed_pro["Processing"]
    )
    ...
    df_dict_ssr = pd.read_excel(
        "dictionaries/dictionnary_livestock.xlsx", sheet_name="self-sufficiency"
    )
    ...
    dm_imports_fbs.rename_col_regex(
        str1="agr_ssr", str2="cal_agr_imported_production_total", dim="Variables"
    )
    dm_imports_fbs.change_unit(
        "cal_agr_imported_production_total", 1.0, "-", "kcal", "*"
    )
...
def exports_processing(list_countries_calc, file_dict):
    ...
    # 1: Pivot the DataFrame to get 'Production', 'Import Quantity', and 'Export Quantity' in separate columns
    pivot_df = df_exports.pivot_table(
        index=["Area", "Year", "Item"], columns="Element", values="Value"
    ).reset_index()

    # Fill na with 0
    cols = ["Production", "Export"]
    ...
    df_dict_exports = pd.read_excel(
        "dictionaries/dictionnary_livestock.xlsx", sheet_name="exports"
    )
...
def trade_origin_asf_processing(years_ots, list_countries_calc, file_dict):
    ...
    try:
        ...
        list_items_egg = [
            k
            for k in dict_items_faostat.keys()
        ...
        exclude = ["juice", "hydrogenated", "acids", "cocoa", "meal", "potato"]
        list_items_meat = [
            k
            for k in dict_items_faostat.keys()
            if any(word in k.lower() for word in keywords)
        ...
        keywords = ["milk", "yog", "cream", "cheese", "butter", "ghee", "curd", "whey"]
        exclude = ["wheat", "soy", "almond", "cocoa", "peanut", "karite"]
        list_items_milk = [
            k
            for k in dict_items_faostat.keys()
            if any(word in k.lower() for word in keywords)
        ...
        dict_item_groups = {
            # ASF
            "pro-liv-abp-hens-egg": list_items_egg,
            "pro-liv-abp-milk": list_items_milk,
            "pro-liv-meat": list_items_meat,
        }
        ...
        code = "TM"
        pars = faostat.list_pars(code)
        my_reporter_countries = [
            faostat.get_par(code, "reporters")[c] for c in list_countries_calc
        ]
        my_partner_regions = [
            faostat.get_par(code, "partners")[p] for p in list_partnerregions
        ]
        my_elements = [faostat.get_par(code, "elements")[e] for e in list_elements]
        ...
        my_pars_egg = {
            "reporters": my_reporter_countries,
            "partners": my_partner_regions,
            "element": my_elements,
            "item": my_items_egg,
            "year": my_years,
        }
    ...
    df_trade_egg = df_trade_egg.groupby(
        ["Reporter Countries", "Partner Countries", "Item", "Year"], as_index=False
    )["Value"].sum()
    df_trade_milk["Item"] = "Milk"
    df_trade_milk = df_trade_milk.groupby(
        ["Reporter Countries", "Partner Countries", "Item", "Year"], as_index=False
    )["Value"].sum()
    ...
    for key, value in mapping.items():
        mask = df_trade["Item"].str.contains(key, case=False, na=False)
    ...
    df_trade_agg = df_trade_agg.groupby(
        ["variables", "Partner Countries", "Reporter Countries", "Year"], as_index=False
    )["Value"].sum()
  ...
  df_trade_agg.rename(columns={'Partner Country Group': 'geoscale',
    ...
    # Filter years
    dm_liv_trade_origin.filter({"Years": years_all}, inplace=True)
    ...
    cdm_kcal_temp.sort("Categories1")
    array_temp = (
        dm_liv_trade_origin[:, :, "agr_split-import", :]
        * cdm_kcal_temp[np.newaxis, np.newaxis, "cp_kcal-per-t", :]
    )
    ...
    dm_cal_imports_countries.rename_col(
        "agr_split-import", "cal_agr_domestic-production", "Variables"
    )
    dm_cal_imports_countries.change_unit(
        "cal_agr_domestic-production", 1.0, "-", "kcal", "*"
    )
    ...
    dm_cal_imports_tot.rename_col(
        "agr_split-import", "cal_agr_imported_production_total", "Variables"
    )
    dm_cal_imports_tot.change_unit(
        "cal_agr_imported_production_total", 1.0, "-", "kcal", "*"
    )
    dm_cal_imports_tot.groupby(
        {"Switzerland": ".*"}, dim="Country", regex=True, inplace=True
    )
...
def trade_origin_feed_processing(years_ots, list_countries_calc, file_dict):
    ...
    try:
        df_trade_molasses = pd.read_csv(file_dict["trade-feed-molasses"])
        df_trade_cakes = pd.read_csv(file_dict["trade-feed-cakes"])
    except OSError:
        # TRADE MATRIX (TM)
        # List of elements
        list_elements = ["Import quantity"]
        ...
        pars = faostat.list_pars(code)
        my_reporter_countries = [
            faostat.get_par(code, "reporterarea")[c] for c in list_countries_calc
        ]
        my_partner_regions = [
            faostat.get_par(code, "partnerregions")[p] for p in list_partnerregions
        ]
        my_elements = [faostat.get_par(code, "elements")[e] for e in list_elements]
        list_years = [
            "1990",
        ...
        for group_name, item_list in dict_item_groups.items():
            my_items = [faostat.get_par(code, "item")[i] for i in item_list]
    ...
    col_filter = ["Reporter Countries", "Partner Countries", "Item", "Year", "Value"]
    df_trade_molasses = df_trade_molasses[col_filter]
    df_trade_cakes = df_trade_cakes[col_filter]

    # Sum items for cakes
    df_trade_cakes["Item"] = "Cakes"
    df_trade_cakes = df_trade_cakes.groupby(
        ["Reporter Countries", "Partner Countries", "Item", "Year"], as_index=False
    )["Value"].sum()

    # Change variables name
    df_trade_cakes["Item"] = "pro-crop-processed-cake"
    df_trade_molasses["Item"] = "pro-crop-processed-molasse"

    # Concat dfs
    df_trade_agg = pd.concat([df_trade_molasses, df_trade_cakes])

    # Rename Item as variables
    df_trade_agg.rename(columns={"Item": "variables"}, inplace=True)

    # Prepend var name and unit
    df_trade_agg["variables"] = df_trade_agg["variables"].apply(
        lambda x: f"agr_split-import_{x}[-]"
    )
  ...
  df_trade_agg.rename(columns={'Partner Country Group': 'geoscale',
    ...
    # Filter years
    dm_feed_trade_origin.filter({"Years": years_all}, inplace=True)
    ...
    cdm_kcal_temp.rename_col_regex(str1="pro-liv-", str2="", dim="Categories1")
    cdm_kcal_temp = cdm_kcal_temp.filter(
        {"Categories1": dm_feed_trade_origin.col_labels["Categories1"]}
    )
    dm_feed_trade_origin.sort("Categories1")
    cdm_kcal_temp.sort("Categories1")
    array_temp = (
        dm_feed_trade_origin[:, :, "agr_split-import", :]
        * cdm_kcal_temp[np.newaxis, np.newaxis, "cp_kcal-per-t", :]
    )
    ...
    dm_cal_imports_feed_countries.rename_col(
        "agr_split-import", "cal_agr_domestic-production", "Variables"
    )
    dm_cal_imports_feed_countries.change_unit(
        "cal_agr_domestic-production", 1.0, "-", "kcal", "*"
    )
    ...
    dm_cal_imports_feed_tot.rename_col(
        "agr_split-import", "cal_agr_imported_production_total", "Variables"
    )
    dm_cal_imports_feed_tot.change_unit(
        "cal_agr_imported_production_total", 1.0, "-", "kcal", "*"
    )
    dm_cal_imports_feed_tot.groupby(
        {"Switzerland": ".*"}, dim="Country", regex=True, inplace=True
    )
...
def production_share(dm_cal_liv_pop):
    ...
    try:
        ...
        # Format
        dm.rename_col("Suisse", "Switzerland", "Country")
        dm.rename_col(
            "Zone de production agricole - total", "agr_livestock", "Variables"
        )
        dm.rename_col("Système d'exploitation - total", "total", "Categories2")
        dm.rename_col("Exploitations biologiques", "organic", "Categories2")
        dm.rename_col_regex("Cheptel - ", "", dim="Categories1")
        dict_cat = {
            "abp-dairy-milk": ["Vaches laitières"],
    ...
    dm_temp = dm_cattle.filter(
        {"Years": years_temp, "Categories1": ["abp-dairy-milk", "other-cattle"]},
        inplace=False,
    )
    ...
    for cat in dm_cattle.col_labels["Categories1"]:
        dm_cattle[:, :, "agr_livestock", cat, :] = (
            lsu_conversion[cat] * dm_cattle[:, :, "agr_livestock", cat, :]
        )
    ...
    try:
        ...
        dm.rename_col_regex("Cheptel - ", "", dim="Categories1")
        dict_cat = {
            "abp-hens-egg": ["Poules de ponte et d'élevage"],
            "meat-poultry": ["Poulets de chair"],
            "meat-other-poultry": ["Dindes", "Canards", "Oies"],
        }
        dm.groupby(dict_cat, dim="Categories1", inplace=True)
        dm.sort("Years")
        dm.filter({"Years": years_ots}, inplace=True)
    ...
    for cat in dm_poultry.col_labels["Categories1"]:
        dm_poultry[:, :, "agr_livestock", cat, :] = (
            lsu_conversion[cat] * dm_poultry[:, :, "agr_livestock", cat, :]
        )
    dm_poultry.change_unit(
        "agr_livestock", old_unit="animals", new_unit="lsu", factor=1
    )

    # Sum poultry meat = young-cattle + other-cattle + other-bovins
    dm_poultry.groupby(
        {"meat-poultry": ["meat-poultry", "meat-other-poultry"]},
        dim="Categories1",
        inplace=True,
    )
    ...
    for cat in dm_others.col_labels["Categories1"]:
        dm_others[:, :, "agr_livestock", cat, :] = (
            lsu_conversion[cat] * dm_others[:, :, "agr_livestock", cat, :]
        )
    dm_others.change_unit("agr_livestock", old_unit="animals", new_unit="lsu", factor=1)

    # Group Horses and other animals
    dm_others.groupby(
        {"meat-oth-animal": ["meat-horse", "meat-oth-animal"]},
        dim="Categories1",
        inplace=True,
    )
    ...
    dm_cal_liv_pop_org.rename_col_regex(
        "agr_livestock", "cal_agr_liv-population_organic", dim="Variables"
    )
    ...
    dm_prod_share.operation(
        "agr_livestock_organic",
        "/",
        "agr_livestock_total",
        out_col="livestock_share-organic",
        unit="-",
    )
...
def livestock_emissions(file_dict):
    ...
    try:
        ...
        df_enteric = faostat.get_data_df(code, pars=my_pars, strval=False)

    # Renaming item as the same animal (for meat and live/producing/slaugthered animals)
    df_enteric.loc[
        df_enteric["Item"].str.contains("Cattle, dairy", case=False, na=False), "Item"
    ] = "Dairy cows"
    df_enteric.loc[
        df_enteric["Item"].str.contains("Cattle, non-dairy", case=False, na=False),
        "Item",
    ] = "Cattle"
    df_enteric.loc[
        df_enteric["Item"].str.contains("Goat", case=False, na=False), "Item"
    ] = "Goat"
    df_enteric.loc[
        df_enteric["Item"].str.contains("Chickens, broilers", case=False, na=False),
        "Item",
    ] = "Chicken"
    df_enteric.loc[
        df_enteric["Item"].str.contains("Chickens, layers", case=False, na=False),
        "Item",
    ] = "Chicken laying hens"
    df_enteric.loc[
        df_enteric["Item"].str.contains("Duck", case=False, na=False), "Item"
    ] = "Duck"
    df_enteric.loc[
        df_enteric["Item"].str.contains("Horse", case=False, na=False), "Item"
    ] = "Horse"
    df_enteric.loc[
        df_enteric["Item"].str.contains("Sheep", case=False, na=False), "Item"
    ] = "Sheep"
    df_enteric.loc[
        df_enteric["Item"].str.contains("Swine", case=False, na=False), "Item"
    ] = "Pig"
    df_enteric.loc[
        df_enteric["Item"].str.contains("Turkey", case=False, na=False), "Item"
    ] = "Turkey"
    df_enteric.loc[
        df_enteric["Item"].str.contains("Asse", case=False, na=False), "Item"
    ] = "Asse"
    df_enteric.loc[
        df_enteric["Item"].str.contains("Buffalo", case=False, na=False), "Item"
    ] = "Buffalo"
    df_enteric.loc[
        df_enteric["Item"].str.contains("Mule", case=False, na=False), "Item"
    ] = "Mule"
    df_enteric.loc[
        df_enteric["Item"].str.contains("Camel", case=False, na=False), "Item"
    ] = "Other non-specified"
    ...
    # Merging
    df_enteric = pd.merge(df_enteric, df_lsu, on="Item")

    # Converting Animals to lsu
    condition = df_enteric["Unit"] == "An"
    df_enteric.loc[condition, "Value"] *= df_enteric["lsu"]

    # Aggregating
    df_enteric_grouped = df_enteric.groupby(
        ["Aggregation", "Area", "Year", "Element", "Unit"], as_index=False
    )["Value"].sum()

    # Pivot the df
    pivot_df = df_enteric_grouped.pivot_table(
    ...
    df_ots, df_fts = database_to_df(df_enteric_pathwaycalc, lever, level="all")
    df_ots = df_ots.drop(columns=[lever])  # Drop column with lever name
    ...
    df_manure_n.loc[
        df_manure_n["Item"].str.contains("Cattle, dairy", case=False, na=False), "Item"
    ] = "Dairy cows"
    df_manure_n.loc[
        df_manure_n["Item"].str.contains("Cattle, non-dairy", case=False, na=False),
        "Item",
    ] = "Cattle"
    df_manure_n.loc[
        df_manure_n["Item"].str.contains("Goat", case=False, na=False), "Item"
    ] = "Goat"
    df_manure_n.loc[
        df_manure_n["Item"].str.contains("Chickens, broilers", case=False, na=False),
        "Item",
    ] = "Chicken"
    df_manure_n.loc[
        df_manure_n["Item"].str.contains("Chickens, layers", case=False, na=False),
        "Item",
    ] = "Chicken laying hens"
    df_manure_n.loc[
        df_manure_n["Item"].str.contains("Duck", case=False, na=False), "Item"
    ] = "Duck"
    df_manure_n.loc[
        df_manure_n["Item"].str.contains("Horse", case=False, na=False), "Item"
    ] = "Horse"
    df_manure_n.loc[
        df_manure_n["Item"].str.contains("Sheep", case=False, na=False), "Item"
    ] = "Sheep"
    df_manure_n.loc[
        df_manure_n["Item"].str.contains("Swine", case=False, na=False), "Item"
    ] = "Pig"
    df_manure_n.loc[
        df_manure_n["Item"].str.contains("Turkey", case=False, na=False), "Item"
    ] = "Turkey"
    df_manure_n.loc[
        df_manure_n["Item"].str.contains("Asse", case=False, na=False), "Item"
    ] = "Asse"
    df_manure_n.loc[
        df_manure_n["Item"].str.contains("Buffalo", case=False, na=False), "Item"
    ] = "Buffalo"
    df_manure_n.loc[
        df_manure_n["Item"].str.contains("Mule", case=False, na=False), "Item"
    ] = "Mule"
    df_manure_n.loc[
        df_manure_n["Item"].str.contains("Camel", case=False, na=False), "Item"
    ] = "Other non-specified"
    ...
    # Aggregating
    df_manure_n = df_manure_n.groupby(
        ["Aggregation", "Area", "Year", "Element", "Unit"], as_index=False
    )["Value"].sum()
    ...
    df_dict_csl = pd.read_excel(
        "dictionaries/dictionnary_livestock.xlsx", sheet_name="climate-smart-livestock"
    )
    ...
    df_ots, df_fts = database_to_df(df_manure_pathwaycalc, lever, level="all")
    df_ots = df_ots.drop(columns=[lever])  # Drop column with lever name
    ...
    df_ots, df_fts = database_to_df(df_fxa_manure_yield, lever, level="all")
    df_ots = df_ots.drop(columns=[lever])  # Drop column with lever name
...
def livestock_losses():
    ...
    try:
        ...
        code = "FBSH"
        my_countries = [
            faostat.get_par(code, "area")[c] for c in list_partnerregions_trade
        ]
        my_elements = [faostat.get_par(code, "elements")[e] for e in list_elements]
        my_items = [faostat.get_par(code, "item")[i] for i in list_items]
        list_years = [
    ...
    df_ots, df_fts = database_to_df(df_losses_csl_pathwaycalc, lever, level="all")
    df_ots = df_ots.drop(columns=[lever])  # Drop column with lever name
...
def feed_ration(df_feed_ration, cdm_efficiency, cdm_kcal):
    ...
    df_ots, df_fts = database_to_df(df_csl_feed_pathwaycalc, lever, level="all")
    df_ots = df_ots.drop(columns=[lever])  # Drop column with lever name
    ...
    # Load
    dm_dom_prod_liv = dm_cal_dom_prod.filter(
        {"Country": list_countries_calc}, inplace=False
    ).copy()
    ...
    dm_grass = dm_dom_prod_liv.filter(
        {"Variables": ["agr_ruminant-feed_share-grass"]}, inplace=False
    )
...
def yield_slaughter_rate(df_liv_pop, dm_prod_share):
    ...
    # ----------------------------------------------------------------------------------------------------------------------
    try:
        df_yield_egg = pd.read_csv(file_dict["QCL_yield_egg"])
        df_yield_milk = pd.read_csv(file_dict["QCL_yield_milk"])
        df_yield_abp = pd.concat([df_yield_egg, df_yield_milk])
        ...
        df_yield_abp = faostat.get_data_df(code, pars=my_pars, strval=False)

    # Keep the rows where Production is not in Nb of Eggs
    df_yield_abp = df_yield_abp[df_yield_abp["Unit"] != "1000 No"]

    # Renaming item as the same animal (for meat and live/producing/slaugthered animals)
    df_yield_abp.loc[
        df_yield_abp["Item"].str.contains("Cattle", case=False, na=False), "Item"
    ] = "Dairy cows"
    df_yield_abp.loc[
        df_yield_abp["Item"].str.contains("Sheep", case=False, na=False), "Item"
    ] = "Dairy sheep"
    df_yield_abp.loc[
        df_yield_abp["Item"].str.contains("Goat", case=False, na=False), "Item"
    ] = "Dairy goat"
    df_yield_abp.loc[
        df_yield_abp["Item"].str.contains("Buffalo", case=False, na=False), "Item"
    ] = "Dairy buffalo"
    df_yield_abp.loc[
        df_yield_abp["Item"].str.contains("Hen eggs", case=False, na=False), "Item"
    ] = "Chicken laying hens"
    df_yield_abp.loc[
        df_yield_abp["Item"].str.contains(
    ...
    df_yield_abp["Value"] = pd.to_numeric(df_yield_abp["Value"], errors="coerce")
    mask = df_yield_abp["Unit"].str.strip() == "1000 An"
    df_yield_abp.loc[mask, "Value"] *= 1000
    df_yield_abp.loc[mask, "Unit"] = "An"
    df_yield_abp = df_yield_abp.copy()
    ...
    # Merging
    df_yield_abp = pd.merge(df_yield_abp, df_lsu, on="Item")

    # Converting Animals to lsu
    condition = (df_yield_abp["Unit"] == "An") | (df_yield_abp["Unit"] == "1000 An")
    df_yield_abp.loc[condition, "Value"] *= df_yield_abp["lsu"]

    # Aggregating
    grouped_df = df_yield_abp.groupby(
        ["Aggregation", "Area", "Year", "Element", "Unit"], as_index=False
    )["Value"].sum()
    ...
    # ----------------------------------------------------------------------------------------------------------------------
    try:
        df_meat_prod = pd.read_csv(file_dict["QCL_prod_meat"])
        df_meat_producing = pd.read_csv(file_dict["QCL_producing_meat"])

        df_yield_meat = pd.concat([df_meat_prod, df_meat_producing])
        ...
        df_yield_meat = faostat.get_data_df(code, pars=my_pars, strval=False)

    # Dropping 'Bees'
    df_yield_meat = df_yield_meat[df_yield_meat["Item"] != "Bees"]

    # Renaming item as the same animal (for meat and live/producing/slaugthered animals)
    df_yield_meat.loc[
        df_yield_meat["Item"].str.contains("Pig", case=False, na=False), "Item"
    ] = "Pig"
    df_yield_meat.loc[
        df_yield_meat["Item"].str.contains("Cattle", case=False, na=False), "Item"
    ] = "Cattle"
    df_yield_meat.loc[
        df_yield_meat["Item"].str.contains("Buffalo", case=False, na=False), "Item"
    ] = "Cattle"
    df_yield_meat.loc[
        df_yield_meat["Item"].str.contains("Chicken", case=False, na=False), "Item"
    ] = "Chicken"
    df_yield_meat.loc[
        df_yield_meat["Item"].str.contains("Duck", case=False, na=False), "Item"
    ] = "Duck"
    df_yield_meat.loc[
        df_yield_meat["Item"].str.contains("Turkeys", case=False, na=False), "Item"
    ] = "Turkey"
    df_yield_meat.loc[
        df_yield_meat["Item"].str.contains("Geese", case=False, na=False), "Item"
    ] = "Goose"
    df_yield_meat.loc[
        df_yield_meat["Item"].str.contains("Pigeon", case=False, na=False), "Item"
    ] = "Pigeon"
    df_yield_meat.loc[
        df_yield_meat["Item"].str.contains("Horse", case=False, na=False), "Item"
    ] = "Horse"
    df_yield_meat.loc[
        df_yield_meat["Item"].str.contains("Rabbits and hares", case=False, na=False),
        "Item",
    ] = "Rabbit"
    df_yield_meat.loc[
        df_yield_meat["Item"].str.contains("Sheep", case=False, na=False), "Item"
    ] = "Sheep"
    df_yield_meat.loc[
        df_yield_meat["Item"].str.contains("Goat", case=False, na=False), "Item"
    ] = "Goat"
    df_yield_meat.loc[
        df_yield_meat["Item"].str.contains("Asse", case=False, na=False), "Item"
    ] = "Asse"
    df_yield_meat.loc[
        df_yield_meat["Item"].str.contains("Camel", case=False, na=False), "Item"
    ] = "Other non-specified"
    df_yield_meat.loc[
        df_yield_meat["Item"].str.contains("Rodent", case=False, na=False), "Item"
    ] = "Other non-specified"
    df_yield_meat.loc[
        df_yield_meat["Item"].str.contains("Other", case=False, na=False), "Item"
    ] = "Other non-specified"
    df_yield_meat.loc[
        df_yield_meat["Item"].str.contains("Game", case=False, na=False), "Item"
    ] = "Game"
    df_yield_meat.loc[
        df_yield_meat["Item"].str.contains("Mule", case=False, na=False), "Item"
    ] = "Mule"

    # HERE! Unit conversion Poultry : [1000 An] => [An]
    df_yield_meat["Value"] = pd.to_numeric(df_yield_meat["Value"], errors="coerce")
    mask = df_yield_meat["Unit"].str.strip() == "1000 An"
    df_yield_meat.loc[mask, "Value"] *= 1000
    df_yield_meat.loc[mask, "Unit"] = "An"
    ...
    # Merging
    df_yield_meat = pd.merge(df_yield_meat, df_lsu, on="Item")

    # Converting Animals to lsu
    condition = (df_yield_meat["Unit"] == "An") | (df_yield_meat["Unit"] == "1000 An")
    df_yield_meat.loc[condition, "Value"] *= df_yield_meat["lsu"]

    # Aggregating
    grouped_df = df_yield_meat.groupby(
        ["Aggregation", "Area", "Year", "Element", "Unit"], as_index=False
    )["Value"].sum()
    ...
    # Replace NaN with 0
    pivot_df_slau["Producing Animals/Slaughtered"] = pivot_df_slau[
    ...
    df_ots, df_fts = database_to_df(df_yield_liv_pathwaycalc, lever, level="all")
    df_ots = df_ots.drop(columns=[lever])  # Drop column with lever name
    dm_liv_yield = DataMatrix.create_from_df(df_ots, num_cat=1)
    dm_liv_yield.filter({"Years": years_ots}, inplace=True)

    # Yield total [kcal/lsu] = Domestic prod with losses [kcal] / producing-slaugthered animals [lsu]
    dm_liv_yield.rename_col(
        "agr_livestock_yield", "agr_livestock_producing", dim="Variables"
    )
    dm_liv_yield.filter(
        {"Country": dm_cal_dom_prod.col_labels["Country"]}, inplace=True
    )
    dm_cal_dom_prod.filter(
        {"Country": dm_liv_yield.col_labels["Country"]}, inplace=True
    )
    dm_liv_yield.append(dm_cal_dom_prod, dim="Variables")
    dm_liv_yield.operation(
    ...
    dm_liv_yield.add(
        0.0, dim="Variables", dummy=True, col_label="agr_livestock_yield_intensive"
    )
    for cat in dm_liv_yield.col_labels["Categories1"]:
        dm_liv_yield[:, :, "agr_livestock_yield_intensive", cat] = dm_cal_dom_prod[
            :, :, "cal_agr_domestic-production-liv", cat
        ] / (
            dm_liv_yield[:, :, "agr_livestock_producing", cat]
            * (
                dm_prod_share[:, :, "livestock_share-organic", cat]
                * (yield_evolution[cat] - 1.0)
                + 1
            )
        )
    ...
    dm_liv_yield.add(
        0.0, dim="Variables", dummy=True, col_label="agr_livestock_yield_organic"
    )
    for cat in dm_liv_yield.col_labels["Categories1"]:
    ...
    df_ots, df_fts = database_to_df(df_slau_liv_pathwaycalc, lever, level="all")
    df_ots = df_ots.drop(columns=[lever])  # Drop column with lever name
...
def livestock_calibration(list_countries_calc, dm_losses):
    ...
    df_dict_calibration = pd.read_excel(
        "dictionaries/dictionnary_livestock.xlsx", sheet_name="calibration"
    )
    ...
    df_domestic_supply_calibration = pd.merge(
        df_dict_calibration, pivot_df_domestic_supply, on="Item"
    )
    ...
    dm_cal_dom_prod.filter(
        {"Variables": ["cal_agr_domestic-production-liv"]}, inplace=True
    )
...
def manure_calibration(list_countries_calc, file_dict):
    ...
    df_liv_emissions_calibration = linear_fitting_ots_db(
        df_liv_emissions_calibration, years_ots, countries="all"
    )
...
def fxa_ffr_milk(df_ffr_milk):
    ...
    # Food item name matching with dictionary
    # Read excel file
    df_dict = pd.read_excel("dictionaries/dictionnary_livestock.xlsx", sheet_name="fxa")
    ...
    # Extrapolation
    df_ffr_milk = linear_fitting_ots_db(df_ffr_milk, years_all, countries="all")
...
def manure_fxa(
    ...
    df_manure_n_fxa = df_manure_n_fxa.rename(
        columns={
...
def feed_calibration(list_countries_calc):
    ...
    try:
        df_feed_FBSH = pd.read_csv(file_dict["FBSH_feed"])
        df_feed_FBS = pd.read_csv(file_dict["FBS_feed"])
        df_feed_CBH_cake = pd.read_csv(file_dict["CBH_cake"])
        df_feed_SCL_molasse_cake = pd.read_csv(file_dict["SCL_molasse_cake"])
        ...
        df_feed_FBSH = faostat.get_data_df(code, pars=my_pars, strval=False)
        ...
        df_feed_FBS = faostat.get_data_df(code, pars=my_pars, strval=False)
        ...
        df_feed_CBH_cake = faostat.get_data_df(code, pars=my_pars, strval=False)
        ...
        df_feed_SCL_molasse_cake = faostat.get_data_df(code, pars=my_pars, strval=False)

    # Renaming the items for name matching
    df_feed_FBSH.loc[
        df_feed_FBSH["Item"].str.contains(
            "Rice (Milled Equivalent)", case=False, regex=False
        ),
        "Item",
    ] = "Rice and products"
    ...
    # Concatenating all the years together
    df_feed = pd.concat([df_feed_FBSH, df_feed_FBS])

    # Aggregating cakes
    df_feed_cake = pd.concat([df_feed_CBH_cake, df_feed_SCL_molasse_cake])
    # Filtering
    ...
    # Filtering for molasse
    df_feed_molasses = df_feed_SCL_molasse_cake[
        df_feed_SCL_molasse_cake["Item"].str.contains("Molasses", case=False)
    ]
    ...
    dm_cal_feed.add(0.0, dim="Categories1", dummy=True, col_label="crop-fruit")
    dm_cal_feed.add(0.0, dim="Categories1", dummy=True, col_label="crop-processed-voil")
...
def constant():
    ...
    # Read excel
    df = pd.read_excel(
        "dictionaries/constants_livestock.xlsx", sheet_name="cp_ibp_processed"
    )
    ...
    # Format as a cdm
    cdm_cp_ibp_feed = ConstantDataMatrix(
        col_labels={
            "Variables": ["cp_ibp_processed"],
            "Categories1": categories1_cleaned,
        }
    )
    arr = np.zeros(
        (
            len(cdm_cp_ibp_feed.col_labels["Variables"]),
            len(cdm_cp_ibp_feed.col_labels["Categories1"]),
        )
    )
    cdm_cp_ibp_feed.array = arr
    idx = cdm_cp_ibp_feed.idx
    for cat, val in dict_temp.items():
        cdm_cp_ibp_feed.array[idx["cp_ibp_processed"], idx[cat]] = val
    cdm_cp_ibp_feed.units["cp_ibp_processed"] = "-"

    # OFFAL YIELDS -------------------------------------------------------
    # Read excel
    df_offal = pd.read_excel(
        "dictionaries/constants_livestock.xlsx", sheet_name="cp_ibp_offal"
    )
    ...
    # Format as a cdm
    cdm_cp_ibp_offal = ConstantDataMatrix(
        col_labels={"Variables": ["cp_ibp_liv"], "Categories1": categories1}
    )
    arr = np.zeros(
        (
            len(cdm_cp_ibp_offal.col_labels["Variables"]),
    ...
    # AFAT YIELDS -------------------------------------------------------
    # Read excel
    df_afat = pd.read_excel(
        "dictionaries/constants_livestock.xlsx", sheet_name="cp_ibp_afat"
    )
    ...
    # Format as a cdm
    cdm_cp_ibp_afat = ConstantDataMatrix(
        col_labels={"Variables": ["cp_ibp_liv"], "Categories1": categories1}
    )
    arr = np.zeros(
        (
            len(cdm_cp_ibp_afat.col_labels["Variables"]),
    ...
    # Read excel
    df_feed_conv = pd.read_excel(
        "dictionaries/constants_livestock.xlsx", sheet_name="cp_feed_efficiency"
    )
    ...
    # Format as a cdm
    cdm_efficiency = ConstantDataMatrix(
        col_labels={"Variables": ["cp_efficiency_liv"], "Categories1": categories1}
    )
    arr = np.zeros(
        (
            len(cdm_efficiency.col_labels["Variables"]),
    ...
    return cdm_cp_ibp_feed, cdm_efficiency, cdm_kcal, cdm_cp_ibp_afat, cdm_cp_ibp_offal
...
def fts_processing():
    ...
    df_fts_data = df_fts_data[
        ["variables", "timescale", "geoscale", "level", "value", "lever"]
    ]
...
def datamatrix_to_pickle(dm_fts):
    ...
    # ssr-liv_.*
    dict_ots["ssr-liv-abp-dairy-milk"] = dm_ssr_liv.filter(
        {"Categories1": ["pro-liv-abp-dairy-milk"]}
    )
    dict_ots["ssr-liv-abp-hens-egg"] = dm_ssr_liv.filter(
        {"Categories1": ["pro-liv-abp-hens-egg"]}
    )
    dict_ots["ssr-liv-meat-poultry"] = dm_ssr_liv.filter(
        {"Categories1": ["pro-liv-meat-poultry"]}
    )
    dict_ots["ssr-liv-meat-pig"] = dm_ssr_liv.filter(
        {"Categories1": ["pro-liv-meat-pig"]}
    )
    dict_ots["ssr-liv-meat-bovine"] = dm_ssr_liv.filter(
        {"Categories1": ["pro-liv-meat-bovine"]}
    )
    dict_ots["ssr-liv-meat-sheep"] = dm_ssr_liv.filter(
        {"Categories1": ["pro-liv-meat-sheep"]}
    )
    dict_ots["ssr-liv-meat-oth-animal"] = dm_ssr_liv.filter(
        {"Categories1": ["pro-liv-meat-oth-animal"]}
    )
    # ssr-feed-pro
  ...
  dict_fts[lever] = dm_fts[lever]"""
    ...
    for level in range(2, 5):
        ...
        array_temp = (
            dm_fts[lever][level][
                :, years_fts[-1], "livestock_share-organic", np.newaxis
            ]
    ...
    dict_const["cdm_ibp_feed"] = cdm_cp_ibp_feed
...
file_dict = {
    ...
    "trade_egg": "data/faostat/TM_csv/TM_asf-egg.csv",
    "trade_milk": "data/faostat/TM_csv/TM_asf-milk.csv",
    "trade_meat": "data/faostat/TM_csv/TM_asf-meat.csv",
    "trade-feed-molasses": "data/faostat/TM_csv/TM_feed-molasses.csv",
    "trade-feed-cakes": "data/faostat/TM_csv/TM_feed-cakes.csv",
    ...
    "CBH_cake": "data/faostat/CBH_csv/CBH_feed_cake.csv",
    "QCL_yield_egg": "data/faostat/QCL_csv/QCL_yield_egg.csv",
    "QCL_yield_milk": "data/faostat/QCL_csv/QCL_yield_milk.csv",
    "QCL_prod_meat": "data/faostat/QCL_csv/QCL_prod_meat.csv",
    "QCL_producing_meat": "data/faostat/QCL_csv/QCL_producing-livestock_meat.csv",
    "trade-bev": "data/faostat/trade-test.csv",
    "exports": "data/faostat/exports.csv",
}

cdm_cp_ibp_feed, cdm_efficiency, cdm_kcal, cdm_cp_ibp_afat, cdm_cp_ibp_offal = (
    constant()
)
dm_ssr_liv, dm_ssr_feed, df_csl_feed, df_ffr_milk, dm_imports_fbs = (
    self_sufficiency_processing(years_ots, list_countries_calc, file_dict)
)
dm_fxa_ffr_milk = fxa_ffr_milk(df_ffr_milk)
dm_liv_trade_origin, dm_cal_imports_countries, dm_cal_imports_tot = (
    trade_origin_asf_processing(years_ots, list_countries_calc, file_dict)
)
dm_feed_trade_origin, dm_cal_imports_feed_tot = trade_origin_feed_processing(
    years_ots, list_countries_calc, file_dict
)
...
dm_cal_liv_emissions, df_liv_emissions = manure_calibration(
    list_countries_calc, file_dict
)
dm_fxa_CH4, dm_fxa_N2O = manure_fxa(
    list_countries_calc, df_liv_emissions, df_manure_n_fxa, df_manure_ch4_fxa
)
