...
def crop_losses():
    ...
    df_dict_csc = pd.read_excel(
        "dictionaries/dictionary_crop.xlsx", sheet_name="climate-smart-crops"
    )
    ...
    df_ots, df_fts = database_to_df(df_losses_pathwaycalc, lever, level="all")
...
def crop_yield(dm_prod_share):
    ...
    df_ots, df_fts = database_to_df(df_yield_pathwaycalc, lever, level="all")
    df_ots = df_ots.drop(columns=[lever])  # Drop column with lever name
    ...
    dm_yield_ch.add(
        0.0, dim="Variables", dummy=True, col_label="agr_crop_yield_intensive"
    )
    for cat in dm_yield_ch.col_labels["Categories1"]:
        dm_yield_ch["Switzerland", :, "agr_crop_yield_intensive", cat] = dm_yield_ch[
            "Switzerland", :, "agr_crop_yield_total", cat
        ] / (
            dm_prod_share["Switzerland", :, "agr_share_intensive", cat]
            + dm_prod_share["Switzerland", :, "agr_share_organic", cat]
            * yield_evolution_o[cat]
            + dm_prod_share["Switzerland", :, "agr_share_extensive", cat]
            * yield_evolution_e[cat]
        )

    # Organic yield_c [kcal/lsu] =  yield_i [kcal/lsu] * yield evolution_o/i [-]
    dm_yield_ch.add(
        0.0, dim="Variables", dummy=True, col_label="agr_crop_yield_organic"
    )
    for cat in dm_yield_ch.col_labels["Categories1"]:
        dm_yield_ch[:, :, "agr_crop_yield_organic", cat] = (
            dm_yield_ch[:, :, "agr_crop_yield_intensive", cat] * yield_evolution_o[cat]
        )

    # Extensive yield_c [kcal/lsu] =  yield_i [kcal/lsu] * yield evolution_o/i [-]
    dm_yield_ch.add(
        0.0, dim="Variables", dummy=True, col_label="agr_crop_yield_extensive"
    )
    for cat in dm_yield_ch.col_labels["Categories1"]:
        dm_yield_ch[:, :, "agr_crop_yield_extensive", cat] = (
            dm_yield_ch[:, :, "agr_crop_yield_intensive", cat] * yield_evolution_e[cat]
        )
...
def crop_calibration(list_countries_calc, dm_losses, dm_fxa_pro_yield, cdm_bev):
    ...
    df_dict_calibration = pd.read_excel(
        "dictionaries/dictionary_crop.xlsx", sheet_name="calibration"
    )
    ...
    array_temp_cereal = (
        dm_cal_dom_prod_bev[:, :, "cal_agr_domestic-production_bev", "crop-cereal"]
        + dm_cal_dom_prod_crop[:, :, "cal_agr_domestic-production", "crop-cereal"]
    )
    dm_cal_dom_prod_crop[:, :, "cal_agr_domestic-production", "crop-cereal"] = (
        array_temp_cereal
    )
    array_temp_fruit = (
        dm_cal_dom_prod_bev[:, :, "cal_agr_domestic-production_bev", "crop-fruit"]
        + dm_cal_dom_prod_crop[:, :, "cal_agr_domestic-production", "crop-fruit"]
    )
    dm_cal_dom_prod_crop[:, :, "cal_agr_domestic-production", "crop-fruit"] = (
        array_temp_fruit
    )
...
def cropland_calibration(list_countries):
    ...
    try:
        ...
        df_cropland.loc[
            df_cropland["Item"].str.contains("Rice", case=False, na=False), "Item"
        ] = "Rice and products"
    ...
    # Extrapolation
    df_cropland_pathwaycalc = linear_fitting_ots_db(
        df_cropland_pathwaycalc, years_ots, countries="all"
    )

    # Format as datamatrix
    df_ots, df_fts = database_to_df(df_cropland_pathwaycalc, lever, level="all")
    df_ots = df_ots.drop(columns=[lever])  # Drop column with lever name
...
def fxa_processing_yield(df_processing_yield_fxa, cdm_kcal):
    ...
    # Food item name matching with dictionary
    # Read excel file
    df_dict = pd.read_excel("dictionaries/dictionary_crop.xlsx", sheet_name="fxa")
...
def self_sufficiency_processing(years_ots, list_countries_calc, file_dict):
    ...
    df_dict_ssr = pd.read_excel(
        "dictionaries/dictionary_crop.xlsx", sheet_name="self-sufficiency"
    )
    ...
    cdm_kcal_temp = cdm_kcal_temp.filter({"Categories1": list_cat_imports})
...
def exports_processing(list_countries_calc, file_dict):
    ...
    df_dict_exports = pd.read_excel(
        "dictionaries/dictionary_crop.xlsx", sheet_name="exports"
    )
...
def production_share():
    ...
    try:
        ...
        dm_crop_area.drop(
            dim="Categories1", col_label=["Exploitations", "SAU - Total (en ha)"]
        )
        dm_crop_area.rename_col_regex("SAU - ", "", dim="Categories1")
        dm_crop_area.rename_col_regex(" (en ha)", "", dim="Categories1")
        dm_crop_area.rename_col("Suisse", "Switzerland", "Country")
        dm_crop_area.rename_col(
            "Zone de production agricole - total", "agr_land-use", dim="Variables"
        )
        dm_crop_area.rename_col(
            "Système d'exploitation - total", "total", "Categories2"
        )
        dm_crop_area.rename_col("Exploitations biologiques", "organic", "Categories2")
        dm_crop_area.rename_col(
            "Exploitations conventionnelles", "intensive", "Categories2"
        )
    ...
    dm_cal_crop_area = dm_crop_area.filter(
        {"Categories2": ["intensive", "extensive", "organic"]}
    )
    ...
    dm_prod_share.operation(
        "agr_land-use_organic",
        "/",
        "agr_land-use_total",
        out_col="agr_share_organic",
        unit="-",
    )
    dm_prod_share.operation(
        "agr_land-use_extensive",
        "/",
        "agr_land-use_total",
        out_col="agr_share_extensive",
        unit="-",
    )
    dm_prod_share.operation(
        "agr_land-use_intensive",
        "/",
        "agr_land-use_total",
        out_col="agr_share_intensive",
        unit="-",
    )

    # Filter
    dm_prod_share.filter(
        {
            "Variables": [
                "agr_share_organic",
                "agr_share_extensive",
...
def constant():
    ...
    arr = np.zeros(
        (
            len(cdm_cp_ibp.col_labels["Variables"]),
    ...
    # Read excel
    df_cp_bev = pd.read_excel("data/crop_constants.xlsx", sheet_name="cp_ibp_bev")
...
def datamatrix_to_pickle(dm_fts):
    ...
    dict_fxa["split-import-crop"] = dm_crop_trade_origin.filter_w_regex(
        {"Categories1": "crop.*"}
    )
    dict_fxa["split-import-crop-pro"] = dm_crop_trade_origin.filter_w_regex(
        {"Categories1": "pro-crop.*"}
    )
    ...
    dict_fxa["cal_agr_imports-crop-pro_total"] = dm_cal_imports_tot.filter_w_regex(
        {"Categories1": "pro-crop.*"}
    )
    ...
    dict_ots["ssr-crop-sugarcrop"] = dm_ssr_crop.filter(
        {"Categories1": ["crop-sugarcrop"]}
    )
    ...
    dict_ots["ssr-bev-bev-alc"] = dm_ssr_crop.filter(
        {"Categories1": ["pro-bev-bev-alc"]}
    )
    dict_ots["ssr-bev-bev-fer"] = dm_ssr_crop.filter(
        {"Categories1": ["pro-bev-bev-fer"]}
    )
    ...
    # crop-share-.*
    dict_ots["crop-share-organic"] = dm_prod_share.filter(
        {"Variables": ["agr_share_organic"]}
    )
    dict_ots["crop-share-extensive"] = dm_prod_share.filter(
        {"Variables": ["agr_share_extensive"]}
    )
    ...
    for level in range(2, 5):
        ...
        array_temp = (
            dm_fts[lever][level][:, years_fts[-1], "agr_share_intensive", np.newaxis]
    ...
    for level in range(2, 5):
        ...
        dm_fts_temp = dm_fts[lever][level]
        array_temp = (
            dm_fts[lever][level][:, years_fts[-1], "agr_share_extensive", np.newaxis]
    ...
    for level in range(2, 5):
        ...
        dm_fts_temp = dm_fts[lever][level]
        array_temp = (
            dm_fts[lever][level][:, years_fts[-1], "agr_share_organic", np.newaxis]
...
dm_ssr_crop, df_processing_yield_fxa, dm_imports_fbs = self_sufficiency_processing(
...
dm_cal_dom_prod_crop, dm_cal_dom_prod_bev = crop_calibration(
    list_countries_calc, dm_losses, dm_fxa_pro_yield, cdm_bev
)
