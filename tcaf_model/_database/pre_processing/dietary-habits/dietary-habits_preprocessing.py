...
def diet_processing(list_countries_calc, cdm_kcal, dm_kcal_req):
    ...
    try:
        ...
        my_countries = [
            faostat.get_par(code, "area")[c] for c in list_countries_calc
    ...
    pivot_df_diet = pivot_df_consumers_diet.drop(
    ...
    df_dict_diet = pd.read_excel(
        "dictionaries/dictionnary_dietary-habits.xlsx", sheet_name="diet_lifestyle"
    )
    ...
    array_temp = (
        10**3
        * dm_diet_share[:, :, "lfs_consumers-diet", :]
    ...
    for lever in df_diet_pathwaycalc["lever"].unique():
        # Format as dm
        df_fts_filtered = df_diet_pathwaycalc[df_diet_pathwaycalc["lever"] == lever]
        ...
        cdm_kcal_diet = cdm_kcal_copy.filter({"Categories1": food_cat})
        # Unit conversion: [kt] => [kcal]
        array_temp = (
            10**3
            * dm_diet_kcal[lever][:, :, "lfs_consumers-diet", :]
            * cdm_kcal_diet[np.newaxis, np.newaxis, "cp_kcal-per-t", :]
        )
        ...
        dm_diet_kcal[lever][:, :, "lfs_consumers-diet", :] = array_temp
        dm_diet_kcal[lever].change_unit(
            "lfs_consumers-diet", old_unit="-", new_unit="kcal/cap/day", factor=1
        )
    ...
    dm_kcal_req.operation(
        ...
        "*",
        "lfs_demography",
        out_col="agr_kcal-req_req",
        unit="kcal/day",
    )
    dm_kcal_req.normalise("Categories1", keep_original=True)

    # Filter for same countries
    dm_diet_share_temp.filter(
        {"Country": dm_kcal_req.col_labels["Country"]}, inplace=True
    )
    ...
    arr = (
        dm_diet_share_temp[:, :, "lfs_consumers-diet_tot", np.newaxis]
        * dm_kcal_req[:, :, "agr_kcal-req_req_share", :]
    )
    dm_kcal_req.add(arr, dim="Variables", col_label="demand_per_group", unit="kcal/day")
...
def dietaryhabits_calibration(list_countries_calc, cdm_kcal):
    ...
    df_dict_calibration = pd.read_excel(
        "dictionaries/dictionnary_dietary-habits.xlsx", sheet_name="calibration"
    )
    ...
    df_diet_calibration.rename(
        columns={"Area": "Country", "Year": "Years", "Food": "value"}, inplace=True
    )

    # Change data type of timescale to int
    df_diet_calibration["Years"] = pd.to_numeric(
        df_diet_calibration["Years"], errors="coerce"
    )

    # Format as datamatrix
    df_pivot = df_diet_calibration.pivot_table(
...
def crop_calibration(list_countries_calc, dm_fxa_pro_yield, cdm_bev):
    ...
    df_cal_dom_prod_crop = df_cal_dom_prod[
        ~df_cal_dom_prod["variables"].str.contains(
            "cal_agr_domestic-production_bev", case=False, na=False
        )
    ...
    array_temp = (
        dm_cal_dom_prod_bev[:, :, "cal_agr_domestic-production_bev", "wine"]
        * dm_fxa_pro_yield_temp[:, :, "fxa_agr_processing-yield", "wine-to-fruit"]
    )
    # Overwrite
    dm_cal_dom_prod_bev["Switzerland", :, "cal_agr_domestic-production_bev", "wine"] = (
        array_temp
    )

    # Bev-alc : Raw materials [kcal] = product [kcal] * processing yield [%]
    array_temp = (
        dm_cal_dom_prod_bev[:, :, "cal_agr_domestic-production_bev", "bev-alc"]
        * cdm_bev[
    ...
    array_temp = (
        dm_cal_dom_prod_bev[:, :, "cal_agr_domestic-production_bev", "bev-fer"]
        * cdm_bev[
    ...
    array_temp = (
        dm_cal_dom_prod_bev[:, :, "cal_agr_domestic-production_bev", "bev-beer"]
        * cdm_bev[np.newaxis, np.newaxis, "cp_ibp_bev_beer_brf_crop_cereal", np.newaxis]
    )
    # Overwrite
    dm_cal_dom_prod_bev[
    ...
    array_temp_cereal = (
        dm_cal_dom_prod_bev[:, :, "cal_agr_domestic-production_bev", "cereal"]
        + dm_cal_dom_prod_crop[:, :, "cal_agr_domestic-production", "cereal"]
    )
    dm_cal_dom_prod_crop[:, :, "cal_agr_domestic-production", "cereal"] = (
        array_temp_cereal
    )
    array_temp_fruit = (
        dm_cal_dom_prod_bev[:, :, "cal_agr_domestic-production_bev", "fruit"]
        + dm_cal_dom_prod_crop[:, :, "cal_agr_domestic-production", "fruit"]
    )
    dm_cal_dom_prod_crop[:, :, "cal_agr_domestic-production", "fruit"] = (
        array_temp_fruit
    )
...
def health_processing():
    ...
    df_food_health.rename(
        columns={"median": "value", "year": "timescale"}, inplace=True
    )
    lever = "dummy"
    ...
    dm_food_health.operation(
        "median_whole-grains",
        "+",
        "median_refined-grains",
        out_col="median_total-grains",
        unit="g/cap/day",
    )
    dm_food_health.operation(
        "median_whole-grains",
        "/",
        "median_total-grains",
        out_col="lfs_share_crop-cereal-whole",
        unit="-",
    )

    # Compute share of unprocessed meat
    dm_food_health.operation(
        "median_processed-meat",
        "+",
        "median_unprocessed-red-meat",
        out_col="median_total-meat",
        unit="g/cap/day",
    )
    dm_food_health.operation(
        "median_unprocessed-red-meat",
        "/",
        "median_total-meat",
        out_col="lfs_share_unprocessed-meat",
        unit="-",
    )

    # Filter dm
    dm_food_health.filter(
        {"Variables": ["lfs_share_crop-cereal-whole", "lfs_share_unprocessed-meat"]},
        inplace=True,
    )
...
def ssr_beverages_processing():
    ...
    try:
        ...
        list_items = ["Beer", "Beverages, Alcoholic", "Beverages, Fermented", "Wine"]
    ...
    df_dict = pd.read_excel(
        "dictionaries/dictionnary_dietary-habits.xlsx", sheet_name="self-sufficiency"
    )
    ...
    # Renaming existing columns (geoscale, timsecale, value)
    df_ssr_bev.rename(columns={"Area": "geoscale", "Year": "timescale"}, inplace=True)
    ...
    df_ssr_bev["geoscale"] = df_ssr_bev["geoscale"].replace("Czechia", "Czech Republic")

    # Extrapolation
    df_ssr_bev = ensure_structure(df_ssr_bev)
    df_ssr_bev = linear_fitting_ots_db(df_ssr_bev, years_ots, countries="all")
...
def trade_origin_processing(years_ots, list_countries_calc, file_dict):
    ...
    df_countries = pd.read_csv(
        "data/faostat/FAOSTAT_data_partner-countries-regions.csv"
    )

    # Filter the regions
    clean_regions = [
        x.replace("-- ", "").replace(" > (List)", "") for x in list_partnerregions
    ]
    mask = df_countries["Partner Country Group"].str.contains(
        "|".join(clean_regions), case=False, na=False
    )
    ...
    df_trade_agg = df_trade_agg.groupby(
        ["variables", "Partner Country Group", "Year"], as_index=False
    )["Value"].sum()

    df_trade_agg.rename(
        columns={
            "Partner Country Group": "geoscale",
            "Year": "timescale",
            "Value": "value",
        },
        inplace=True,
    ...
    cdm_kcal_temp = cdm_kcal_temp.filter(
        {"Categories1": dm_crop_trade_origin.col_labels["Categories1"]}
    )
    dm_crop_trade_origin.sort("Categories1")
    cdm_kcal_temp.sort("Categories1")
    array_temp = (
        dm_crop_trade_origin[:, :, "agr_split-import", :]
...
def fxa_processing_yield(cdm_kcal):
    ...
    try:
        ...
        df_ssr_2010_2021_molasse_cake.loc[
            df_ssr_2010_2021_molasse_cake["Element"].str.contains(
                "Production Quantity", case=False, na=False, regex=False
            ),
            "Element",
        ] = "Production"
        df_ssr_2010_2021_molasse_cake.loc[
            df_ssr_2010_2021_molasse_cake["Element"].str.contains(
                "Import quantity", case=False, na=False, regex=False
            ),
            "Element",
        ] = "Import"
        df_ssr_2010_2021_molasse_cake.loc[
            df_ssr_2010_2021_molasse_cake["Element"].str.contains(
                "Export quantity", case=False, na=False, regex=False
            ),
            "Element",
        ] = "Export"
        df_ssr_2010_2021_molasse_cake.loc[
    ...
    # Read excel file
    df_dict = pd.read_excel(
        "dictionaries/dictionnary_dietary-habits.xlsx", sheet_name="fxa"
    )
    ...
    # Voil
    array_temp = (
        dm_fxa_pro_yield[:, :, "fxa_agr_processing-yield", "voil-to-oilcrop"]
    ...
    # Cake
    array_temp = (
        dm_fxa_pro_yield[:, :, "fxa_agr_processing-yield", "cake-to-oilcrop"]
        * cdm_kcal[np.newaxis, np.newaxis, "cp_kcal-per-t", "crop-oilcrop"]
    ...
    # Molasse
    array_temp = (
        dm_fxa_pro_yield[:, :, "fxa_agr_processing-yield", "molasse-to-sugarcrop"]
    ...
    # Sugar
    array_temp = (
        dm_fxa_pro_yield[:, :, "fxa_agr_processing-yield", "sugar-to-sugarcrop"]
        * cdm_kcal[np.newaxis, np.newaxis, "cp_kcal-per-t", "crop-sugarcrop"]
...
def constant():
    ...
    df_kcal_t = pd.read_excel(
        "data/dietary-habits_constants.xlsx", sheet_name="cp_kcal_t"
    )
    ...
    # Read excel
    df_cp_bev = pd.read_excel(
        "data/dietary-habits_constants.xlsx", sheet_name="cp_ibp_bev"
    )
    ...
    # Format as a cdm
    cdm_lifestyle = ConstantDataMatrix(
        col_labels={"Variables": ["cp_time_days-per-year"]}
    )
...
def fts_processing(list_countries_calc, years_ots, years_fts, cdm_kcal):
    ...
    df_fts_diet_kcal = pd.read_excel(
        "data/dietary-habits_fts.xlsx", sheet_name="diet-split-kcal"
    )
    ...
    for lever in df_fts_diet_kcal["lever"].unique():
        ...
        for level in df_fts_diet_kcal["level"].unique():
            ...
            df_ots, df_fts = database_to_df(df_fts_filtered.copy(), lever, level="all")
            df_fts = df_fts.drop(columns=[lever])  # Drop column with lever name
            if (lever == "share-kcal-processed-food_crop-cereal-whole") | (
  ...
  dm_fts[lever] = dm_kcal"""
    ...
    df_fts_diet = pd.read_excel(
        "data/dietary-habits_fts.xlsx", sheet_name="diet-split-share"
    )
    df_fts_diet = df_fts_diet[
        [
            "variables",
            "diet_eat-lancet-phd-2025",
            "diet_eat-lancet-phd-2019",
            "diet_sfp-2024",
        ]
    ]
    ...
    df_fts_diet = df_fts_diet.melt(
        id_vars="variables", var_name="level_name", value_name="value"
    )
    ...
    for level in df_fts_diet["level"].unique():
        ...
        dm_cereal = (
            dm[level]
            .filter({"Categories1": ["crop-cereal-whole", "crop-cereal-refined"]})
            .copy()
        )
        dm_cereal_tot = dm_cereal.copy()
        dm_cereal_tot.groupby(
            {"crop-cereal": "crop-cereal.*"},
            regex=True,
            inplace=True,
            dim="Categories1",
        )
        ...
        dm_cereal.operation(
            "lfs_consumers-diet_crop-cereal-whole",
            "/",
            "lfs_consumers-diet_crop-cereal",
            out_col="lfs_share_crop-cereal-whole",
            unit="-",
        )
        ...
        dm_meat.operation(
            "lfs_consumers-diet_pro-liv-meat-unprocessed",
            "/",
            "lfs_consumers-diet_pro-liv-meat-total",
            out_col="lfs_share_unprocessed-meat",
            unit="-",
        )
    ...
    for level in df_fts_diet["level"].unique():
        ...
        # Change unit
        dm_fts[lever][level].change_unit(
            "lfs_consumers-diet", old_unit="%", new_unit="-", factor=1
        )
...
def datamatrix_to_pickle(dm_fts, cdm_bev):
    ...
    dict_ots["share-processed-food_crop-cereal-whole"] = dm_food_health.filter(
        {"Variables": ["lfs_share_crop-cereal-whole"]}
    )
    dict_ots["share-processed-food_unprocessed-meat"] = dm_food_health.filter(
        {"Variables": ["lfs_share_unprocessed-meat"]}
    )
    ...
    dict_ots["share-kcal-processed-food_crop-cereal-whole"] = dm_food_health.filter(
        {"Variables": ["lfs_share_crop-cereal-whole"]}
    )
    dict_ots["share-kcal-processed-food_unprocessed-meat"] = dm_food_health.filter(
        {"Variables": ["lfs_share_unprocessed-meat"]}
    )
    ...
    level = 1
    dm_fts[lever][level] = dict_ots[lever].copy()
    linear_fitting(dm_fts[lever][level], years_fts, min_t0=1e-6, min_tb=1e-6)
    dm_fts[lever][level].filter({"Years": years_fts}, inplace=True)
    for level in range(2, 5):
        # Compute the reduction objective in 2050 compared to the last ots value,
        # for each food category
        dm_ots = dict_ots[lever].copy()
        array_temp = (
            dm_ots[:, years_ots[-1], "agr_kcal-req", :]
    ...
    dict_const["cdm_cp_ibp_bev_beer"] = cdm_bev.filter_w_regex(
        {"Variables": ".*beer.*"}
    )
...
dm_diet_share, dm_waste, dm_kcal_req, dm_diet_kcal = diet_processing(
    list_countries_calc, cdm_kcal, dm_kcal_req_temp
)
...
dm_cal_dom_prod_crop, dm_cal_dom_prod_bev = crop_calibration(
    list_countries_calc, dm_fxa_pro_yield, cdm_bev
)
dm_crop_trade_origin, dm_cal_imports_countries, dm_cal_imports_tot = (
    trade_origin_processing(years_ots, list_countries_calc, file_dict)
)
