...
def alcoholic_beverages_workflow(DM_alc_bev, CDM_const, dm_lfs, years_setting):
    ...
    dm_demand_bev = dm_lfs.filter_w_regex(
        {"Categories1": "pro-bev.*", "Variables": "agr_demand"}
    )
    ...
    array_agr_domestic_production = (
        dm_demand_bev[:, :, "agr_demand", :] * food_net_import_pro[:, :, "agr_ssr"]
    )
    dm_demand_bev.add(
        array_agr_domestic_production,
        dim="Variables",
    ...
    # Beer
    dm_bev_beer = dm_demand_bev.filter_w_regex(
    ...
    # Bev-alc
    dm_bev_alc = dm_demand_bev.filter_w_regex(
    ...
    # Bev-fer
    dm_bev_fer = dm_demand_bev.filter_w_regex(
    ...
    # Wine
    dm_bev_wine = dm_demand_bev.filter_w_regex(
    ...
    dm_cal_rates_bev = calibration_rates(
        dm_bev_dom_prod,
        DM_alc_bev["cal_bev"],
    ...
    dm_bev_dom_prod.operation(
        "agr_domestic-production_bev_raw",
        "*",
        "cal_rate",
        dim="Variables",
        out_col="agr_domestic-production_bev",
        unit="kcal",
    )
    ...
    agr_ibp_bev_beer_fdk_cereal = (
        dm_bev_beer.array[:, :, idx_dm_bev_beer["agr_domestic_production_beer"]]
        * cdm_cp_ibp_bev_beer.array[
            idx_cdm_ibp_beer["cp_ibp_bev_beer_brf_fdk_crop_cereal"]
        ]
    )
    dm_bev_beer.add(
        agr_ibp_bev_beer_fdk_cereal,
    ...
    dm_demand_bev.operation(
        "agr_demand",
        "-",
        "agr_domestic_production",
        out_col="agr_imported_production_raw",
        unit="kcal",
    )

    # Calibration imports (processed beverages)
    dm_cal_rates_bev = calibration_rates(
        dm_demand_bev.filter({"Variables": ["agr_imported_production_raw"]}),
        DM_alc_bev["cal_bev_imports"],
    ...
    dm_demand_bev.operation(
        "agr_imported_production_raw",
        "*",
        "cal_rate",
        dim="Variables",
        out_col="agr_imported_production",
        unit="kcal",
    )
    dm_demand_bev.filter(
        {
            "Variables": [
                "agr_demand",
                "agr_domestic_production",
                "agr_imported_production",
            ]
        },
        inplace=True,
    )
    ...
    array_temp = (
        dm_demand_bev[:, :, "agr_imported_production", :]
        * dm_trade[:, :, "agr_split-import", :]
    )
    DM_alc_bev["split-import"].add(
    ...
    DM_alc_bev["split-import"].add(
        0.0,
        dummy=True,
        col_label="agr_domestic_production_bev_raw",
        dim="Variables",
        unit="kcal",
    )
    ...
    # Beer - Crop Cereal
    array_temp = (
        DM_alc_bev["split-import"][:, :, "agr_domestic_production", "pro-bev-beer"]
        * cdm_cp_ibp_bev_beer["cp_ibp_bev_beer_brf_crop_cereal"]
    )
    DM_alc_bev["split-import"][
        :, :, "agr_domestic_production_bev_raw", "pro-bev-beer"
    ] = array_temp

    # Bev-fer - Crop cereal
    array_temp = (
        DM_alc_bev["split-import"][:, :, "agr_domestic_production", "pro-bev-bev-fer"]
        * cdm_cp_ibp_bev_fer["cp_ibp_bev_bev-fer_brf_crop_cereal"]
    )
    DM_alc_bev["split-import"][
        :, :, "agr_domestic_production_bev_raw", "pro-bev-bev-fer"
    ] = array_temp

    # Bev-alc - Crop fruit
    array_temp = (
        DM_alc_bev["split-import"][:, :, "agr_domestic_production", "pro-bev-bev-alc"]
        * cdm_cp_ibp_bev_alc["cp_ibp_bev_bev-alc_brf_crop_fruit"]
    )
    DM_alc_bev["split-import"][
        :, :, "agr_domestic_production_bev_raw", "pro-bev-bev-alc"
    ] = array_temp

    # Wine - Crop Grape (fruit)
    dm_pro_yield_imports = DM_alc_bev["processing-yields"].filter(
        {"Country": DM_alc_bev["split-import"].col_labels["Country"]}
    )
    array_temp = (
        dm_bev_wine[:, :, "agr_domestic_production_wine"]
    ...
    DM_alc_bev["split-import"][
        :, :, "agr_domestic_production_bev_raw", "pro-bev-wine"
    ] = array_temp
...
def alcoholic_beverages(
    ...
    if interface.has_link(
        from_sector="dietary-habits", to_sector="alcoholic-beverages"
    ):
        dm_lfs = interface.get_link(
            from_sector="dietary-habits", to_sector="alcoholic-beverages"
        )
    else:
    ...
    interface.add_link(
        from_sector="alcoholic-beverages", to_sector="livestock", dm=DM_alc_livestock
    )

    # alcoholic-beverages to crop
    DM_alc_to_crop = {
        "crop_bev": dm_bev_dom_prod,
        "imports_bev_raw": DM_alc_bev["split-import"].filter(
            {"Variables": ["agr_domestic_production_bev_raw"]}
        ),
    }
    if write_pickle is True:
        current_file_directory = os.path.dirname(os.path.abspath(__file__))
    ...
    interface.add_link(
        from_sector="alcoholic-beverages", to_sector="crop", dm=DM_alc_to_crop
    )
...
def alcoholic_beverages_local_run():
    ...
    DM_input = filter_country_and_load_data_from_pickles(
        ...
        modules_list="alcoholic-beverages",
        filter_country=False,
    )
    years_setting, lever_setting = init_years_lever()
    alcoholic_beverages(
        lever_setting, years_setting, DM_input["alcoholic-beverages"], write_pickle=True
    )
