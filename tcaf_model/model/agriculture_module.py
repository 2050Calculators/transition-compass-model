...
from tcaf_model.model.common.io_database import dm_to_database
...
def lifestyle_workflow(DM_lifestyle, DM_lfs, CDM_const, years_setting):
    ...
    dm_diet_requirement.operation(
        "lfs_demography", "*", "agr_kcal-req", out_col="lfs_kcal-req", unit="kcal/day"
    )
    ...
    ay_diet_consumers = (
        dm_diet_share.array[:, :, 0, :]
        * dm_diet_requirement.array[:, :, idx["lfs_healthy-gap"], np.newaxis]
    )
    dm_diet_share.add(
        ay_diet_consumers,
        dim="Variables",
    ...
    ay_total_diet = (
        dm_diet_share.array[:, :, idx_d["lfs_consumers-diet"], :]
        * dm_population.array[:, :, idx_p["lfs_population_total"], np.newaxis]
        * 365.25
    )
    start = time.time()
    dm_diet_tmp = DataMatrix.based_on(
        ay_total_diet[:, :, np.newaxis, :],
        dm_diet_share,
    ...
    ay_total_fwaste = (
        dm_diet_fwaste.array[:, :, 0, :]
        * dm_population.array[:, :, idx["lfs_population_total"], np.newaxis]
        * cdm_lifestyle.array[idx_const["cp_time_days-per-year"]]
    )
    dm_diet_fwaste.add(
        ay_total_fwaste, dim="Variables", col_label="lfs_food-wastes", unit="kcal"
    )  # to bypass calibration data missing
    ...
    ay_total_food = (
        dm_diet_split.array[:, :, 0, :]
        * dm_population.array[:, :, idx["lfs_population_total"], np.newaxis]
        * cdm_lifestyle.array[idx_const["cp_time_days-per-year"]]
    )
    dm_diet_food = DataMatrix.based_on(
        ay_total_food[:, :, np.newaxis, :],
        dm_diet_split,
        change={"Variables": ["lfs_diet_raw"]},
        units={"lfs_diet_raw": "kcal"},
    )
    ...
    dm_diet_pre = dm_diet_food.filter(
        {"Variables": ["lfs_diet_raw"]}
    ).copy()  # create copy
    dm_diet_food.operation(
        "lfs_diet_raw",
        "+",
        "lfs_food-wastes",
        dim="Variables",
        out_col="agr_demand_raw",
        unit="kcal",
    )
    ...
    array_temp = (
        dm_diet_pre[:, :, "lfs_diet_raw", :]
        / dm_population[:, :, "lfs_population_total", np.newaxis]
        / 365.25
    )
    dm_diet_pre.add(
        array_temp, dim="Variables", col_label="lfs_diet_raw_cap", unit="kcal/cap/day"
    )
    ...
    array_temp = (
        dm_diet_pre[:, :, "lfs_diet_raw_cap", :]
        / cdm_kcal[np.newaxis, np.newaxis, "cp_kcal-per-t", :]
    )
    dm_diet_pre.add(
        array_temp, dim="Variables", col_label="lfs_consumers-diet", unit="t/cap/day"
    )
    dm_diet_pre = dm_diet_pre.filter({"Variables": ["lfs_consumers-diet"]})
    # Convert from [t/cap/day] to [g/cap/day]
    dm_diet_pre.change_unit(
        "lfs_consumers-diet", factor=1e6, old_unit="t/cap/day", new_unit="g/cap/day"
    )
    ...
    df_diet_pre = dm_to_database(dm_diet_pre, "none", "agriculture", level=0)
    df_diet_pre = df_diet_pre[df_diet_pre["geoscale"] == "Switzerland"].copy()
    df_diet_pre = df_diet_pre[["timescale", "variables", "value"]].copy()
    df_pivot = df_diet_pre.pivot(index="variables", columns="timescale", values="value")
    ...
    dm_cal_rates_diet = calibration_rates(
        dm_diet_food,
        dm_cal_diet,
    ...
    df_cal_rates_diet = dm_to_database(
        dm_cal_rates_diet, "none", "agriculture", level=0
    )  # Exporting calibration rates to check at the end
...
def food_demand_workflow(DM_food_demand, dm_lfs):
    ...
    dm_lfs_pro = dm_lfs.filter_w_regex(
        {"Categories1": "pro-.*", "Variables": "agr_demand"}
    )
    food_net_import_pro = DM_food_demand["food-net-import-pro"].filter_w_regex(
        {"Categories1": "pro-.*", "Variables": "agr_food-net-import"}
    )
    # Dropping the unwanted columns
    food_net_import_pro.drop(
        dim="Categories1",
        col_label=["pro-crop-processed-cake", "pro-crop-processed-molasse"],
    )
    ...
    agr_domestic_production = (
        dm_lfs_pro.array[:, :, idx_lfs["agr_demand"], :]
...
def livestock_workflow(DM_livestock, CDM_const, dm_lfs_pro, years_setting):
    ...
    dm_lfs_pro_liv = dm_lfs_pro.filter_w_regex(
        {"Categories1": "pro-liv.*", "Variables": "agr_domestic_production"}
    )
    ...
    DM_livestock["losses"].operation(
        ...
        "*",
        "agr_domestic_production",
        out_col="agr_domestic_production_liv_afw_raw",
        unit="kcal",
    )

    # Calibration - Livestock domestic production
    dm_cal_liv_prod = DM_livestock["cal_liv_prod"]
    dm_liv_prod = DM_livestock["losses"].filter(
        {"Variables": ["agr_domestic_production_liv_afw_raw"]}
    )
    dm_liv_prod.drop(
        dim="Categories1", col_label=["abp-processed-offal", "abp-processed-afat"]
    )  # Filter dm_liv_prod to drop offal & afats
    dm_cal_rates_liv_prod = calibration_rates(
        dm_liv_prod,
        dm_cal_liv_prod,
        calibration_start_year=1990,
        calibration_end_year=2023,
    ...
    dm_liv_prod.operation(
        "agr_domestic_production_liv_afw_raw",
        "*",
        "cal_rate",
        dim="Variables",
    ...
    df_cal_rates_liv_prod = dm_to_database(
        dm_cal_rates_liv_prod, "none", "agriculture", level=0
    )
    ...
    DM_livestock["yield"].append(
        dm_liv_slau, dim="Variables"
    )  # Append cal_agr_domestic_production_liv_afw in yield
    DM_livestock["yield"].operation(
        "agr_domestic_production_liv_afw",
        "/",
        "agr_climate-smart-livestock_yield",
        dim="Variables",
        out_col="agr_liv_population_slau",
        unit="lsu",
    )
    ...
    DM_livestock["liv_slaughtered_rate"].operation(
        "agr_liv_population_slau",
        "/",
        "agr_climate-smart-livestock_slaughtered",
        dim="Variables",
        out_col="agr_liv_population_raw",
        unit="lsu",
    )
    ...
    dm_liv_pop = DM_livestock["liv_slaughtered_rate"].filter(
        {"Variables": ["agr_liv_population_raw"]}
    )
    dm_cal_rates_liv_pop = calibration_rates(
        dm_liv_pop,
        dm_cal_liv_pop,
        calibration_start_year=1990,
    ...
    dm_liv_pop.operation(
        "agr_liv_population_raw",
        "*",
        "cal_rate",
        dim="Variables",
        out_col="agr_liv_population",
        unit="lsu",
    )
    # dm_liv_slau_egg_dairy.operation('agr_liv_population_raw', '*', 'cal_rate', dim='Variables', out_col='agr_liv_population', unit='lsu')
    df_cal_rates_liv_pop = dm_to_database(
        dm_cal_rates_liv_pop, "none", "agriculture", level=0
    )
    ...
    dm_liv_ruminants.groupby(
        {"ruminant": ".*"}, dim="Categories1", regex=True, inplace=True
    )
    ...
    DM_livestock["ruminant_density"].append(
        dm_liv_ruminants, dim="Variables"
    )  # Append to caf
    # Agriculture grassland [ha] = ruminant livestock [lsu] / livestock density [lsu/ha]
    DM_livestock["ruminant_density"].operation(
        "agr_liv_population_ruminant",
        "/",
    ...
    agr_ibp_offal = (
        cal_liv_population_meat.array[:, :, idx_liv_pop["agr_liv_population"], :]
        * cdm_cp_ibp_offal.array[idx_cdm_offal["cp_ibp_liv"]]
    )
    cal_liv_population_meat.add(
        agr_ibp_offal, dim="Variables", col_label="agr_ibp_offal", unit="kcal"
    )
    ...
    cal_liv_population_meat.add(
        agr_ibp_afat, dim="Variables", col_label="agr_ibp_afat", unit="kcal"
    )
    ...
    dm_processed_offal_afat = DM_livestock["losses"].filter(
        {
            "Variables": ["agr_domestic_production_liv_afw_raw"],
            "Categories1": ["abp-processed-offal", "abp-processed-afat"],
        }
    )
    dm_processed_offal_afat.rename_col_regex(
        str1="abp-processed-", str2="", dim="Categories1"
    )
    ...
    dm_liv_ibp.operation(
        "agr_ibp_total",
        "-",
        "agr_domestic_production_liv_afw_raw",
        out_col="agr_ibp_liv_fdk",
        unit="kcal",
    )
...
def alcoholic_beverages_workflow(DM_alc_bev, CDM_const, dm_lfs_pro):
    ...
    # Beer
    dm_bev_beer = dm_lfs_pro.filter_w_regex(
        {"Categories1": "pro-bev-beer.*", "Variables": "agr_domestic_production"}
    )
    dm_bev_beer.rename_col_regex(str1="pro-bev-", str2="", dim="Categories1")
    dm_bev_beer = dm_bev_beer.flatten()
    # Bev-alc
    dm_bev_alc = dm_lfs_pro.filter_w_regex(
        {"Categories1": "pro-bev-bev-alc.*", "Variables": "agr_domestic_production"}
    )
    dm_bev_alc.rename_col_regex(str1="pro-bev-", str2="", dim="Categories1")
    dm_bev_alc = dm_bev_alc.flatten()
    # Bev-fer
    dm_bev_fer = dm_lfs_pro.filter_w_regex(
        {"Categories1": "pro-bev-bev-fer.*", "Variables": "agr_domestic_production"}
    )
    dm_bev_fer.rename_col_regex(str1="pro-bev-", str2="", dim="Categories1")
    dm_bev_fer = dm_bev_fer.flatten()
    # Wine
    dm_bev_wine = dm_lfs_pro.filter_w_regex(
        {"Categories1": "pro-bev-wine.*", "Variables": "agr_domestic_production"}
    )
    ...
    agr_ibp_bev_beer_crop_cereal = (
        dm_bev_beer.array[:, :, idx_dm_bev_beer["agr_domestic_production_beer"]]
        * cdm_cp_ibp_bev_beer.array[idx_cdm_ibp_beer["cp_ibp_bev_beer_brf_crop_cereal"]]
    )
    dm_bev_beer.add(
        agr_ibp_bev_beer_crop_cereal,
        dim="Variables",
        col_label="agr_ibp_bev_beer_crop_cereal",
        unit="kcal",
    )
    ...
    agr_ibp_bev_fer_crop_cereal = (
        dm_bev_fer.array[:, :, idx_dm_bev_fer["agr_domestic_production_bev-fer"]]
        * cdm_cp_ibp_bev_fer.array[
            idx_cdm_ibp_fer["cp_ibp_bev_bev-fer_brf_crop_cereal"]
        ]
    )
    dm_bev_fer.add(
        agr_ibp_bev_fer_crop_cereal,
        dim="Variables",
        col_label="agr_ibp_bev_bev-fer_crop_cereal",
        unit="kcal",
    )
    ...
    agr_ibp_bev_alc_crop_fruit = (
        dm_bev_alc.array[:, :, idx_dm_bev_alc["agr_domestic_production_bev-alc"]]
        * cdm_cp_ibp_bev_alc.array[idx_cdm_ibp_alc["cp_ibp_bev_bev-alc_brf_crop_fruit"]]
    )
    dm_bev_alc.add(
        agr_ibp_bev_alc_crop_fruit,
        dim="Variables",
        col_label="agr_ibp_bev_bev-alc_crop_fruit",
        unit="kcal",
    )

    # Wine - Crop Grape (fruit)
    array_temp = (
        dm_bev_wine[:, :, "agr_domestic_production_wine"]
        * DM_alc_bev["processing-yields"][
    ...
    dm_bev_wine.add(
        array_temp,
        dim="Variables",
        col_label="agr_ibp_bev_wine_crop_fruit",
        unit="kcal",
    )
    ...
    dm_bev_dom_prod.operation(
        "agr_ibp_bev_beer_crop_cereal",
        "+",
        "agr_ibp_bev_bev-fer_crop_cereal",
        out_col="agr_domestic-production_bev_cereal",
        unit="kcal",
    )
    ...
    dm_bev_dom_prod.operation(
        ...
        out_col="agr_domestic-production_bev_fruit",
        unit="kcal",
    )
    ...
    dm_bev_dom_prod = dm_bev_dom_prod.filter(
        {
            "Variables": [
                "agr_domestic-production_bev_cereal",
    ...
    dm_bev_beer.add(
        agr_ibp_bev_beer_fdk_yeast,
        dim="Variables",
        col_label="agr_ibp_bev_beer_fdk_yeast",
        unit="kcal",
    )
    ...
    dm_bev_beer.add(
        agr_ibp_bev_beer_fdk_cereal,
        dim="Variables",
        col_label="agr_ibp_bev_beer_fdk_cereal",
        unit="kcal",
    )
    ...
    agr_ibp_bev_wine_fdk_marc = (
        dm_bev_wine.array[:, :, idx_dm_bev_wine["agr_domestic_production_wine"]]
        * cdm_cp_ibp_bev_wine.array[idx_cdm_ibp_wine["cp_ibp_bev_wine_brf_fdk_marc"]]
    )
    dm_bev_wine.add(
        agr_ibp_bev_wine_fdk_marc,
        dim="Variables",
        col_label="agr_ibp_bev_wine_fdk_marc",
        unit="kcal",
    )
    ...
    dm_bev_wine.add(
        agr_ibp_bev_wine_fdk_lees,
        dim="Variables",
        col_label="agr_ibp_bev_wine_fdk_lees",
        unit="kcal",
    )

    # Byproducts for other uses [kcal] = sum (wine byproducts [kcal])
    dm_bev_wine.operation(
        "agr_ibp_bev_wine_fdk_marc",
        "+",
        "agr_ibp_bev_wine_fdk_lees",
        out_col="agr_bev_ibp_use_oth",
        unit="kcal",
    )
    ...
    agr_bev_ibp_use_oth = (
        dm_bev_ibp_use_oth.array[
            :, :, idx_bev_ibp_use_oth["agr_bev_ibp_use_oth"], np.newaxis
        ]
        * DM_alc_bev["biomass_hierarchy"].array[
            :, :, idx_bev_biomass_hierarchy["agr_biomass-hierarchy-bev-ibp-use-oth"], :
        ]
    )
    DM_alc_bev["biomass_hierarchy"].add(
        agr_bev_ibp_use_oth,
        dim="Variables",
        col_label="agr_bev_ibp_use_oth",
        unit="kcal",
    )

    # Cereal bev byproducts allocated to feed [kcal] = sum (beer byproducts for feedstock [kcal])
    dm_bev_beer.operation(
        "agr_ibp_bev_beer_fdk_yeast",
        "+",
        "agr_ibp_bev_beer_fdk_cereal",
        out_col="agr_use_bev_ibp_cereal_feed",
        unit="kcal",
    )
    dm_bev_ibp_cereal_feed = dm_bev_beer.filter(
        {"Variables": ["agr_use_bev_ibp_cereal_feed"]}
    )
    ...
    array_temp = (
        dm_bev_ibp_cereal_feed[:, :, "agr_use_bev_ibp_cereal_feed"]
        / cdm_kcal[np.newaxis, np.newaxis, "cp_kcal-per-t_crop-cereal"]
    )
    dm_bev_ibp_cereal_feed.add(
        array_temp, dim="Variables", col_label="agr_use_bev_ibp_cereal_feed_t", unit="t"
    )
...
def bioenergy_workflow(DM_bioenergy, CDM_const, DM_ind, dm_bld, dm_tra):
    ...
    dm_bio_cap = (
        DM_bioenergy["electricity_production"].array[
            :, :, idx_bio_cap_elec["agr_bioenergy-capacity_elec"], :
        ]
        * cdm_load.array[idx_const["cp_load_hours-per-year-twh"]]
    )
    DM_bioenergy["electricity_production"].add(
        dm_bio_cap, dim="Variables", col_label="agr_bioenergy-capacity_lfe", unit="TWh"
    )

    # Electricity production [TWh] = bioenergy capacity [TWh] * load-factors per technology [%]
    DM_bioenergy["electricity_production"].operation(
        "agr_bioenergy-capacity_lfe",
        "*",
        "agr_bioenergy-capacity_load-factor",
        out_col="agr_bioenergy-capacity_elec-prod",
        unit="TWh",
    )

    # Feedstock requirements [TWh] = Electricity production [TWh] / Efficiency per technology [%]
    DM_bioenergy["electricity_production"].operation(
        "agr_bioenergy-capacity_elec-prod",
        "/",
        "agr_bioenergy-capacity_efficiency",
        out_col="agr_bioenergy-capacity_fdk-req",
        unit="TWh",
    )
    ...
    dm_bio_gas_demand = (
        dm_bld.array[:, :, idx_bld["bld_bioenergy"], idx_bld["gas-bio"]]
        + dm_ind_bioenergy.array[
            :, :, idx_ind_bioenergy["ind_bioenergy"], idx_ind_bioenergy["gas-bio"]
        ]
        + dm_ind_biomaterial.array[
            :, :, idx_ind_biomaterial["ind_biomaterial"], idx_ind_biomaterial["gas-bio"]
        ]
        + dm_tra.array[:, :, idx_tra["tra_bioenergy"], idx_tra["gas"]]
        + DM_bioenergy["electricity_production"].array[
            :, :, idx_elec["agr_bioenergy-capacity_fdk-req"], idx_elec["biogases"]
        ]
        + DM_bioenergy["electricity_production"].array[
            :, :, idx_elec["agr_bioenergy-capacity_fdk-req"], idx_elec["biogases-hf"]
        ]
    )
    ...
    dm_biogas.add(
        dm_bio_gas_demand,
        dim="Variables",
        col_label="agr_bioenergy-capacity_biogas-req",
        unit="TWh",
    )
    dm_biogas.drop(
        dim="Variables", col_label=["ind_dem_natfibers"]
    )  # FIXME to empty when upper comment fixed
    ...
    dm_biogas_mix = (
        dm_biogas.array[
            :, :, idx_biogas["agr_bioenergy-capacity_biogas-req"], np.newaxis
        ]
        * DM_bioenergy["bgs-mix"].array[
            :, :, idx_mix["agr_bioenergy-capacity_bgs-mix"], :
        ]
    )
    DM_bioenergy["bgs-mix"].add(
        dm_biogas_mix,
        dim="Variables",
        col_label="agr_bioenergy-capacity_bgs-tec",
        unit="TWh",
    )

    # Digestor feedstock per type [TWh] = biogas demand for digestor [TWh] * biomass share for digestor [%]
    dm_digestor_demand = DM_bioenergy["bgs-mix"].filter(
        {"Variables": ["agr_bioenergy-capacity_bgs-tec"], "Categories1": ["digestor"]}
    )
    ...
    dm_biogas_demand_digestor = (
        dm_digestor_demand.array[
            :,
            :,
            idx_demand_digestor["agr_bioenergy-capacity_bgs-tec_digestor"],
            np.newaxis,
        ]
    ...
    DM_bioenergy["digestor-mix"].add(
        dm_biogas_demand_digestor,
        dim="Variables",
        col_label="agr_bioenergy_biomass-demand_biogas",
        unit="TWh",
    )
    ...
    dm_solid_demand = (
        dm_bld.array[:, :, idx_bld["bld_bioenergy"], idx_bld["solid-bio"]]
        + dm_ind_bioenergy.array[
            :, :, idx_ind_bioenergy["ind_bioenergy"], idx_ind_bioenergy["solid-bio"]
        ]
        + DM_bioenergy["electricity_production"].array[
            :, :, idx_elec["agr_bioenergy-capacity_fdk-req"], idx_elec["solid-biofuel"]
        ]
        + DM_bioenergy["electricity_production"].array[
            :,
            :,
            idx_elec["agr_bioenergy-capacity_fdk-req"],
    ...
    dm_solid.add(
        dm_solid_demand,
        dim="Variables",
        col_label="agr_bioenergy-capacity_solid-biofuel-req",
        unit="TWh",
    )
    dm_solid.drop(
        dim="Variables", col_label=["ind_dem_natfibers"]
    ...
    dm_solid_fdk = (
        dm_solid.array[
            :,
            :,
            idx_demand_solid["agr_bioenergy-capacity_solid-biofuel-req"],
            np.newaxis,
        ]
        * DM_bioenergy["solid-mix"].array[
            :, :, idx_mix_solid["agr_biomass-hierarchy_biomass-mix_solid"], :
        ]
    )
    DM_bioenergy["solid-mix"].add(
        dm_solid_fdk,
        dim="Variables",
        col_label="agr_bioenergy_biomass-demand_solid",
        unit="TWh",
    )
    ...
    dm_biodiesel_temp = (
        dm_biodiesel.array[
            :, :, idx_biodiesel_cap["agr_bioenergy-capacity_liq_biodiesel"], np.newaxis
        ]
    ...
    DM_bioenergy["liquid-biodiesel"].add(
        dm_biodiesel_temp,
        dim="Variables",
        col_label="agr_bioenergy-capacity_liq-bio-prod_biodiesel",
        unit="TWh",
    )
    ...
    dm_biogasoline_temp = (
        dm_biogasoline.array[
            :,
            :,
            idx_biogasoline_cap["agr_bioenergy-capacity_liq_biogasoline"],
            np.newaxis,
        ]
    ...
    DM_bioenergy["liquid-biogasoline"].add(
        dm_biogasoline_temp,
        dim="Variables",
        col_label="agr_bioenergy-capacity_liq-bio-prod_biogasoline",
        unit="TWh",
    )
    ...
    dm_biojetkerosene_temp = (
        dm_biojetkerosene.array[
            :,
            :,
        ...
        * DM_bioenergy["liquid-biojetkerosene"].array[
            :,
            :,
            idx_biojetkerosene_tec[
                "agr_biomass-hierarchy_bioenergy_liquid_biojetkerosene"
            ],
            :,
        ]
    )
    DM_bioenergy["liquid-biojetkerosene"].add(
        dm_biojetkerosene_temp,
        dim="Variables",
    ...
    dm_calc = (
        DM_bioenergy["liquid-biodiesel"].array[
            :, :, idx_bio["agr_bioenergy-capacity_liq-bio-prod_biodiesel"], :
        ]
    ...
    dm_calc = (
        DM_bioenergy["liquid-biogasoline"].array[
            :, :, idx_bio["agr_bioenergy-capacity_liq-bio-prod_biogasoline"], :
        ]
    ...
    dm_biojetkerosene.add(
        0.0, dummy=True, col_label="est", dim="Categories1", unit="kcal"
    )
    dm_biojetkerosene.add(
        0.0, dummy=True, col_label="ezm", dim="Categories1", unit="kcal"
    )
    dm_biojetkerosene.add(
        0.0, dummy=True, col_label="fer", dim="Categories1", unit="kcal"
    )
    ...
    dm_biofuel_fdk.operation(
        "agr_bioenergy-capacity_liq-bio-prod_fdk-req_biodiesel",
        "+",
        "agr_bioenergy-capacity_liq-bio-prod_fdk-req_biogasoline",
        out_col="agr_bioenergy_biomass-demand_liquid_biodiesel_biogasoline",
        unit="kcal",
    )
    dm_biofuel_fdk.operation(
        "agr_bioenergy_biomass-demand_liquid_biodiesel_biogasoline",
        "+",
        "agr_bioenergy-capacity_liq-bio-prod_fdk-req_biojetkerosene",
        out_col="agr_bioenergy_biomass-demand_liquid",
        unit="kcal",
    )
    dm_biofuel_fdk = dm_biofuel_fdk.filter(
        {"Variables": ["agr_bioenergy_biomass-demand_liquid"]}
    )

    # Sum using group by for each feedstock (fer => eth, btl & ezm => lgn, hvo & est =>oil)
    dm_biofuel_fdk.groupby(
        {"eth": ".*fer"}, dim="Categories1", regex=True, inplace=True
    )
    dm_biofuel_fdk.groupby(
        {"lgn": ".*btl|.*ezm"}, dim="Categories1", regex=True, inplace=True
    )
    dm_biofuel_fdk.groupby(
        {"oil": ".*hvo|.*est"}, dim="Categories1", regex=True, inplace=True
    )
    ...
    array_temp = (
        dm_biofuel_fdk.array[
            :, :, idx_eth_demand["agr_bioenergy_biomass-demand_liquid_eth"], np.newaxis
        ]
    ...
    dm_eth.add(
        array_temp,
        dim="Variables",
        col_label="agr_bioenergy_biomass-demand_liquid_eth",
        unit="kcal",
    )
    ...
    array_temp = (
        dm_biofuel_fdk.array[
            :, :, idx_lgn_demand["agr_bioenergy_biomass-demand_liquid_lgn"], np.newaxis
        ]
    ...
    dm_lgn.add(
        array_temp,
        dim="Variables",
        col_label="agr_bioenergy_biomass-demand_liquid_lgn",
        unit="kcal",
    )
    ...
    array_temp = (
        dm_biofuel_fdk.array[
            :, :, idx_oil_demand["agr_bioenergy_biomass-demand_liquid_oil"], np.newaxis
        ]
    ...
    dm_oil.add(
        array_temp,
        dim="Variables",
        col_label="agr_bioenergy_biomass-demand_liquid_oil",
        unit="kcal",
    )
...
def livestock_manure_workflow(
    ...
    DM_manure["liv_n-stock"].operation(
        "fxa_liv_manure_n-stock",
        "*",
        "agr_liv_population",
        out_col="agr_liv_n-stock",
        unit="t",
    )
    ...
    dm_temp = (
        DM_manure["liv_n-stock"].array[
            :, :, idx_nstock["agr_liv_n-stock"], :, np.newaxis
        ]
        * DM_manure["manure"].array[
            :, :, idx_split["agr_climate-smart-livestock_manure"], :, :
        ]
    )
    DM_manure["ef_liv_N2O"].add(
        dm_temp, dim="Variables", col_label="agr_liv_n-stock_split", unit="t"
    )

    # Manure emission [MtN2O] = Manure management practices [MtN] * emission factors per practices [MtN2O/Mt]
    DM_manure["ef_liv_N2O"].operation(
        "agr_liv_n-stock_split",
        "*",
        "fxa_ef_liv_N2O-emission_ef",
        out_col="agr_liv_N2O-emission_raw",
        unit="t",
    )
    ...
    # Calibration N2O
    dm_liv_N2O = DM_manure["ef_liv_N2O"].filter(
        {"Variables": ["agr_liv_N2O-emission_raw"]}
    )
    dm_cal_liv_N2O = DM_manure["cal_liv_N2O"]
    dm_cal_liv_N2O.switch_categories_order(
        cat1="Categories2", cat2="Categories1"
    )  # Switch categories
    dm_cal_liv_N2O.change_unit(
        "cal_agr_liv_N2O-emission", factor=1e3, old_unit="kt", new_unit="t"
    )
    dm_cal_rates_liv_N2O = calibration_rates(
        dm_liv_N2O,
        dm_cal_liv_N2O,
        calibration_start_year=1990,
        calibration_end_year=2023,
    ...
    dm_liv_N2O.operation(
        "agr_liv_N2O-emission_raw",
        "*",
        "cal_rate",
        dim="Variables",
        out_col="agr_liv_N2O-emission",
        unit="t",
    )
    df_cal_rates_liv_N2O = dm_to_database(
        dm_cal_rates_liv_N2O, "none", "agriculture", level=0
    )

    # CH4
    # Enteric emission [tCH4] = livestock population [lsu] * enteric emission factor [tCH4/lsu]
    DM_manure["enteric_emission"].operation(
        "agr_climate-smart-livestock_enteric",
        "*",
        "agr_liv_population",
        dim="Variables",
        out_col="agr_liv_CH4-emission_raw",
        unit="t",
    )

    # Manure emission [tCH4] = livestock population [lsu] * emission factors treated manure [tCH4/lsu]
    DM_manure["ef_liv_CH4_treated"].operation(
        "fxa_ef_liv_CH4-emission_treated",
        "*",
        "agr_liv_population",
        dim="Variables",
        out_col="agr_liv_CH4-emission_raw",
    ...
    # Treated
    dm_CH4 = DM_manure["ef_liv_CH4_treated"].filter(
        {"Variables": ["agr_liv_CH4-emission_raw"]}
    )
    ...
    # Enteric
    dm_CH4_enteric = DM_manure["enteric_emission"].filter(
        {"Variables": ["agr_liv_CH4-emission_raw"]}
    )
    ...
    dm_cal_liv_CH4.switch_categories_order(
        cat1="Categories2", cat2="Categories1"
    )  # Switch categories
    dm_cal_liv_CH4.change_unit(
        "cal_agr_liv_CH4-emission", factor=1e3, old_unit="kt", new_unit="t"
    )
    dm_cal_rates_liv_CH4 = calibration_rates(
        dm_CH4,
        dm_cal_liv_CH4,
        calibration_start_year=1990,
        calibration_end_year=2023,
    ...
    dm_CH4.operation(
        "agr_liv_CH4-emission_raw",
        "*",
        "cal_rate",
        dim="Variables",
        out_col="agr_liv_CH4-emission",
        unit="t",
    )
    df_cal_rates_liv_CH4 = dm_to_database(
        dm_cal_rates_liv_CH4, "none", "agriculture", level=0
    )
...
def feed_workflow(
    ...
    # Convert from [kcal] to [t]
    array_temp = (
        dm_feed_req[:, :, "agr_domestic_production_liv_afw", :]
        / cdm_kcal[np.newaxis, np.newaxis, "cp_kcal-per-t", :]
    )
    dm_feed_req.add(
        array_temp,
    ...
    dm_temp = (
        dm_feed_req[:, :, "agr_domestic_production_liv_afw_t", :]
    ...
    dm_feed_ruminant = dm_feed_req.filter(
        {"Variables": ["agr_feed-requirement"], "Categories1": list_ruminant}
    )
    array_temp = (
        dm_feed_ruminant[:, :, "agr_feed-requirement", :]
        * DM_feed["ruminant-feed"]["ruminant-feed"][
            :, :, np.newaxis, "agr_ruminant-feed_share-grass"
        ]
    )
    dm_feed_ruminant.add(
        array_temp, dim="Variables", col_label="agr_feed-requirement_grass", unit="t"
    )
    dm_feed_ruminant.operation(
        "agr_feed-requirement",
        "-",
        "agr_feed-requirement_grass",
        out_col="agr_feed-requirement_without-grass",
        unit="t",
    )
    dm_feed_ruminant = dm_feed_ruminant.filter(
        {"Variables": ["agr_feed-requirement_without-grass"]}
    )
    ...
    dm_feed_without_grass = dm_feed_req.filter(
        {"Variables": ["agr_feed-requirement"], "Categories1": list_others}
    )
    dm_feed_without_grass.rename_col(
        "agr_feed-requirement", "agr_feed-requirement_without-grass", dim="Variables"
    )
    ...
    dm_feed_req_total = dm_feed_without_grass.filter(
        {"Variables": ["agr_feed-requirement_without-grass"]}
    )
    dm_feed_req_total.groupby(
        {"total": ".*"}, dim="Categories1", regex=True, inplace=True
    )
    ...
    dm_temp = (
        dm_feed_without_grass.array[
            :, :, idx_feed["agr_feed-requirement_without-grass"], :, np.newaxis
        ]
        * DM_feed["alt-protein"].array[:, :, idx_aps["agr_alt-protein"], :, :]
    )
    DM_feed["alt-protein"].add(
        dm_temp, dim="Variables", col_label="agr_feed_aps", unit="t"
    )

    # Insect meals [t] = sum algae feed req
    dm_aps = DM_feed["alt-protein"].filter(
        {"Variables": ["agr_feed_aps"], "Categories2": ["algae"]}
    )
    dm_aps = dm_aps.flatten()
    dm_aps.groupby({"algae": ".*"}, dim="Categories1", regex=True, inplace=True)

    # Insect meals [t] = sum insect feed req
    dm_insect = DM_feed["alt-protein"].filter(
        {"Variables": ["agr_feed_aps"], "Categories2": ["insect"]}
    )
    ...
    idx_aps = dm_aps.idx
    dm_temp = (
        dm_aps.array[:, :, idx_aps["agr_feed_aps"], np.newaxis, :, np.newaxis]
    ...
    dm_aps_feed.operation(
        "agr_feed_aps_total",
        "+",
        "agr_use_bev_ibp_cereal_feed_t",
        dim="Variables",
    ...
    dm_feed_req_total.operation(
        "agr_feed-requirement_without-grass_total",
        "-",
        "agr_alt-feed-ration",
        out_col="agr_crop-feed-demand",
        unit="t",
    )
    ...
    dm_temp = (
        dm_feed_req_total.array[
            :, :, idx_feed["agr_feed-requirement_without-grass_total"], np.newaxis
        ]
        * DM_feed["ration"].array[
            :, :, idx_ration["agr_climate-smart-livestock_ration"], :
        ]
    )
    DM_feed["ration"].add(
        dm_temp, dim="Variables", col_label="agr_demand_feed_raw", unit="kcal"
    )
    ...
    dm_cal_rates_feed = calibration_rates(
        dm_feed_demand,
        dm_cal_feed,
        calibration_start_year=1990,
        calibration_end_year=2023,
    ...
    DM_feed["ration"].operation(
        "agr_demand_feed_raw",
        "*",
        "cal_rate",
        dim="Variables",
        out_col="agr_demand_feed_t",
        unit="t",
    )
    ...
    DM_feed["ration"][:, :, "agr_demand_feed_t", :] = dm_temp[
        :, :, "agr_demand_feed_t", :
    ]
    ...
    array_temp = (
        DM_feed["ration"][:, :, "agr_demand_feed_t", :]
        * cdm_kcal[np.newaxis, np.newaxis, "cp_kcal-per-t", :]
    )
    DM_feed["ration"].add(
        array_temp, dim="Variables", col_label="agr_demand_feed", unit="kcal"
    )
...
def biomass_allocation_workflow(dm_aps_ibp, dm_oil):
    ...
    dm_aps_ibp_oil.groupby(
        {"oil-voil": ".*"}, dim="Categories1", regex=True, inplace=True
    )
...
def crop_workflow(
    ...
    list_crop_feed_processed = [
        "crop-processed-cake",
        "crop-processed-sugar",
        "crop-processed-voil",
    ]
    dm_feed_processed = DM_feed["ration"].filter(
        {"Variables": ["agr_demand_feed"], "Categories1": list_crop_feed_processed}
    )
    dm_feed_processed.rename_col(
        "crop-processed-cake", "cake-to-oilcrop", dim="Categories1"
    )
    # dm_feed_processed.rename_col('crop-processed-molasse', 'molasse-to-sugarcrop', dim='Categories1')
    dm_feed_processed.rename_col(
        "crop-processed-sugar", "sugar-to-sugarcrop", dim="Categories1"
    )
    dm_feed_processed.rename_col(
        "crop-processed-voil", "voil-to-oilcrop", dim="Categories1"
    )

    # Unprocessed Feed pre-processing
    list_crop_feed_unprocessed = [
        "crop-cereal",
        "crop-fruit",
        "crop-pulse",
        "crop-rice",
    ...
    dm_feed_unprocessed.add(
        0.0, dummy=True, col_label="crop-oilcrop", dim="Categories1", unit="kcal"
    )
    dm_feed_unprocessed.add(
        0.0, dummy=True, col_label="crop-sugarcrop", dim="Categories1", unit="kcal"
    )

    # Processed Feed - Accounting for SSR
    # Domestic production [kcal] = Processed Feed-demand [kcal] * net import [%]
    dm_ssr_feed_pro = (
        DM_crop["food-net-import-pro"]
        .filter(
            {
    ...
    dm_ssr_feed_pro.rename_col(
        "pro-crop-processed-cake", "cake-to-oilcrop", dim="Categories1"
    )
    # dm_ssr_feed_pro.rename_col('pro-crop-processed-molasse', 'molasse-to-sugarcrop', dim='Categories1')
    dm_ssr_feed_pro.rename_col(
        "pro-crop-processed-sugar", "sugar-to-sugarcrop", dim="Categories1"
    )
    dm_ssr_feed_pro.rename_col(
        "pro-crop-processed-voil", "voil-to-oilcrop", dim="Categories1"
    )
    dm_feed_processed.append(dm_ssr_feed_pro, dim="Variables")
    dm_feed_processed.operation(
        "agr_demand_feed",
        "*",
        "agr_food-net-import",
    ...
    dm_feed_processed.groupby(
        {"sugar-to-sugarcrop": ".*-to-sugarcrop"},
        dim="Categories1",
        regex=True,
        inplace=True,
    )
    ...
    dm_pro_yield = DM_crop["processing-yields"].filter(
        {"Categories1": ["cake-to-oilcrop", "sugar-to-sugarcrop", "voil-to-oilcrop"]}
    )
    ...
    dm_feed_processed.groupby(
        {"crop-oilcrop": ".*-to-oilcrop", "crop-sugarcrop": ".*-to-sugarcrop"},
        dim="Categories1",
        regex=True,
        inplace=True,
    )
    ...
    dm_feed_processed.add(
        0.0, dummy=True, col_label="crop-cereal", dim="Categories1", unit="kcal"
    )
    dm_feed_processed.add(
        0.0, dummy=True, col_label="crop-pulse", dim="Categories1", unit="kcal"
    )
    dm_feed_processed.add(
        0.0, dummy=True, col_label="crop-fruit", dim="Categories1", unit="kcal"
    )
    dm_feed_processed.add(
        0.0, dummy=True, col_label="crop-veg", dim="Categories1", unit="kcal"
    )
    dm_feed_processed.add(
        0.0, dummy=True, col_label="crop-starch", dim="Categories1", unit="kcal"
    )
    dm_feed_processed.add(
        0.0, dummy=True, col_label="crop-rice", dim="Categories1", unit="kcal"
    )
    ...
    dm_feed_processed = dm_feed_processed.filter(
        {"Variables": ["agr_demand_feed_pro_raw"]}
    )
    dm_feed_unprocessed.append(dm_feed_processed, dim="Variables")
    # Summing
    dm_feed_unprocessed.operation(
        "agr_demand_feed_pro_raw",
        "+",
        "agr_demand_feed",
        out_col="agr_demand_feed_total",
        unit="kcal",
    )
    dm_feed_unprocessed = dm_feed_unprocessed.filter(
        {"Variables": ["agr_demand_feed_total"]}
    )

    # Adding dummy categories
    dm_feed_unprocessed.add(
        0.0, dummy=True, col_label="crop-lgn-energycrop", dim="Categories1", unit="kcal"
    )
    dm_feed_unprocessed.add(
        0.0, dummy=True, col_label="crop-algae", dim="Categories1", unit="kcal"
    )
    dm_feed_unprocessed.add(
        0.0, dummy=True, col_label="crop-insect", dim="Categories1", unit="kcal"
    )
    ...
    dm_food_processed = dm_lfs_pro.filter(
        {
            ...
            "Categories1": ["pro-crop-processed-sweet", "pro-crop-processed-sugar"],
        }
    )
    dm_ssr_food_pro = (
        DM_crop["food-net-import-pro"]
    ...
    dm_food_processed.operation(
        "agr_demand",
        "*",
        "agr_food-net-import",
        out_col="agr_domestic-production_food_pro",
        unit="kcal",
    )
    ...
    dm_food_processed.groupby(
        {"pro-crop-processed-sweet": ".*"}, dim="Categories1", regex=True, inplace=True
    )
    dm_food_processed.rename_col(
        "pro-crop-processed-sweet", "crop-sugarcrop", dim="Categories1"
    )
    dm_food_processed.rename_col(
        "agr_domestic-production_food_pro",
        "agr_domestic-production_food_pro_temp",
        dim="Variables",
    )

    dm_pro_yield = DM_crop["processing-yields"].filter(
        {"Categories1": ["sugar-to-sugarcrop"]}
    )
    dm_pro_yield.rename_col("sugar-to-sugarcrop", "crop-sugarcrop", dim="Categories1")
    ...
    dm_crop_demand = dm_crop_demand.filter_w_regex(
        {"Variables": "agr_demand", "Categories1": "crop-|rice"}
    )
    # Renaming categories
    dm_crop_demand.rename_col_regex(
        str1="agr_demand", str2="agr_demand_food", dim="Variables"
    )
    ...
    # Adding dummy categories
    dm_crop_demand.add(
        0.0, dummy=True, col_label="lgn-energycrop", dim="Categories1", unit="kcal"
    )
    dm_crop_demand.add(
        0.0, dummy=True, col_label="algae", dim="Categories1", unit="kcal"
    )
    dm_crop_demand.add(
        0.0, dummy=True, col_label="insect", dim="Categories1", unit="kcal"
    )
    ...
    # Adding dummy categories
    dm_bev_dom_prod.add(
        0.0, dummy=True, col_label="oilcrop", dim="Categories1", unit="kcal"
    )
    dm_bev_dom_prod.add(
        0.0, dummy=True, col_label="pulse", dim="Categories1", unit="kcal"
    )
    dm_bev_dom_prod.add(
        0.0, dummy=True, col_label="veg", dim="Categories1", unit="kcal"
    )
    dm_bev_dom_prod.add(
        0.0, dummy=True, col_label="starch", dim="Categories1", unit="kcal"
    )
    dm_bev_dom_prod.add(
        0.0, dummy=True, col_label="sugarcrop", dim="Categories1", unit="kcal"
    )
    dm_bev_dom_prod.add(
        0.0, dummy=True, col_label="rice", dim="Categories1", unit="kcal"
    )
    ...
    array_temp = (
        dm_oilcrop_voil.array[:, :, idx_voil["agr_demand_bioe_pro"], :]
        / cdm_feed_yield.array[idx_cdm["cp_ibp_processed"], idx_cdm["voil-to-oilcrop"]]
    )
    dm_oilcrop_voil.add(
        array_temp, dim="Variables", col_label="agr_demand_bioe", unit="kcal"
    )
    ...
    dm_lgn_energycrop.groupby(
        {"lgn-energycrop": ".*"}, dim="Categories1", regex=True, inplace=True
    )
    dm_lgn_energycrop.rename_col(
        "agr_bioenergy_biomass-demand_liquid_lgn", "agr_demand_bioe", dim="Variables"
    )
    ...
    dm_feed_unprocessed.rename_col_regex(
        str1="crop-", str2="", dim="Categories1"
    )  # Renaming categories
    ...
    dm_crop_demand.operation(
        "agr_demand_feed_total",
        "+",
        "agr_demand_food",
        out_col="agr_demand_feed_food",
        unit="kcal",
    )
    dm_crop_demand.operation(
        "agr_demand_feed_food",
        "+",
        "agr_demand_bioe",
        out_col="agr_demand_feed_food_bioe",
        unit="kcal",
    )
    dm_crop_demand.operation(
        "agr_demand_feed_food_bioe",
        "+",
        "agr_aps",
        out_col="agr_demand_total",
        unit="kcal",
    )
    ...
    DM_crop["crop"].operation(
        "agr_domestic-production_without_bev",
        "+",
        "agr_domestic-production_bev",
        out_col="agr_domestic-production",
        unit="kcal",
    )

    # Domestic production with losses [kcal] = domestic prod * food losses [%]
    DM_crop["crop"].operation(
        "agr_domestic-production",
        "*",
        "agr_climate-smart-crop_losses",
        out_col="agr_domestic-production_afw_raw",
        unit="kcal",
    )

    # Processing for calibration :
    dm_cal_bev_fruit = DM_crop["cal_bev"].filter({"Categories1": ["wine"]})
    dm_cal_bev_cereal = DM_crop["cal_bev"].filter(
        {"Categories1": ["bev-beer", "bev-fer"]}
    )
    # Groupby fruits or cereals
    dm_cal_bev_fruit.groupby(
        {"fruit": ".*"}, dim="Categories1", regex=True, inplace=True
    )
    dm_cal_bev_cereal.groupby(
        {"cereal": ".*"}, dim="Categories1", regex=True, inplace=True
    )
    # cal_crop total = cal_crop_food (actually also includes feed) + cal_crop_bev
    dm_cal_crop = DM_crop["cal_crop"].copy()
    array_temp_cereal = (
        dm_cal_bev_cereal[:, :, "cal_agr_domestic-production_bev", "cereal"]
        + dm_cal_crop[:, :, "cal_agr_domestic-production_food", "cereal"]
    )
    dm_cal_crop[:, :, "cal_agr_domestic-production_food", "cereal"] = array_temp_cereal
    array_temp_fruit = (
        dm_cal_bev_fruit[:, :, "cal_agr_domestic-production_bev", "fruit"]
    ...
    dm_cal_rates_crop = calibration_rates(
        dm_crop,
        dm_cal_crop,
        calibration_start_year=1990,
        calibration_end_year=2023,
    ...
    DM_crop["crop"].operation(
        "agr_domestic-production_afw_raw",
        "*",
        "cal_rate",
        dim="Variables",
        out_col="agr_domestic-production_afw",
        unit="kcal",
    )
    df_cal_rates_crop = dm_to_database(
        dm_cal_rates_crop, "none", "agriculture", level=0
    )
    df_cal_crop = dm_to_database(dm_cal_crop, "none", "agriculture", level=0)
    df_crop = dm_to_database(
        dm_crop.filter({"Variables": ["agr_domestic-production_afw_raw"]}),
        "none",
        "agriculture",
        level=0,
    )
    ...
    dm_residues = DM_crop["crop"].filter(
        {
            ...
            "Categories1": ["cereal", "oilcrop", "sugarcrop"],
        }
    )
    DM_crop["residues_yield"].append(dm_residues, dim="Variables")
    DM_crop["residues_yield"].operation(
        "agr_domestic-production_afw",
        "*",
        "fxa_residues_yield",
        out_col="agr_residues",
        unit="kcal",
    )
    ...
    dm_residues_cereal = DM_crop["residues_yield"].filter(
        {"Variables": ["agr_residues"], "Categories1": ["cereal"]}
    )
    ...
    array_temp = (
        dm_residues_cereal.array[:, :, idx_residues["agr_residues_cereal"], np.newaxis]
        * DM_crop["hierarchy_residues_cereals"].array[
            :, :, idx_hierarchy["agr_biomass-hierarchy_crop_cereal"], :
        ]
    )
    DM_crop["hierarchy_residues_cereals"].add(
        array_temp, dim="Variables", col_label="agr_residues_emission", unit="Mt"
    )
    ...
    array_temp = (
        DM_crop["hierarchy_residues_cereals"].array[
            :, :, idx_residues["agr_residues_emission"], :, np.newaxis
        ]
        * DM_crop["ef_residues"].array[:, :, idx_ef["ef"], :, :]
    )
    DM_crop["ef_residues"].add(
        array_temp, dim="Variables", col_label="agr_crop_emission", unit="Mt"
    )
...
def land_workflow(DM_land, DM_crop, DM_livestock, dm_crop_other, DM_ind, years_setting):
    ...
    DM_land["fibers"].change_unit(
        "ind_dem_natfibers", factor=1000, old_unit="kt", new_unit="t"
    )

    # Domestic supply fiber crop demand [t] = ind demand natural fibers [t] + domestic supply quantity fibers [t]
    DM_land["fibers"].operation(
        "ind_dem_natfibers",
        "+",
        "fxa_domestic-supply-quantity_fibres-plant-eq",
        out_col="agr_domestic-supply-quantity_fibres-plant-eq",
        unit="t",
    )

    # Domestic production fiber crop [t] = Domestic supply fiber crop demand [t] * Self sufficiency ratio [%]
    DM_land["fibers"].operation(
        "agr_domestic-supply-quantity_fibres-plant-eq",
        "*",
        "fxa_domestic-self-sufficiency_fibres-plant-eq",
        out_col="agr_domestic-production_fibres-plant-eq",
        unit="t",
    )
    ...
    DM_land["fibers"].operation(
        ...
        out_col="agr_land_cropland_raw_fibres-plant-eq",
        unit="ha",
    )
    ...
    dm_crop_other.rename_col(
        "agr_demand_total", "agr_domestic-production_afw", dim="Variables"
    )
    dm_crop_afw.append(dm_crop_other, dim="Categories1")
    # Dropping unused yield categories
    DM_land["yield"].drop(
        dim="Categories1", col_label=["gas-energycrop", "fibres-plant-eq"]
    )
    ...
    DM_land["yield"].operation(
        "agr_domestic-production_afw",
        "/",
        "agr_climate-smart-crop_yield",
        out_col="agr_land_cropland_raw",
        unit="ha",
    )
    ...
    DM_land["fibers"] = DM_land["fibers"].filter(
        {"Variables": ["agr_land_cropland_raw_fibres-plant-eq"]}
    )
    DM_land["fibers"].deepen()
    DM_land["yield"].drop(
        dim="Variables",
        col_label=["agr_climate-smart-crop_yield", "agr_domestic-production_afw"],
    )
    ...
    dm_cal_rates_cropland = calibration_rates(
        dm_cropland,
        dm_cal_cropland,
        calibration_start_year=1990,
        calibration_end_year=2023,
    ...
    dm_cropland.operation(
        "agr_land_cropland_raw",
        "*",
        "cal_rate",
        dim="Variables",
        out_col="agr_land_cropland",
        unit="ha",
    )

    # Append with cropland for lgn-energycrop, algae & insect
    dm_cropland_others = DM_land["yield"].filter(
        {"Categories1": ["algae", "insect", "lgn-energycrop"]}
    )
    dm_cropland_others.rename_col(
        "agr_land_cropland_raw", "agr_land_cropland", dim="Variables"
    )
    ...
    dm_grassland = DM_livestock["ruminant_density"].filter(
        {"Variables": ["agr_lus_land_raw_grassland"]}
    )
    dm_grassland.deepen()
    ...
    dm_cal_rates_land = calibration_rates(
        dm_land,
        dm_cal_land,
        calibration_start_year=1990,
        calibration_end_year=2023,
    ...
    dm_land.operation(
        ...
        "cal_rate",
        dim="Variables",
        out_col="agr_lus_land",
        unit="ha",
    )
    df_cal_rates_land = dm_to_database(
        dm_cal_rates_land, "none", "agriculture", level=0
    )

    # Overall agricultural land [ha] = overall cropland + grasssland [ha]
    dm_land_use = dm_land.filter(
        {"Variables": ["agr_lus_land"]}
    ).copy()  # copu for Land use module
    ...
    DM_land["rice"].operation(
        "fxa_emission_crop_rice",
        "*",
        "agr_land_cropland_raw_rice",
        out_col="agr_rice_crop_CH4-emission",
        unit="t",
    )
...
def nitrogen_workflow(DM_nitrogen, dm_land, CDM_const, years_setting):
    ...
    dm_agricultural_land = dm_land.filter(
        {"Variables": ["agr_lus_land"], "Categories1": ["agriculture"]}
    )
    dm_agricultural_land = dm_agricultural_land.flatten()
    idx_land = dm_agricultural_land.idx
    idx_fert = DM_nitrogen["input"].idx
    dm_temp = (
        dm_agricultural_land.array[
            :, :, idx_land["agr_lus_land_agriculture"], np.newaxis
        ]
        * DM_nitrogen["input"].array[
            :, :, idx_fert["agr_climate-smart-crop_input-use"], :
        ]
    )
    DM_nitrogen["input"].add(
        dm_temp, dim="Variables", col_label="agr_input-use", unit="t"
    )

    # Mineral fertilizers [t] = sum Fertilizer application [t] (nitrogen + phosphate + potash)
    dm_mineral_fertilizer = DM_nitrogen["input"].filter(
        {
            "Variables": ["agr_input-use"],
            "Categories1": ["nitrogen", "phosphate", "potash"],
        }
    )
    dm_mineral_fertilizer.groupby(
        {"mineral": ".*"}, dim="Categories1", regex=True, inplace=True
    )

    # NO2 EMISSIONS ----------------------------------------------------------------------------------------------------
    # Mineral fertilizer emissions [tNO2] = input use nitrogen [tN] * fertilizer emission [N2O/N]
    dm_nitrogen = DM_nitrogen["input"].filter(
        {"Variables": ["agr_input-use"], "Categories1": ["nitrogen"]}
    )
    dm_nitrogen = dm_nitrogen.flatten()
    DM_nitrogen["emissions"].append(dm_nitrogen, dim="Variables")
    DM_nitrogen["emissions"].operation(
        "agr_input-use_nitrogen",
        "*",
        "fxa_agr_emission_fertilizer",
        out_col="agr_crop_emission_N2O-emission_fertilizer_raw",
        unit="t",
    )

    # Calibration
    dm_n = DM_nitrogen["emissions"].filter(
        {"Variables": ["agr_crop_emission_N2O-emission_fertilizer_raw"]}
    )
    dm_cal_n = DM_nitrogen["cal_n"]
    dm_cal_n.change_unit(
        "cal_agr_crop_emission_N2O-emission_fertilizer",
        10**6,
        old_unit="Mt",
        new_unit="t",
    )  # Unit conversion [Mt] => [t]
    dm_cal_rates_n = calibration_rates(
        dm_n,
        dm_cal_n,
        calibration_start_year=1990,
        calibration_end_year=2023,
    ...
    dm_n.operation(
        "agr_crop_emission_N2O-emission_fertilizer_raw",
        "*",
        "cal_rate",
        dim="Variables",
        out_col="agr_crop_emission_N2O-emission_fertilizer",
        unit="t",
    )
    ...
    # CO2 EMISSIONS ----------------------------------------------------------------------------------------------------
    # Pre processing
    dm_fertilizer_co = DM_nitrogen["input"].filter(
        {"Variables": ["agr_input-use"], "Categories1": ["liming", "urea"]}
    )
    ...
    dm_temp = (
        dm_fertilizer_co.array[:, :, idx_fert["agr_input-use"], :]
        * cdm_fertilizer_co.array[idx_cdm["cp_ef"], :]
    )
    dm_fertilizer_co.add(
        dm_temp, dim="Variables", col_label="agr_input-use_emissions-CO2", unit="t"
    )
...
def energy_ghg_workflow(
    ...
    array_temp = (
        ...
        * DM_energy_ghg["energy_demand"].array[
            :, :, idx_energy["agr_climate-smart-crop_energy-demand"], :
        ]
    )
    DM_energy_ghg["energy_demand"].add(
        array_temp, dim="Variables", col_label="agr_energy-demand_raw", unit="ktoe"
    )
    ...
    dm_energy_demand = DM_energy_ghg["energy_demand"].filter(
        {"Variables": ["agr_energy-demand_raw"]}
    )
    dm_cal_rates_energy_demand = calibration_rates(
        dm_energy_demand,
        dm_cal_energy_demand,
        calibration_start_year=1990,
        calibration_end_year=2023,
        years_setting=years_setting,
    )
    ...
    DM_energy_ghg["energy_demand"].operation(
        "agr_energy-demand_raw",
        "*",
        "cal_rate",
        dim="Variables",
        out_col="agr_energy-demand",
        unit="ktoe",
    )
    df_cal_rates_energy_demand = dm_to_database(
        dm_cal_rates_energy_demand, "none", "agriculture", level=0
    )
    ...
    idx_cdm = cdm_CO2.idx
    array_temp = (
        dm_energy.array[:, :, idx_energy["agr_energy-demand"], :]
        * cdm_CO2.array[idx_cdm["cp_emission-factor_CO2"], :]
    )
    DM_energy_ghg["energy_demand"].add(
        array_temp, dim="Variables", col_label="agr_input-use_emissions-CO2", unit="Mt"
    )

    # Overall CO2 emission from fuel [Mt] = sum (Energy direct emission [MtCO2])
    dm_CO2 = DM_energy_ghg["energy_demand"].filter(
        {"Variables": ["agr_input-use_emissions-CO2"]}
    )
    ...
    dm_CO2.change_unit(
        "agr_input-use_emissions-CO2_fuel", 10**6, old_unit="Mt", new_unit="t"
    ...
    dm_CO2.append(
        dm_fertilizer_co.filter({"Variables": ["agr_input-use_emissions-CO2"]}),
        dim="Categories1",
    )

    # Rename to _raw for calibration
    dm_CO2.rename_col(
        "agr_input-use_emissions-CO2",
        "agr_input-use_emissions-CO2_raw",
        dim="Variables",
    )
    ...
    dm_cal_CO2_input.change_unit(
        "cal_agr_input-use_emissions-CO2", 10**3, old_unit="kt", new_unit="t"
    )  # Unit conversion [kt] => [t]
    dm_cal_rates_CO2_input = calibration_rates(
        dm_CO2,
        dm_cal_CO2_input,
        calibration_start_year=1990,
        calibration_end_year=2023,
    ...
    dm_CO2.operation(
        ...
        "cal_rate",
        dim="Variables",
        out_col="agr_input-use_emissions-CO2",
        unit="t",
    )
    df_cal_rates_CO2_input = dm_to_database(
        dm_cal_rates_CO2_input, "none", "agriculture", level=0
    )

    # Overall CO2 emission [t] = sum (fuel, liming, urea)
    dm_fuel_input = dm_CO2.filter({"Variables": ["agr_input-use_emissions-CO2"]})
    dm_fuel_input.groupby(
        {"CO2-emission": ".*"}, dim="Categories1", regex=True, inplace=True
    )

    # Adding dummy columns
    dm_fuel_input.add(
        0.0, dummy=True, col_label="N2O-emission", dim="Categories1", unit="t"
    )
    dm_fuel_input.add(
        0.0, dummy=True, col_label="CH4-emission", dim="Categories1", unit="t"
    )
    ...
    dm_N2O_liv.groupby(
        {"N2O-emission": ".*"}, dim="Categories1", regex=True, inplace=True
    )
    # Adding dummy columns
    dm_N2O_liv.add(
        0.0, dummy=True, col_label="CO2-emission", dim="Categories1", unit="t"
    )
    dm_N2O_liv.add(
        0.0, dummy=True, col_label="CH4-emission", dim="Categories1", unit="t"
    )
    ...
    dm_CH4_liv.groupby(
        {"CH4-emission": ".*"}, dim="Categories1", regex=True, inplace=True
    )  # Problem
    # Adding dummy columns
    dm_CH4_liv.add(
        0.0, dummy=True, col_label="CO2-emission", dim="Categories1", unit="t"
    ...
    # Filter and format
    dm_N2O_fert = dm_n.filter(
        {"Variables": ["agr_crop_emission_N2O-emission_fertilizer"]}
    )
    dm_N2O_fert.rename_col(
        "agr_crop_emission_N2O-emission_fertilizer",
        "agr_fertilizer_N2O-emission",
        "Variables",
    )
    dm_N2O_fert.deepen()
    # Adding dummy columns
    ...
    dm_ghg.append(
        dm_N2O_liv, dim="Variables"
    )  # N2O, CH4 from crop residues with NO2 from livestock
    ...
    dm_ghg.operation(
        "agr_emission_residues",
        "+",
        "agr_liv_N2O-emission",
        out_col="residues_and_N2O_liv",
        unit="t",
    )
    dm_ghg.operation(
        "residues_and_N2O_liv",
        "+",
        "agr_liv_CH4-emission",
        out_col="residues_and_N2O_liv_and_CH4_liv",
        unit="t",
    )
    dm_ghg.operation(
        "residues_and_N2O_liv_and_CH4_liv",
        "+",
        "agr_rice_crop",
        out_col="residues_and_N2O_liv_and_CH4_liv_and_rice",
        unit="t",
    )
    dm_ghg.operation(
        "residues_and_N2O_liv_and_CH4_liv_and_rice",
        "+",
        "agr_input-use_emissions-CO2",
        out_col="residues_and_N2O_liv_and_CH4_liv_and_rice_and_CO2",
        unit="t",
    )
    dm_ghg.operation(
        "residues_and_N2O_liv_and_CH4_liv_and_rice_and_CO2",
        "+",
        "agr_fertilizer",
        out_col="agr_emissions_raw",
        unit="t",
    )
    ...
    dm_cal_rates_ghg = calibration_rates(
        dm_ghg,
        dm_cal_ghg,
        calibration_start_year=1990,
        calibration_end_year=2023,
    ...
    dm_input_use_CO2.change_unit(
        "agr_input-use_emissions-CO2", 1e-6, old_unit="t", new_unit="Mt"
    )
    dm_input_use_CO2 = dm_input_use_CO2.flatten()

    # Fertilizer emissions N2O
    dm_fertilizer_N2O = dm_n.filter(
        {"Variables": ["agr_crop_emission_N2O-emission_fertilizer"]}
    )
    dm_fertilizer_N2O.change_unit(
        "agr_crop_emission_N2O-emission_fertilizer", 1e-6, old_unit="t", new_unit="Mt"
    )
    # dm_fertilizer_N2O.rename_col('agr_crop_emission_N2O-emission', 'agr_emissions-N2O_crop_fertilizer', 'Variables')

    # Crop residue emissions
    dm_crop_residues = DM_crop["ef_residues"].filter(
        {
            "Variables": ["agr_crop_emission"],
    ...
    dm_crop_residues.rename_col(
        "burnt-residues", "crop_burnt-residues", dim="Categories1"
    )
    dm_crop_residues.rename_col(
        "soil-residues", "crop_soil-residues", dim="Categories1"
    )
    ...
    dm_CH4_liv_tpe.change_unit(
        "agr_liv_CH4-emission", 1e-6, old_unit="t", new_unit="Mt"
    )
    dm_CH4_liv_tpe.switch_categories_order(cat1="Categories2", cat2="Categories1")
    dm_CH4_liv_tpe.rename_col(
        "agr_liv_CH4-emission", "agr_emissions-CH4_liv", "Variables"
    )
    ...
    dm_N2O_liv_tpe.change_unit(
        "agr_liv_N2O-emission", 1e-6, old_unit="t", new_unit="Mt"
    )
    dm_N2O_liv_tpe.switch_categories_order(cat1="Categories2", cat2="Categories1")
    dm_N2O_liv_tpe.rename_col(
        "agr_liv_N2O-emission", "agr_emissions-N2O_liv", "Variables"
    )
    ...
    # Rice emissions
    dm_CH4_rice = DM_land["rice"].filter({"Variables": ["agr_rice_crop_CH4-emission"]})
    dm_CH4_rice.change_unit(
        "agr_rice_crop_CH4-emission", 1e-6, old_unit="t", new_unit="Mt"
    )
    dm_CH4_rice.rename_col(
        "agr_rice_crop_CH4-emission", "agr_emissions-CH4_crop_rice", "Variables"
    )
...
def agriculture(lever_setting, years_setting, DM_input, interface=Interface()):
    ...
    dm_lfs, df_cal_rates_diet = lifestyle_workflow(
    ...
    (
        ...
        df_cal_rates_liv_prod,
        df_cal_rates_liv_pop,
    ) = livestock_workflow(DM_livestock, CDM_const, dm_lfs_pro, years_setting)
    DM_alc_bev, dm_bev_ibp_cereal_feed, dm_bev_dom_prod = alcoholic_beverages_workflow(
        DM_alc_bev, CDM_const, dm_lfs_pro
    )
    DM_bioenergy, dm_oil, dm_lgn, dm_eth, dm_biofuel_fdk = bioenergy_workflow(
        DM_bioenergy, CDM_const, DM_ind, dm_bld, dm_tra
    )
    dm_liv_N2O, dm_CH4, df_cal_rates_liv_N2O, df_cal_rates_liv_CH4, DM_manure = (
        livestock_manure_workflow(
            DM_manure, DM_livestock, dm_liv_pop, CDM_const, years_setting
        )
    )
    DM_feed, dm_aps_ibp, dm_feed_req, dm_aps, dm_feed_demand = feed_workflow(
        DM_feed, dm_liv_prod, dm_bev_ibp_cereal_feed, CDM_const, years_setting
    )
    dm_voil, dm_aps_ibp_oil, dm_voil_tpe = biomass_allocation_workflow(
        dm_aps_ibp, dm_oil
    )
    (
        DM_crop,
    ...
    dm_n, dm_fertilizer_co, dm_mineral_fertilizer, df_cal_rates_n = nitrogen_workflow(
        DM_nitrogen, dm_land, CDM_const, years_setting
    )
    (
        DM_energy_ghg,
        dm_CO2,
...
def agriculture_local_run():
    ...
    DM_input = filter_country_and_load_data_from_pickles(
        country_list=country_list, modules_list="agriculture", filter_country=True
    )
