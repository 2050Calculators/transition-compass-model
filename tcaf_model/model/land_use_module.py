...
def read_data(DM_landuse_pickle, lever_setting):
    ...
    DM_grassland = {"ruminant_density": dm_livestock_density}
...
def simulate_crop_to_landuse_input():
    ...
    f = os.path.join(
        current_file_directory, "../_database/data/interface/crop_to_land-use.pickle"
    )
...
def cropland_workflow(dm_crop_prod, DM_cropland, years_setting):
    ...
    DM_cropland["crop-share"].switch_categories_order(
        cat1="Categories2", cat2="Categories1"
    )
    dm_crop_ch = dm_crop_prod.filter(
        {
    ...
    array_temp = (
        DM_cropland["yield-ch"]["Switzerland", :, "agr_crop_yield_organic", :]
    ...
    dm_cal_rates_cropland = calibration_rates(
        dm_crop_ch.filter({"Variables": ["agr_cropland_total_raw"]}),
        dm_cal_cropland,
    ...
    array_temp = (
        dm_crop_ch["Switzerland", :, "agr_cropland_total", np.newaxis, :]
        * DM_cropland["crop-share"]["Switzerland", :, "agr_share", :, :]
    )
    DM_cropland["crop-share"].add(
    ...
    dm_crop_share_area = DM_cropland["crop-share"].filter(
        {"Variables": ["agr_cropland_raw"]}
    )
    dm_cal_rates_crop_share_area = calibration_rates(
        dm_crop_share_area,
    ...
    DM_cropland["crop-share"].operation(
        "agr_cropland_raw",
        "*",
        "cal_rate",
        dim="Variables",
    ...
    DM_cropland["crop-share"].add(
        0.0,
        dummy=True,
        col_label="agr_domestic-production_afw",
        dim="Variables",
        unit="kcal",
    )
    array_org = (
    ...
    DM_cropland["crop-share"][:, :, "agr_domestic-production_afw", "organic", :] = (
        array_org
    )
    ...
    DM_cropland["crop-share"][:, :, "agr_domestic-production_afw", "extensive", :] = (
        array_org
    )
    ...
    DM_cropland["crop-share"][:, :, "agr_domestic-production_afw", "intensive", :] = (
        array_org
    )
    ...
    dm_cal_cropland_total_swiss.rename_col_regex(
        "agr_cropland", "agr_cropland_swiss", dim="Variables"
    )
...
def crop(lever_setting, years_setting, DM_input, write_pickle, interface=Interface()):
    ...
    current_file_directory = os.path.dirname(os.path.abspath(__file__))
    DM_ots_fts, DM_cropland, DM_grassland, CDM_const = read_data(
    ...
    # land-use to TCAF
    DM_landuse_to_TCAF = {
        "cropland-world": dm_crop_imports.filter({"Variables": ["agr_cropland_total"]}),
...
def landuse_local_run():
    ...
    DM_input = filter_country_and_load_data_from_pickles(
        country_list=country_list, modules_list="land-use", filter_country=False
    )
