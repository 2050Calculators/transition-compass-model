...
def compute_stock_fts(DM, years_ots, years_fts):
    ...
    for ti in future_years:
        ...
        dem_distr_tm1 = (
            dm_dem_distr.array[:, idx_dr[ti - 1], idx_dr["bld_to-demolish"], :, :]
            * s_tm1
        )
        ...
        r_t = arr_rr * (
            dm_renov_distr.array[
                :, idx_rd[ti], idx_rd["bld_renovation-redistribution-in"], np.newaxis, :
            ]
            + -dm_renov_distr.array[
                :,
                idx_rd[ti],
                idx_rd["bld_renovation-redistribution-out"],
                np.newaxis,
                :,
            ]
        )
        ...
        if mask_negative.any():
            ...
            ren_distr_in = dm_renov_distr.array[
                :, idx_rd[ti], idx_rd["bld_renovation-redistribution-in"], np.newaxis, :
            ]
        ...
        n_t = (
            ...
            * dm.array[:, idx[ti], idx["bld_building-mix_new"], ...]
        )
...
def bld_energy_workflow(DM_energy, dm_clm, dm_floor_area, cdm_const):
    ...
    arr = dm_clm.array[:, :, idx_c["clm_HDD"], np.newaxis, np.newaxis] + dm_clm.array[
        :, :, idx_c["clm_days-below-15"], np.newaxis, np.newaxis
    ...
    arr = dm_clm.array[:, :, idx_c["clm_CDD"], np.newaxis, np.newaxis] + dm_clm.array[
        :, :, idx_c["clm_days-above-24"], np.newaxis, np.newaxis
    ...
    # 24 * Us * A * m2
    arr_W_K = (
        dm_floor_area.array[:, :, idx["bld_floor-area_stock"], :, :]
    ...
    idx_c = dm_Tint.idx
    arr_energy = (
        dm_floor_area.array[:, :, idx["bld_power-per-K"], :, :]
        * dm_Tint.array[:, :, idx_c["bld_HDD-adj"], :, :]
        * 24
    )
    ...
    idx_t = dm_tech.idx
    arr = (
        dm_floor_area.array[:, :, idx["bld_heating"], :, :, np.newaxis]
    ...
    idx = dm_energy.idx
    arr = (
        dm_energy.array[:, :, idx["bld_heating"], :, :, :]
    ...
    arr_energy = (
        dm_floor_area.array[:, :, idx["bld_power-per-K"], :, :]
        * dm_Tint.array[:, :, idx_c["bld_CDD-adj"], :, :]
        * 24
    ...
    arr = (
        dm_floor_area.array[:, :, idx["bld_cooling"], :, :]
        / dm_eff.array[
            :, :, idx_e["bld_heating-efficiency"], np.newaxis, :, idx_e["heat-pump"]
        ]
    )
    idx = dm_energy.idx
    dm_energy.add(
    ...
    dm_energy.array[:, :, idx["bld_energy-demand_cooling"], :, :, idx["heat-pump"]] = (
        arr
    )
    ...
    arr = (
        dm_emissions.array[:, :, idx["bld_energy-demand_heating"], :, :, :]
        * cdm_emission.array[
            np.newaxis, np.newaxis, idx_e["bld_CO2-factors"], np.newaxis, np.newaxis, :
        ]
    )
    dm_emissions.add(
    ...
    arr = (
        dm_elec.array[:, :, idx["bld_energy-demand_heating"], :, :, :]
        * dm_emis_elec.array[
            :,
            :,
            idx_e["bld_CO2-factor"],
            idx_e["electricity"],
            np.newaxis,
            np.newaxis,
            np.newaxis,
